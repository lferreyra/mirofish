...
| **Python** | 3.11.x（必需） | 后端运行环境 | `python --version` |
...
> Intel Mac（`macOS x86_64`）说明：
> 项目已自动限制 `torch<2.3` 以规避 PyTorch 新版本缺少 x86_64 wheel 的问题。
> 如果你此前安装失败，请先清理旧环境再重试：
>
> ```bash

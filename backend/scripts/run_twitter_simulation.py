...
class UnicodeFormatter(logging.Formatter):
    """Logging formatter that converts Unicode escape sequences to readable characters."""
...
def setup_oasis_logging(log_dir: str):
    """Configure OASIS loggers to write to fixed-name log files."""
...
class TwitterSimulationRunner:
    ...
    def __init__(self, config_path: str, wait_for_commands: bool = True):
        ...
        Args:
            ...
            wait_for_commands: Stay alive for IPC commands after the simulation loop (default True)
        """
    ...
    def _get_profile_path(self) -> str:
        """Return the profile file path (OASIS Twitter uses CSV format)."""
        return os.path.join(self.simulation_dir, "twitter_profiles.csv")
    
    def _get_db_path(self) -> str:
        """Return the simulation database path."""
    ...
    def _create_model(self):
        ...
        Uses the project root .env file as the highest-priority source:
        - LLM_API_KEY: API key
    ...
    def _get_active_agents_for_round(
        ...
        Returns:
            List of (agent_id, agent) pairs that are active this round.
        """
    ...
    async def run(self, max_rounds: int = None):
        ...
        if initial_posts:
            print(f"Publishing initial posts ({len(initial_posts)} posts)...")
            initial_actions = {}
...
def setup_signal_handlers():
    ...
    (close database, environment, etc.) before exiting.
    """

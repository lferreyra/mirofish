"""
OASIS TwitterSimulação
ConfiguraçãoSimulação
...
class IPCHandler:
    ...
    async def handle_batch_interview(self, command_id: str, interviews: List[Dict]) -> bool:
        ...
        try:
            ...
            error_msg = str(e)
            print(f"  InterviewFalhou: {error_msg}")
            self.send_response(command_id, "failed", error=error_msg)
            return False
...
class TwitterSimulationRunner:
    ...
    def __init__(self, config_path: str, wait_for_commands: bool = True):
        ...
        Args:
            config_path: Configuração (simulation_config.json)
            wait_for_commands: SimulaçãoTrue
        """
    ...
    def _create_model(self):
        ...
        print(f"LLMConfiguração: model={llm_model}, base_url={llm_base_url[:40] if llm_base_url else ''}...")
    ...
    async def run(self, max_rounds: int = None):
        ...
        """
        print("=" * 60)
        print("OASIS TwitterSimulação")
        print(f"Configuração: {self.config_path}")
        print(f"ID da simulação: {self.config.get('simulation_id', 'unknown')}")
...
async def main():
    parser = argparse.ArgumentParser(description='OASIS TwitterSimulação')
    parser.add_argument(
        '--config', 
        type=str, 
        required=True,

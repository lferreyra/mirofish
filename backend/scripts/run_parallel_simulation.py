...
class ParallelIPCHandler:
    ...
    async def handle_interview(self, command_id: str, agent_id: int, prompt: str, platform: str = None) -> bool:
        ...
        if success_count > 0:
            ...
            print(f"  Interview done: agent_id={agent_id}, platforms succeeded={success_count}/{len(platforms_to_interview)}")
            return True
        else:
...
async def run_twitter_simulation(
    ...
    log_info(f"Simulation loop finished in {elapsed:.1f}s, total actions: {total_actions}")
...
async def main():
    ...
    parser.add_argument(
        ...
        action='store_true',
        default=False,
        help='Exit immediately after simulation (no wait-for-commands mode)'
    )
    ...
    log_manager.info(f"Wait-for-commands mode: {'on' if wait_for_commands else 'off'}")
    log_manager.info("=" * 60)
    ...
    log_manager.info(f"  - Total simulated hours: {total_hours}")
    log_manager.info(f"  - Minutes per round: {minutes_per_round}")
    log_manager.info(f"  - Configured total rounds: {config_total_rounds}")
    if args.max_rounds:
        log_manager.info(f"  - Max rounds cap: {args.max_rounds}")
        if args.max_rounds < config_total_rounds:
            log_manager.info(f"  - Effective rounds: {args.max_rounds} (truncated)")
    ...
    log_manager.info("=" * 60)
    log_manager.info(f"Simulation loop finished. Total time: {total_elapsed:.1f}s")
    
    # 是否进入等待命令模式
    if wait_for_commands:
        log_manager.info("")
        log_manager.info("=" * 60)
        log_manager.info("Waiting for commands — environment stays running")

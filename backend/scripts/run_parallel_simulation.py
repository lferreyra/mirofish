...
Runs Twitter and Reddit simulations simultaneously from a single config file.

Features:
- Dual-platform (Twitter + Reddit) parallel simulation
- Stays alive after the simulation loop, waiting for IPC commands
- Supports Interview commands for single and batch agents
...
# ============================================================
# Windows encoding fix: set UTF-8 before all other imports.
# This works around third-party libraries (e.g. OASIS) that
# open files without specifying an encoding.
# ============================================================
import sys
import os
...
if sys.platform == 'win32':
    ...
    def _utf8_open(file, mode='r', buffering=-1, encoding=None, errors=None,
        ...
        """Wrap open() to default to UTF-8 for text mode when no encoding is given."""
...
def disable_oasis_logging():
    """
    Silence OASIS library loggers.

    OASIS logs every agent observation and action, which is too verbose.
    We use our own action_logger instead.
    """
    oasis_loggers = [
        "social.agent",
        "social.twitter", 
...
# Available Twitter actions (INTERVIEW excluded; it must be triggered via ManualAction)
TWITTER_ACTIONS = [
...
# Available Reddit actions (INTERVIEW excluded; it must be triggered via ManualAction)
REDDIT_ACTIONS = [
    ActionType.LIKE_POST,
...
class ParallelIPCHandler:
    """
    IPC command handler for dual-platform simulations.

    Manages both platform environments and dispatches Interview commands.
    """
    ...
    def poll_command(self) -> Optional[Dict[str, Any]]:
        """Return the next pending command, or None if the queue is empty."""
    ...
    async def _interview_single_platform(self, agent_id: int, prompt: str, platform: str) -> Dict[str, Any]:
        ...
        Returns:
            Result dict, or a dict containing an "error" key on failure.
        """
    ...
    async def handle_interview(self, command_id: str, agent_id: int, prompt: str, platform: str = None) -> bool:
        ...
        Args:
            ...
            platform: Target platform (optional)
                ...
                - None: Interview on both platforms; returns combined result
...
# Maps database action names to canonical names
ACTION_TYPE_MAP = {
...
def get_agent_names_from_config(config: Dict[str, Any]) -> Dict[int, str]:
    ...
    This lets actions.jsonl show real entity names instead of "Agent_0" placeholders.
...
def fetch_new_actions_from_db(
    ...
    Args:
        ...
        last_rowid: Highest rowid already processed (rowid is used instead of
            created_at because the format differs between Twitter and Reddit)
        agent_names: agent_id -> agent_name mapping

    Returns:
        (actions_list, new_last_rowid)
        - actions_list: Each element has agent_id, agent_name, action_type,
          action_args (including enriched context)
        - new_last_rowid: Updated highest rowid
    """
    actions = []
...
def _enrich_action_context(
    ...
    """
    Enrich action_args in-place with post/comment content and author names.
...
def _get_post_info(
    ...
    """
    Fetch post content and author name.
...
def create_model(config: Dict[str, Any], use_boost: bool = False):
    ...
    Supports a dual-LLM configuration for parallel simulation throughput:
    - Primary: LLM_API_KEY, LLM_BASE_URL, LLM_MODEL_NAME
    - Boost (optional): LLM_BOOST_API_KEY, LLM_BOOST_BASE_URL, LLM_BOOST_MODEL_NAME

    When a boost LLM is configured, different platforms can use different API
    providers in parallel, increasing concurrency.

    Args:
        config: Simulation config dict
        use_boost: Use the boost LLM config if available
    """
...
class PlatformSimulation:
    """Container for the results of a single-platform simulation run."""
    def __init__(self):
        self.env = None
...
async def run_twitter_simulation(
    ...
    Args:
        ...
        max_rounds: Optional cap on the number of rounds to run

    Returns:
        PlatformSimulation containing env and agent_graph
    """
    ...
    if max_rounds is not None and max_rounds > 0:
        ...
        if total_rounds < original_rounds:
            log_info(f"Round count capped: {original_rounds} -> {total_rounds} (max_rounds={max_rounds})")
    ...
    # Environment is intentionally kept alive for Interview commands.
...
async def run_reddit_simulation(
    ...
    # Reddit uses the boost LLM config if available, otherwise falls back to primary
...
async def main():
    ...
    parser.add_argument(
        ...
        type=int,
        default=None,
        help='Maximum number of rounds to run (optional)'
    )
    parser.add_argument(
        '--no-wait',
        action='store_true',
        default=False,
        help='Close the environment immediately after simulation; do not wait for commands'
    )
    ...
    if args.max_rounds:
        ...
        if args.max_rounds < config_total_rounds:
            log_manager.info(f"  - Effective rounds: {args.max_rounds} (capped)")
...
def setup_signal_handlers(loop=None):
    """
    Install signal handlers for SIGTERM/SIGINT to allow graceful cleanup.

    After simulation finishes the process stays alive waiting for interview
    commands. On signal:
    1. Notify the asyncio loop to exit its wait
    2. Allow normal resource cleanup (close databases, environments, etc.)
    3. Then exit
    """
...
if __name__ == "__main__":
    ...
    try:
        ...
        # Clean up multiprocessing resource tracker to suppress exit warnings
        try:

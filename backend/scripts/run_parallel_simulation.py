"""
Dual-Platform Parallel Simulation Preset Script
...
# This fixes third-party libraries reading files without specifying encoding
# ============================================================
import sys
import os
...
if sys.platform == 'win32':
    ...
    # Reconfigure stdout streams to UTF-8 (fix console garbled text)
    if hasattr(sys.stdout, 'reconfigure'):
        sys.stdout.reconfigure(encoding='utf-8', errors='replace')
    if hasattr(sys.stderr, 'reconfigure'):
        sys.stderr.reconfigure(encoding='utf-8', errors='replace')

    # Force default encoding for open() function
    import builtins
    ...
    def _utf8_open(file, mode='r', buffering=-1, encoding=None, errors=None,
        ...
        Fixes third-party libraries reading files without specifying encoding.
        """
...
from app.simulation.agent_graph import (
    generate_twitter_agent_graph,
    generate_reddit_agent_graph,
)


# Twitter available actions (INTERVIEW is only triggered manually via ManualAction)
TWITTER_ACTIONS = [
...
class ParallelIPCHandler:
    ...
    async def handle_interview(self, command_id: str, agent_id: int, prompt: str, platform: str = None) -> bool:
        ...
        Args:
            ...
            platform: Specified platform (optional)
                ...
                - None/unspecified: Interview on both platforms, return combined results
    ...
    async def handle_batch_interview(self, command_id: str, interviews: List[Dict], platform: str = None) -> bool:
        ...
        Args:
            ...
            platform: Default platform (can be overridden per interview)
                - "twitter": Interview on Twitter platform only
                - "reddit": Interview on Reddit platform only
...
# Non-essential action types to filter (low analytical value)
...
def get_agent_names_from_config(config: Dict[str, Any]) -> Dict[int, str]:
    ...
    This allows displaying real entity names in actions.jsonl
    instead of generic codes like "Agent_0".
...
def fetch_new_actions_from_db(
    ...
    Args:
        ...
        last_rowid: Last processed rowid value (using rowid instead of created_at
                    because different platforms have different created_at formats)
        agent_names: agent_id -> agent_name mapping

    Returns:
        (actions_list, new_last_rowid)
        - actions_list: Action list, each element contains agent_id, agent_name,
                       action_type, action_args (with context info)
    ...
    try:
        ...
        for rowid, user_id, action, info_json in cursor.fetchall():
            ...
            # Simplify action_args, keep only key fields (full content, no truncation)
            simplified_args = {}
...
def _enrich_action_context(
    ...
    try:
        ...
        # Context enrichment failure should not affect the main flow
...
def create_model(config: Dict[str, Any], use_boost: bool = False):
    ...
    The use_boost parameter is kept for API compatibility but has no effect
    since all calls go through the same Claude CLI.

    Args:
        config: Simulation config dict
        use_boost: Kept for API compatibility (ignored)
    """
    config_label = "[Boost LLM]" if use_boost else "[Standard LLM]"
    print(f"{config_label} Using Claude CLI model")
...
async def run_twitter_simulation(
    ...
    # Get real agent names from config (use entity_name instead of default Agent_X)
    ...
    for round_num in range(total_rounds):
        ...
        # Fetch actual executed actions from database and log them
...
async def run_reddit_simulation(
    ...
    # Reddit uses boost LLM config (if available, otherwise falls back to standard)
...
async def main():
    parser = argparse.ArgumentParser(description='Dual-Platform Parallel Simulation')
    parser.add_argument(
        '--config',
        type=str,
        required=True,
    ...
    log_manager.info("=" * 60)
    log_manager.info("Dual-Platform Parallel Simulation")
...
def setup_signal_handlers(loop=None):
    ...
    Persistent simulation scenario: simulation stays running after completion,
    waiting for interview commands. When a termination signal is received:

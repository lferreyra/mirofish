...
if sys.platform == 'win32':
    ...
    def _utf8_open(file, mode='r', buffering=-1, encoding=None, errors=None,
        ...
        """Default to UTF-8 for text mode when encoding not specified (fixes OASIS etc.)."""
...
def disable_oasis_logging():
    """Turn off OASIS verbose logging; we use our own action_logger."""
    # Set OASIS loggers to CRITICAL only
    oasis_loggers = [
        "social.agent",
        "social.twitter", 
        "social.rec",
...
def init_logging_for_simulation(simulation_dir: str):
    """Initialize simulation logging. Args: simulation_dir."""
...
# Twitter allowed actions (INTERVIEW excluded; INTERVIEW is only via ManualAction)
TWITTER_ACTIONS = [
...
# Reddit allowed actions (INTERVIEW excluded; INTERVIEW is only via ManualAction)
REDDIT_ACTIONS = [
    ActionType.LIKE_POST,
...
class ParallelIPCHandler:
    ...
    Manages both platforms' environments and handles Interview commands.
    """
    ...
    def _get_env_and_graph(self, platform: str):
        """
        Get env and agent_graph for the given platform.
    ...
    async def _interview_single_platform(self, agent_id: int, prompt: str, platform: str) -> Dict[str, Any]:
        ...
        Returns:
            Dict with result or with "error" key.
        """
    ...
    async def handle_interview(self, command_id: str, agent_id: int, prompt: str, platform: str = None) -> bool:
        ...
        Args:
            ...
            platform: Optional. "twitter" | "reddit" | None (both platforms, combined result).
    ...
    async def handle_batch_interview(self, command_id: str, interviews: List[Dict], platform: str = None) -> bool:
        ...
        Args:
            ...
            platform: Default platform (overridable per item). "twitter" | "reddit" | None (both per agent).
        """
        twitter_interviews = []
        reddit_interviews = []
    ...
    def _get_interview_result(self, agent_id: int, platform: str) -> Dict[str, Any]:
        """Load latest Interview result from DB."""
...
def get_agent_names_from_config(config: Dict[str, Any]) -> Dict[int, str]:
    """
    Build agent_id -> entity_name from simulation_config for use in actions.jsonl.

    Args:
        config: simulation_config.json content.

    Returns:
        agent_id -> entity_name mapping.
    """
    agent_names = {}
...
def fetch_new_actions_from_db(
    ...
    """
    Fetch new actions from DB and enrich with full context.

    Args:
        db_path: DB file path.
        last_rowid: Last max rowid (rowid used to avoid created_at format differences across platforms).
        agent_names: agent_id -> agent_name map.

    Returns:
        (actions_list, new_last_rowid). actions_list items have agent_id, agent_name, action_type, action_args.
    """
    actions = []
    ...
    try:
        ...
        print(f"Failed to read actions from DB: {e}")
...
def _get_post_info(
    ...
    """Get post content and author name from DB."""
    try:
        cursor.execute("""
...
def _get_comment_info(
    ...
    """Get comment content and author name from DB."""
    try:
        cursor.execute("""
...
def create_model(config: Dict[str, Any], use_boost: bool = False):
    ...
    if not os.environ.get("OPENAI_API_KEY"):
        raise ValueError("LLM API key not set; set LLM_API_KEY in project root .env")
...
def get_active_agents_for_round(
    ...
    """Select which agents are active this round based on time and config."""
...
class PlatformSimulation:
    """Container for platform simulation result (env, agent_graph, total_actions)."""
    def __init__(self):
        self.env = None
...
async def run_twitter_simulation(
    ...
    if initial_posts:
        ...
        if initial_actions:
            ...
            log_info(f"Posted {len(initial_actions)} initial posts")
...
async def main():
    ...
    log_manager.info("Simulation params:")
    log_manager.info(f"  - Total sim hours: {total_hours}")
    ...
    if args.max_rounds:
        ...
        if args.max_rounds < config_total_rounds:
            log_manager.info(f"  - Actual rounds: {args.max_rounds} (capped)")
    ...
    log_manager.info("=" * 60)
    log_manager.info(f"Simulation loop done. Total elapsed: {total_elapsed:.1f}s")

    if wait_for_commands:
        log_manager.info("")
        log_manager.info("=" * 60)
        log_manager.info("Entering wait-for-commands mode - environments kept running")

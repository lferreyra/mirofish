...
Uses LLM to auto-generate detailed simulation parameters based on simulation requirements, document content, and graph info
Fully automated, no manual parameter setting needed

Uses step-by-step generation strategy to avoid failure from generating too-long content at once:
...
class EventConfig:
    """Event config"""
    # Initial events (triggered when simulation starts)
...
class SimulationConfigGenerator:
    ...
    def generate_config(
        ...
        """
        logger.info(f"Starting intelligent config generation: simulation_id={simulation_id}, entity_count={len(entities)}")
        ...
        reasoning_parts.append(f"Initial post assignment: {assigned_count} posts assigned posters")
    ...
    def _build_context(
        ...
        remaining_length = self.MAX_CONTEXT_LENGTH - current_length - 500  # Leave 500 char margin
    ...
    def _call_llm_with_retry(self, prompt: str, system_prompt: str) -> Dict[str, Any]:
        """LLM call with retry, including JSON fix logic"""
        import re
...
- Peak hours at 19-22 (activity coefficient 1.5)
- Activity declines after 23:00 (activity coefficient 0.5)
- General pattern: low activity at dawn, gradually increasing in morning, medium during work hours, peak in evening
- **Important**: The example values below are for reference only; you need to adjust specific time periods based on event nature and participant characteristics
  - Example: student peak may be 21-23; media active all day; official agencies only during work hours
...
- reasoning (string): Brief explanation of why this config was chosen"""
    ...
    def _parse_time_config(self, result: Dict[str, Any], num_entities: int) -> TimeSimulationConfig:
        ...
        # Validate and correct: ensure doesn't exceed total agent count
...
**Important**: poster_type must be chosen from the "Available Entity Types" above, so initial posts can be assigned to appropriate Agents.
...
{{
    ...
    def _generate_agent_configs_batch(
        ...
        prompt = f"""Based on the following info, generate social media activity configs for each entity.

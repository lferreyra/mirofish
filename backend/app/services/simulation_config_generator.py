...
### General Principles (for reference only — adjust flexibly based on the specific event and participant groups):
- The user base follows typical daily activity patterns in the Beijing time zone
- 0-5 AM: almost no activity (activity multiplier 0.05)
- 6-8 AM: activity gradually increases (activity multiplier 0.4)
...
- General pattern: low activity at night, gradual increase in the morning, moderate during work hours, peak in the evening
- **Important**: The example values below are for reference only. You must adjust specific time slots based on the nature of the event and participant demographics.
  - For example: student groups may peak at 9-11 PM; media outlets may be active throughout the day; official organizations may only be active during work hours.
  - For example: a breaking hot topic may trigger discussions even late at night, so off_peak_hours can be shortened accordingly.
...
- peak_hours (int array): Peak activity hours, adjusted based on participant demographics.
...
- reasoning (string): Brief explanation of why this configuration was chosen."""
        ...
        system_prompt = "You are a social media simulation expert. Return pure JSON format. Time configuration should reflect typical daily activity patterns."
...
**Important**: poster_type must be chosen from the "Available Entity Types" above, so that initial posts can be assigned to appropriate agents.
For example: official statements should be posted by Official/University types, news by MediaOutlet, and student opinions by Student.
...
{{
    ...
    "reasoning": "<Brief explanation>"
        ...
        system_prompt = "You are a public opinion analysis expert. Return pure JSON format. Note that poster_type must exactly match one of the available entity types."
...
- **Timing should reflect typical daily activity patterns**: Almost no activity from 0-5 AM; most active from 7-10 PM.
...
- **Media** (MediaOutlet): Moderate activity (0.4-0.6), active throughout the day (8-23), fast response (5-30 minutes), high influence (2.0-2.5).
...
{{
    "agent_configs": [
        {{
            ...
            "active_hours": [<List of active hours, reflecting typical daily activity patterns>],
        ...
        system_prompt = "You are a social media behavior analysis expert. Return pure JSON. Configurations should reflect typical daily activity patterns."

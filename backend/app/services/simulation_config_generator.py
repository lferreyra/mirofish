...
logger = get_logger('arus.simulation_config')
...
DEFAULT_TIMEZONE_CONFIG = {
    ...
    "activity_multipliers": {
        "dead": 0.05,      # Almost nobody late at night
...
class TimeSimulationConfig:
    ...
    # Time per round (simulated minutes) - default 60 minutes (1 hour)
    ...
    # Peak hours (evening 19-22, most active period)
    peak_hours: List[int] = field(default_factory=lambda: [19, 20, 21, 22])
    peak_activity_multiplier: float = 1.5

    # Off-peak hours (late night 0-5, almost no activity)
    off_peak_hours: List[int] = field(default_factory=lambda: [0, 1, 2, 3, 4, 5])
    off_peak_activity_multiplier: float = 0.05  # Very low activity late at night
...
class SimulationConfigGenerator:
    ...
    def _generate_agent_config_by_rule(self, entity: EntityNode) -> Dict[str, Any]:
        """Generate a single Agent configuration by rule (general daily patterns)"""

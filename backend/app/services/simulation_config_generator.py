...
Employs a step-by-step generation strategy to avoid failures caused by
generating excessively long content in a single pass:
...
class TimeSimulationConfig:
    """Time simulation configuration (based on typical daily schedule patterns)."""
    # Total simulation duration (simulated hours)
    total_simulation_hours: int = 72  # Default: simulate 72 hours (3 days)
    
    # Time represented per round (simulated minutes) — default 60 min (1 hour), accelerates time flow
    ...
    # Peak hours (evening 19:00-22:00, the most active period)
    peak_hours: List[int] = field(default_factory=lambda: [19, 20, 21, 22])
    peak_activity_multiplier: float = 1.5
    
    # Off-peak hours (midnight 0:00-5:00, almost no activity)
...
class PlatformConfig:
    ...
    # Viral spread threshold (number of interactions to trigger viral spread)
    viral_threshold: int = 10
    
    # Echo chamber strength (degree of clustering of similar opinions)
...
class SimulationConfigGenerator:
    ...
    Employs a step-by-step generation strategy:
    1. Generate time configuration and event configuration (lightweight)
    2. Generate agent configurations in batches (10-20 agents per batch)
    ...
    AGENT_SUMMARY_LENGTH = 300           # Entity summary used in agent configuration
...
- reasoning (string): Brief explanation of why this configuration was chosen"""
        ...
        system_prompt = "You are a social media simulation expert. Return pure JSON format; time configuration should follow typical daily activity patterns."
    ...
    def _get_default_time_config(self, num_entities: int) -> Dict[str, Any]:
        """Get default time configuration (standard daily activity patterns)"""
        return {
            "total_simulation_hours": 72,
...
{{
    "agent_configs": [
        ...
        for i, entity in enumerate(entities):
            ...
            # If LLM did not generate a config, fall back to rule-based generation
            if not cfg:
    ...
    def _generate_agent_config_by_rule(self, entity: EntityNode) -> Dict[str, Any]:
        """Generate a single agent configuration using rules (standard daily activity patterns)"""
        ...
        if entity_type in ["university", "governmentagency", "ngo"]:
            ...
            # Experts/Professors: active during work and evening, moderate frequency
            return {
                "activity_level": 0.4,
                "posts_per_hour": 0.3,

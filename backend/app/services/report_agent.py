...
2. Plans the table of contents structure first, then generates content section by section
3. Each section uses multi-round ReACT thinking and reflection
4. Supports user dialogue with autonomous tool-calling for retrieval
"""
...
class ReportLogger:
    ...
    Generates an agent_log.jsonl file in the report folder, recording each step's detailed actions.
    ...
    def log_section_full_complete(
        ...
        The frontend should monitor this log to determine when a section is truly complete and retrieve full content
        """
        self.log(
...
class ReportConsoleLogger:
    ...
    These logs differ from agent_log.jsonl in that they are plain text console output format.
    """
    ...
    def _setup_file_handler(self):
        ...
        # Use the same concise format as console output
...
This tool is used to obtain a complete overview of simulation results, especially suited for understanding event evolution. It will:
...
[Use Cases]
- Need to understand the complete development context of an event
...
This is not LLM simulation, but calls the real interview interface to get original responses from simulation agents.
...
[Use Cases]
- Need to understand views on events from different role perspectives (What do students think? What does media say? What do officials say?)
...
PLAN_SYSTEM_PROMPT = """\
You are an expert writer of "Future Prediction Reports," possessing a "god's-eye view" of the simulated world — you can observe the behavior, statements, and interactions of every Agent in the simulation.

[Core Concept]
We have constructed a simulated world and injected specific "simulation requirements" as variables. The evolution of the simulated world is a prediction of what may happen in the future. What you are observing is not "experimental data," but a "rehearsal of the future."
...
2. [Must Quote Agent's Original Speech and Actions]
   - Agent speech and behavior are predictions of future human behavior
   - Present these predictions using quote format in the report, for example:
...
3. [Language Consistency - Quoted Content Must Be Translated to Report Language]
   ...
   - When quoting English or mixed-language content returned by tools, translate it to fluent Chinese before writing into the report
...
4. [Faithfully Present Prediction Results]
   ...
   - If information is insufficient in some areas, state this honestly
...
- Each section is the minimum division unit of the report
- ❌ Prohibited from using any Markdown headings within sections (#, ##, ###, #### etc.)
- ❌ Prohibited from adding the section main heading at the start of content
- ✅ Section headings are automatically added by the system; you only need to write pure body text
- ✅ Use **bold**, paragraph breaks, quotes, and lists to organize content, but no headings

[Correct Example]
```
This section analyzes the public opinion dissemination of the event. Through deep analysis of simulation data, we found...

**Initial Explosion Phase**

Weibo, as the first scene of public opinion, played a core role in initial information release:

> "Weibo contributed 68% of the initial reach..."
...
When you have retrieved sufficient information through tools, start output with "Final Answer:" followed by the section content.

⚠️ Strictly Prohibited:
- Prohibited from including both a tool call and Final Answer in a single reply
...
2. Extensively quote original text to showcase simulation effects
3. Use Markdown format (but prohibited from using headings):
...
4. [Quote Format Requirements - Must Be Standalone Paragraphs]
   Quotes must stand alone as paragraphs with a blank line before and after; cannot be mixed into paragraphs:
   ...
   ❌ Incorrect format:
   ```
   The school's response was considered to lack substantive content. > "The school's response mode..." This evaluation reflects...
   ```
...
2. Must call tools first to retrieve simulation data before starting
...
- ✅ Section heading is automatically added by the system
...
3. After collecting sufficient information, output Final Answer (pure body text, no headings at all)"""
...
REACT_INSUFFICIENT_TOOLS_MSG = (
    "[Notice] You have only called {tool_calls_count} tools, at least {min_tool_calls} are required. "
...
REACT_UNUSED_TOOLS_HINT = "\n💡 You have not yet used: {unused_list} — recommend trying different tools for multi-perspective information"
...
2. Answer questions directly, avoid lengthy reasoning exposition
...
[Answer Style]
- Concise and direct, avoid lengthy elaboration
...
class ReportAgent:
    ...
    def _execute_tool(self, tool_name: str, parameters: Dict[str, Any], report_context: str = "") -> str:
        ...
        try:
            if tool_name == "insight_forge":
                ...
                # Deep interview - calls real OASIS interview API to get simulation agent responses (dual platform)
    ...
    # Valid tool name set, used for validation in bare JSON fallback parsing
    ...
    def _parse_tool_calls(self, response: str) -> List[Dict[str, Any]]:
        ...
        # Format 2: Fallback - LLM outputs bare JSON directly (no <tool_call> tags)
        # Only attempted when format 1 does not match, to avoid mismatching JSON in body text
    ...
    def _generate_section_react(
        ...
        for iteration in range(max_iterations):
            ...
            if has_tool_calls and has_final_answer:
                ...
                if conflict_retries <= 2:
                    # First two times: discard this response, require LLM to re-reply
                    ...
                    logger.warning(
                        ...
                        "downgrading to truncating and executing first tool call"
                    )
            ...
            if has_tool_calls:
                # Tool quota exhausted → inform explicitly, require Final Answer output
            ...
            # Directly use this content as the final answer without idling
            logger.info(f"Section {section.title}: 'Final Answer:' prefix not detected, using LLM output directly as final content (tool calls: {tool_calls_count})")
        ...
        if response is None:
            logger.error(f"Section {section.title} force completion: LLM returned None, using default error message")
    ...
    def generate_report(
        ...
        Each section is immediately saved to the folder after generation, without waiting for the entire report.
        File structure:
        reports/{report_id}/
...
class ReportManager:
    ...
    def get_console_log(cls, report_id: str, from_line: int = 0) -> Dict[str, Any]:
        ...
        Args:
            report_id: Report ID
            from_line: Line to start reading from (for incremental retrieval, 0 means from beginning)
        ...
        return {
            ...
            "from_line": from_line,
            "has_more": False  # Read to end of file
        }
    ...
    def get_agent_log_stream(cls, report_id: str) -> List[Dict[str, Any]]:
        """
        Get complete Agent log (for one-time retrieval of all entries)
    ...
    def save_outline(cls, report_id: str, outline: ReportOutline) -> None:
        ...
        Call immediately after planning phase is complete
        """
    ...
    def _clean_section_content(cls, content: str, section_title: str) -> str:
        ...
        for i, line in enumerate(lines):
            ...
            if heading_match:
                ...
                # Check if it duplicates the section title (skip duplicates within first 5 lines)
                if i < 5:
                ...
                # Because section headings are added by the system, content should have no headings
    ...
    def assemble_full_report(cls, report_id: str, outline: ReportOutline) -> str:
        ...
        Assembles complete report from saved section files with heading cleanup
        """
        folder = cls._get_report_folder(report_id)
    ...
    def _post_process_report(cls, content: str, outline: ReportOutline) -> str:
        ...
        2. Preserve report main heading (#) and section headings (##), remove other heading levels (###, #### etc.)
        ...
        while i < len(lines):
            ...
            if heading_match:
                ...
                # Check if it is a duplicate heading (same content within 5 consecutive lines)
                is_duplicate = False
                ...
                # - # (level=1) preserve only report main heading
                # - ## (level=2) preserve section headings
                # - ### and below (level>=3) convert to bold text

                if level == 1:
                    if title == outline.title:
                        # Preserve report main heading
                ...
                # Preserve only one blank line after headings
        ...
        # Clean consecutive multiple blank lines (preserve at most 2)
        result_lines = []
        empty_count = 0

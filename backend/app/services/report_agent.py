...
Uses LangChain + Zep for ReACT-pattern simulation report generation
...
4. Support user conversation with autonomous retrieval tool calls
"""
...
class ReportLogger:
    ...
    Each line is a complete JSON object with timestamp, action type, detailed content, etc.
    """
...
[Returns]
- Related fact originals (can be directly cited)
- Core entity insights
- Relationship chain analysis"""

TOOL_DESC_PANORAMA_SEARCH = """\
[Broad Search - Full Picture View]
This tool is for getting a complete picture of simulation results, especially suitable for understanding event evolution. It will:
...
[Use Cases]
- Need to understand the complete development trajectory
- Need to compare opinion changes across different phases
...
This is not LLM simulation, but calls the real interview API to get simulation Agents' original responses.
...
- Need real responses from simulation Agents (from OASIS simulation environment)
...
PLAN_SYSTEM_PROMPT = """\
You are an expert in writing "future prediction reports," with a "God's eye view" of the simulation world -- you can observe the behavior, speech, and interactions of every Agent in the simulation.
...
Please output JSON format report outline as follows:
{
    "title": "Report Title",
...
Variables injected into simulation world (simulation requirements):{simulation_requirement}
...
1. Under our set conditions, what state does the future present?
...
1. [Must Call Tools to Observe Simulation World]
   ...
   - Call tools at least 3 times (max 5) per section to observe the simulation world, which represents the future

2. [Must Cite Agents' Original Behavior]
   - Agent speech and behavior are predictions of future human behavior
   - Use citation format in the report to show these predictions, e.g.:
...
3. [Language Consistency - Citations Must Be Translated to Report Language]
   ...
   - When citing English or mixed content from tools, translate to fluent Chinese before including in report
...
4. [Faithfully Present Prediction Results]
   ...
   - If information is insufficient in some aspect, state so honestly
...
- Section titles are added automatically by the system, you only need to write body content
- Use **bold**, paragraph breaks, citations, lists to organize content, but do not use headings
...
Weibo, as the primary scene of public opinion, serves as the core function of initial information release:

> "Weibo contributed 68% of initial voice volume..."

**Emotion Amplification Phase**

TikTok platform further amplified the event's impact:
...
- insight_forge: Deep insight analysis, auto-decomposes questions and retrieves facts and relationships multi-dimensionally
...
4. [Citation Format - Must Be Standalone Paragraph]
   Citations must be standalone paragraphs, with a blank line before and after, not mixed within paragraphs:
   ...
   Incorrect format:
   ```
   The institution's response was considered lacking in substance.> "The institution's response pattern..." This assessment reflects...
   ```
...
[Important Reminders]
1. Carefully read completed sections above, avoid repeating the same content!
...
- If information is sufficient: output section content starting with "Final Answer:" (must cite above originals)
- If more information needed: call a tool to continue retrieval
═══════════════════════════════════════════════════════════════"""

REACT_INSUFFICIENT_TOOLS_MSG = (
    "[Note] You have only called tools {tool_calls_count} times, at least {min_tool_calls} times needed."
    "Please call more tools to get more simulation data before outputting Final Answer.{unused_hint}"
)

REACT_INSUFFICIENT_TOOLS_MSG_ALT = (
    "Currently only called tools {tool_calls_count} times, at least {min_tool_calls} times needed."
    "Please call tools to get simulation data.{unused_hint}"
)

REACT_TOOL_LIMIT_MSG = (
    "Tool call count has reached the limit ({tool_calls_count}/{max_tool_calls}), cannot call more tools."
    'Please immediately output section content starting with "Final Answer:" based on information already obtained.'
)

REACT_UNUSED_TOOLS_HINT = "\n💡 You have not yet used: {unused_list}, suggest trying different tools for multi-angle information"
...
[Answer Style]
- Concise and direct, no lengthy prose
...
class ReportAgent:
    ...
    def __init__(
        ...
        graphiti_tools: Optional[GraphitiToolsService] = None
    ):
        """
        ...
        Args:
            ...
            graphiti_tools: Graphiti tools service (optional)
        """
        ...
        self.graphiti_tools = graphiti_tools or GraphitiToolsService()
    ...
    def _define_tools(self) -> Dict[str, Dict[str, Any]]:
        ...
        return {
            ...
            "interview_agents": {
                ...
                "parameters": {
                    "interview_topic": "Interview topic or requirement description (e.g., 'understand students views on the event')",
    ...
    def _execute_tool(self, tool_name: str, parameters: Dict[str, Any], report_context: str = "") -> str:
        ...
        try:
            if tool_name == "insight_forge":
                ...
                result = self.graphiti_tools.insight_forge(
                    graph_id=self.graph_id,
                    query=query,
                    simulation_requirement=self.simulation_requirement,
                    report_context=ctx
                ...
                result = self.graphiti_tools.panorama_search(
                    graph_id=self.graph_id,
                    query=query,
                ...
                if isinstance(limit, str):
                    limit = int(limit)
                result = self.graphiti_tools.quick_search(
                    graph_id=self.graph_id,
                    query=query,
                    limit=limit
                )
                ...
                result = self.graphiti_tools.interview_agents(
                    simulation_id=self.simulation_id,
            ...
            # ========== Backward-compatible legacy tools (internally redirected to new tools) ==========
                ...
                result = self.graphiti_tools.get_graph_statistics(self.graph_id)
                return json.dumps(result, ensure_ascii=False, indent=2)
            
            elif tool_name == "get_entity_summary":
                entity_name = parameters.get("entity_name", "")
                result = self.graphiti_tools.get_entity_summary(
                    graph_id=self.graph_id,
                ...
                nodes = self.graphiti_tools.get_entities_by_type(
                    graph_id=self.graph_id,
    ...
    # Valid tool name set, for validation during bare JSON fallback parsing
    ...
    def _parse_tool_calls(self, response: str) -> List[Dict[str, Any]]:
        ...
        # Only attempted when Format 1 has no matches, to avoid false matches on JSON in body text
    ...
    def plan_outline(
        ...
        context = self.graphiti_tools.get_simulation_context(
            graph_id=self.graph_id,
            simulation_requirement=self.simulation_requirement
        )
    ...
    def _generate_section_react(
        ...
        conflict_retries = 0  # Consecutive conflict count for tool call and Final Answer appearing together
        ...
        for iteration in range(max_iterations):
            ...
            if has_tool_calls:
                # Tool quota exhausted -> clearly inform, request Final Answer output
            ...
            # Use this content directly as final answer, no more spinning
...
class ReportManager:
    ...
    def _clean_section_content(cls, content: str, section_title: str) -> str:
        ...
        for i, line in enumerate(lines):
            ...
            if heading_match:
                ...
                # Check if this is a heading that duplicates the section title (skip duplicates in first 5 lines)
                if i < 5:
    ...
    def _post_process_report(cls, content: str, outline: ReportOutline) -> str:
        ...
        while i < len(lines):
            ...
            if heading_match:
                ...
                # Check if this is a duplicate heading (same title within 5 consecutive lines)
                is_duplicate = False
                ...
                if is_duplicate:
                    # Skip duplicate heading and its trailing blank lines
                    i += 1
                ...
                # Skip divider immediately after heading
                i += 1
                continue
    ...
    def get_report(cls, report_id: str) -> Optional[Report]:
        ...
        if not os.path.exists(path):
            # Legacy format compatibility: check files stored directly in reports dir
        ...
        for item in os.listdir(cls.REPORTS_DIR):
            ...
            if os.path.isdir(item_path):
                ...
                if report and report.simulation_id == simulation_id:
                    return report
            # Legacy format compatibility: JSON files

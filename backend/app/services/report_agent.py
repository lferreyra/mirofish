...
4. Support conversations with users, autonomously calling retrieval tools during chat
"""
...
class ReportLogger:
    ...
    def log_section_content(
        ...
        """Record section content generation complete (only content, does not mean the entire section is done)"""
        self.log(
    ...
    def log_section_full_complete(
        ...
        The frontend should listen for this log to determine whether a section is truly complete
...
[Returns]
- Relevant fact excerpts (can be directly quoted)
...
[Core Concept]
We have built a simulation world and injected specific "simulation requirements" as variables. The evolution results of the simulation world are predictions of what might happen in the future. What you are observing is not "experimental data," but a "rehearsal of the future."
...
1. [Must Call Tools to Observe the Simulation World]
   ...
   - All content must come from events and Agent behaviors that occurred in the simulation world
...
3. [Language Consistency - Quoted Content Must Match the Report Language]
   ...
   - The report language should match the language of the simulation requirements and source material
   - When quoting content in a different language from the report, translate it fluently before writing it into the report
...
- The section title is automatically added by the system; you only need to write plain body text
...
As the first scene of public opinion, the platform served the core function of information distribution:

> "The platform contributed 68% of the initial discussion volume..."

**Emotion Amplification Phase**

The video platform further amplified the event's impact:
...
- insight_forge: Deep insight analysis, automatically decompose questions and retrieve facts and relationships across multiple dimensions
...
Each reply can only do one of the following two things (cannot do both simultaneously):
...
REACT_UNUSED_TOOLS_HINT = "\nHint: You haven't used: {unused_list}. Consider trying different tools for multi-angle information."
...
class ReportAgent:
    ...
    def _define_tools(self) -> Dict[str, Dict[str, Any]]:
        ...
        return {
            ...
            "interview_agents": {
                ...
                "parameters": {
                    "interview_topic": "Interview topic or requirement description (e.g., 'understand students' views on the dormitory event')",
    ...
    def _parse_tool_calls(self, response: str) -> List[Dict[str, Any]]:
        ...
        # Only attempt when format 1 doesn't match, to avoid false matches in body text
    ...
    def _generate_section_react(
        ...
        # Build user prompt - each completed section limited to max 4000 characters
        ...
        for iteration in range(max_iterations):
            ...
            if has_tool_calls and has_final_answer:
                ...
                logger.warning(
                    ...
                    f"LLM simultaneously output tool calls and Final Answer (conflict #{conflict_retries})"
                )
                ...
                if conflict_retries <= 2:
                    ...
                    messages.append({
                        "role": "user",
                        "content": (
                            "[Format Error] You included both tool calls and Final Answer in one reply, which is not allowed.\n"
                    ...
                    # Third time: degrade handling, truncate to first tool call and force execute
                    logger.warning(
            ...
            # Directly adopt this content as the final answer, no more spinning
...
class ReportManager:
    ...
    def get_console_log(cls, report_id: str, from_line: int = 0) -> Dict[str, Any]:
        ...
        Args:
            report_id: Report ID
            from_line: Starting line number (for incremental retrieval, 0 means from beginning)
    ...
    def _post_process_report(cls, content: str, outline: ReportOutline) -> str:
        ...
        while i < len(lines):
            ...
            if heading_match:
                ...
                # Check for duplicate headings (same title within consecutive 5 lines)
                is_duplicate = False

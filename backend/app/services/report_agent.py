...
logger = get_logger('arus.report_agent')
...
# Tool descriptions and prompt templates are now loaded via get_prompt().
# The constants below are kept as thin accessors for backward compatibility.

# Plan and section prompt constants are now sourced from the i18n layer.

# Section prompts are also sourced from the i18n layer.

# ── ReACT 循环内消息模板 ──

# ReACT loop message constants removed — now served by get_prompt().

# ── Chat prompt ──

# Chat prompts removed — now served by get_prompt().
...
class ReportAgent:
    ...
    def _define_tools(self) -> Dict[str, Dict[str, Any]]:
        ...
        return {
            "insight_forge": {
                "name": "insight_forge",
                "description": get_prompt('tool_desc_insight_forge'),
                "parameters": {
            ...
            "panorama_search": {
                ...
                "description": get_prompt('tool_desc_panorama_search'),
                "parameters": {
            ...
            "quick_search": {
                "name": "quick_search",
                "description": get_prompt('tool_desc_quick_search'),
                "parameters": {
            ...
            "interview_agents": {
                ...
                "description": get_prompt('tool_desc_interview_agents'),
                "parameters": {
                    "interview_topic": "Interview topic or requirement description (e.g. 'understand students\\'s views on the dormitory incident')",
    ...
    def plan_outline(
        ...
        system_prompt = get_prompt('plan_system_prompt')
    ...
    def _generate_section_react(
        ...
        for iteration in range(max_iterations):
            ...
            if has_final_answer:
                ...
                if tool_calls_count < min_tool_calls:
                    ...
                    unused_hint = get_prompt('react_unused_tools_hint', unused_list=", ".join(unused_tools)) if unused_tools else ""
                    messages.append({
                        "role": "user",
                        "content": get_prompt(
            ...
            if has_tool_calls:
                ...
                if unused_tools and tool_calls_count < self.MAX_TOOL_CALLS_PER_SECTION:
                    unused_hint = get_prompt('react_unused_tools_hint', unused_list=", ".join(unused_tools))
        ...
        messages.append({"role": "user", "content": get_prompt('react_force_final')})
    ...
    def chat(
        ...
        for iteration in range(max_iterations):
            ...
            messages.append({
                "role": "user",
                "content": observation + get_prompt('chat_observation_suffix')
            })

...
class ZepEntityReader:
    ...
    def get_node_edges(self, node_uuid: str) -> List[Dict[str, Any]]:
        """
        Get all related edges for a specific node (with retry)
    ...
    def filter_defined_entities(
        ...
        Filtering logic:
        - If a node's Labels only contain "Entity", it does not match our predefined types and is skipped
        - If a node's Labels contain labels beyond "Entity" and "Node", it matches a predefined type and is kept
        ...
        logger.info(f"Filtering complete: total nodes {total_count}, eligible {len(filtered_entities)}, "
    ...
    def get_entity_with_context(
        ...
        """
        Get a single entity with full context (edges and related nodes, with retry)

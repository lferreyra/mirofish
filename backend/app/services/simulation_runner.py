...
class SimulationRunner:
    ...
    def start_simulation(
        ...
        if not os.path.exists(config_path):
            raise ValueError(f"Simulation configuration does not exist, please call /prepare endpoint first")
        ...
        try:
            ...
            # This fixes issues where third-party libraries (e.g. OASIS) open files without specifying encoding
            env = os.environ.copy()
            env['PYTHONUTF8'] = '1'  # Python 3.7+ supported, makes all open() default to UTF-8
            ...
            # Use start_new_session=True to create a new process group, ensuring all child processes can be killed via os.killpg
    ...
    def _monitor_simulation(cls, simulation_id: str):
        ...
        try:
            ...
            # After process ends, do one final log read
            if os.path.exists(twitter_actions_log):
                        ...
                        try:
                            ...
                            if "event_type" in action_data:
                                ...
                                # Update round information (from round_end event)
                                    ...
                                    # Overall rounds take the maximum across both platforms
                                    if round_num > state.current_round:
                                        state.current_round = round_num
                                    # Overall time takes the maximum across both platforms
    ...
    def _check_all_platforms_completed(cls, state: SimulationRunState) -> bool:
        ...
        Determines whether a platform is enabled by checking whether its corresponding actions.jsonl file exists
        ...
        with open(file_path, 'r', encoding='utf-8') as f:
            for line in f:
                ...
                try:
                    ...
                    # Get platform: prefer the platform field in the record, otherwise use the default platform
    ...
    def get_all_actions(
        ...
        if not platform or platform == "twitter":
            actions.extend(cls._read_actions_from_file(
                twitter_actions_log,
                default_platform="twitter",  # Automatically fill in platform field
        ...
        if not platform or platform == "reddit":
            actions.extend(cls._read_actions_from_file(
                reddit_actions_log,
                default_platform="reddit",  # Automatically fill in platform field
                platform_filter=platform,
                agent_id=agent_id,
                round_num=round_num
            ))
    ...
    def cleanup_simulation_logs(cls, simulation_id: str) -> Dict[str, Any]:
        """
        Clean up simulation run logs (used to force a fresh restart of the simulation)
        ...
        # Clean up run state from memory
        if simulation_id in cls._run_states:
    ...
    def register_cleanup(cls):
        ...
        Called when the Flask application starts to ensure all simulation processes
    ...
    def check_env_alive(cls, simulation_id: str) -> bool:
        ...
        Returns:
            True if the environment is alive, False if the environment has closed
        """
        sim_dir = os.path.join(cls.RUN_STATE_DIR, simulation_id)
        if not os.path.exists(sim_dir):
            return False

...
Runs simulation in the background, records each Agent's actions, supports real-time status monitoring
"""
...
# Flag whether cleanup function has been registered
...
class SimulationRunner:
    ...
    def start_simulation(
        ...
        max_rounds: int = None,  # Max simulation rounds (optional, for truncating too-long simulations)
        ...
        Args:
            ...
            max_rounds: Max simulation rounds (optional, for truncating too-long simulations)
        ...
        if enable_graph_memory_update:
            ...
            try:
                GraphitiMemoryManager.create_updater(simulation_id, graph_id)
        ...
        try:
            ...
            # Set subprocess env vars to ensure UTF-8 encoding on Windows
            # This fixes encoding issues when third-party libraries (like OASIS) read files without specifying encoding
            env = os.environ.copy()
            env['PYTHONUTF8'] = '1'  # Python 3.7+, makes all open() default to UTF-8
            ...
            process = subprocess.Popen(
                ...
                start_new_session=True,  # Create new process group, ensure all related processes can be terminated on server shutdown
            )
    ...
    def _monitor_simulation(cls, simulation_id: str):
        ...
        try:
            ...
            if cls._graph_memory_enabled.get(simulation_id, False):
                try:
                    GraphitiMemoryManager.stop_updater(simulation_id)
    ...
    def _read_action_log(
        ...
        if graph_memory_enabled:
            graph_updater = GraphitiMemoryManager.get_updater(state.simulation_id)
                        ...
                        try:
                            ...
                            if "event_type" in action_data:
                                ...
                                if event_type == "simulation_end":
                                    ...
                                    # If two platforms running, both must complete
    ...
    def _check_all_platforms_completed(cls, state: SimulationRunState) -> bool:
        ...
        Determines if platform is enabled by checking if corresponding actions.jsonl file exists
    ...
    def cleanup_simulation_logs(cls, simulation_id: str) -> Dict[str, Any]:
        """
        Clean up simulation run logs (for forcing restart)
    ...
    def cleanup_all_simulations(cls):
        ...
        # Check if anything needs cleanup (avoid printing useless logs for empty processes)
        ...
        # First stop all graph memory updaters (stop_all prints its own logs)
        try:
    ...
    def register_cleanup(cls):
        ...
        # In Flask debug mode, only register cleanup in reloader subprocess (the process that actually runs the app)
        # WERKZEUG_RUN_MAIN=true indicates reloader subprocess
        # If not in debug mode, this env var doesn't exist, also need to register
        ...
        if is_debug_mode and not is_reloader_process:
            _cleanup_registered = True  # Mark as registered, prevent subprocess from trying again
            return
    ...
    def interview_agent(
        ...
        Args:
            ...
            platform: Specify platform (optional)
                ...
                - None: Interview both platforms in dual-platform mode, return integrated results
        ...
        if not ipc_client.check_env_alive():
            raise ValueError(f"Simulation environment not running or closed, cannot perform Interview: {simulation_id}")
    ...
    def interview_agents_batch(
        ...
        Args:
            ...
            platform: Default platform (optional, overridden by each interview item's platform)
                ...
                - None: Interview both platforms for each Agent in dual-platform mode
            timeout: Timeout in seconds

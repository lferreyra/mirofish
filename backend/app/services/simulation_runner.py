...
class SimulationRunner:
    ...
    # Thread lock for all shared dict access
    ...
    def get_run_state(cls, simulation_id: str) -> Optional[SimulationRunState]:
        ...
        # Try loading from file (outside lock to avoid I/O under lock)
    ...
    def start_simulation(
        ...
        max_rounds: int = None,  # Max simulation rounds (optional, truncate long simulations)
        ...
        if not os.path.exists(config_path):
            raise ValueError(f"Simulation config not found, please call /prepare endpoint first")
        ...
        try:
            ...
            # This fixes encoding issues in third-party libraries (like OASIS) that read files without specifying encoding
            env = os.environ.copy()
            ...
            # Save file handles, process ref, and monitor thread under lock
            with cls._lock:
    ...
    def _monitor_simulation(cls, simulation_id: str):
        ...
        try:
            ...
            # Clean up process resources and close file handles under lock
            with cls._lock:
    ...
    def _check_all_platforms_completed(cls, state: SimulationRunState) -> bool:
        ...
        Determines whether a platform is enabled by checking if the corresponding actions.jsonl file exists
    ...
    def _terminate_process(cls, process: subprocess.Popen, simulation_id: str, timeout: int = 10):
        ...
        if IS_WINDOWS:
            ...
            # /F = force kill, /T = kill process tree (including child processes)
    ...
    def stop_simulation(cls, simulation_id: str) -> SimulationRunState:
        ...
        if process and process.poll() is None:
            try:
                ...
                # Fall back to directly terminating process
                try:
                    process.terminate()
                    process.wait(timeout=5)
                except Exception:
        ...
        with open(file_path, 'r', encoding='utf-8') as f:
            for line in f:
                ...
                try:
                    ...
                    # Skip non-action records (events like simulation_start, round_start, round_end)
                    ...
                    # Get platform: prefer record's platform, otherwise use default
    ...
    def get_all_actions(
        ...
        """
        Get full action history from all platforms (no pagination limit)
    ...
    def cleanup_simulation_logs(cls, simulation_id: str) -> Dict[str, Any]:
        """
        Clean simulation run logs (for force-restarting a simulation)
        ...
        Note: Will not delete config files (simulation_config.json) or profile files
    ...
    def cleanup_all_simulations(cls):
        ...
        # Check if there is content to clean up (avoid useless logs for empty processes)
            ...
            try:
                if process.poll() is None:  # Process still running
                    ...
                    try:
                        ...
                        logger.info(f"Trying to update state.json: {state_file}")
    ...
    def register_cleanup(cls):
        ...
        # In Flask debug mode, only register cleanup in the reloader child process (the actual app process)
        # WERKZEUG_RUN_MAIN=true indicates the reloader child process
        # In non-debug mode, this env var does not exist, but cleanup still needs to be registered
    ...
    def check_env_alive(cls, simulation_id: str) -> bool:
        ...
        Returns:
            True if environment is alive, False if closed
        """
        sim_dir = os.path.join(cls.RUN_STATE_DIR, simulation_id)
        if not os.path.exists(sim_dir):
            return False
    ...
    def interview_agent(
        ...
        Args:
            ...
            platform: Specify platform (optional)
                ...
                - None: Interview both platforms simultaneously in dual-platform simulation, return integrated results
    ...
    def close_simulation_env(
        ...
        try:
            ...
            return {
                "success": True,
                "message": "Environment close command sent (response timeout, environment may be shutting down)"
            }

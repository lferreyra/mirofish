...
Runs simulations in the background, records every agent action, and supports real-time status monitoring.
"""
...
class SimulationRunState:
    ...
    # Per-platform round and time counters for parallel display
    ...
    # Completion flags (detected via simulation_end event in actions.jsonl)
    ...
    def to_dict(self) -> Dict[str, Any]:
        return {
            ...
            # Per-platform round and time counters
    ...
    def to_detail_dict(self) -> Dict[str, Any]:
        """Detailed dict including recent actions."""
        result = self.to_dict()
...
class SimulationRunner:
    ...
    Responsibilities:
    1. Run OASIS simulations in background subprocesses.
    2. Parse action logs and record every agent action.
    ...
    # simulation_id -> enabled
    _graph_memory_enabled: Dict[str, bool] = {}

    @classmethod
    def get_run_state(cls, simulation_id: str) -> Optional[SimulationRunState]:
        """Return the run state for a simulation, loading from disk if necessary."""
    ...
    def start_simulation(
        ...
        Args:
            ...
            enable_graph_memory_update: Whether to push agent activity to the Zep graph.
            graph_id: Zep graph ID (required when graph memory update is enabled).

        Returns:
            Initial SimulationRunState.
        """
        ...
        if max_rounds is not None and max_rounds > 0:
            ...
            if total_rounds < original_rounds:
                logger.info(f"Round count capped: {original_rounds} -> {total_rounds} (max_rounds={max_rounds})")
        ...
        # Determine which script to run (scripts live in backend/scripts/)
        if platform == "twitter":
        ...
        try:
            ...
            # Redirect stdout/stderr to a file to prevent pipe buffer overflow blocking the process
            main_log_path = os.path.join(sim_dir, "simulation.log")
            main_log_file = open(main_log_path, 'w', encoding='utf-8')

            # Force UTF-8 in the subprocess so third-party libs (e.g. OASIS) don't break on Windows
            env = os.environ.copy()
            env['PYTHONUTF8'] = '1'
            env['PYTHONIOENCODING'] = 'utf-8'

            # Use start_new_session=True so os.killpg can terminate all child processes
    ...
    def _monitor_simulation(cls, simulation_id: str):
        ...
        try:
            ...
            # Final log read after process exits
            if os.path.exists(twitter_actions_log):
    ...
    def _read_action_log(
        ...
        """
        Read new entries from an action log file starting at the given byte offset.

        Args:
            log_path: Path to the action log file.
            position: Last read byte position.
            state: Current run state object (mutated in place).
                        ...
                        try:
                            ...
                            if "event_type" in action_data:
                                ...
                                if event_type == "simulation_end":
                                    ...
                                    # Overall values are the max across platforms
    ...
    def _check_all_platforms_completed(cls, state: SimulationRunState) -> bool:
        """
        Check whether all enabled platforms have finished their simulation.
    ...
    def _terminate_process(cls, process: subprocess.Popen, simulation_id: str, timeout: int = 10):
        """
        Cross-platform process termination (including all child processes).

        Args:
            process: Process to terminate.
            simulation_id: Simulation ID (used for logging).
            timeout: Seconds to wait for graceful exit before forcing kill.
        """
        if IS_WINDOWS:
            # Windows: use taskkill to terminate the process tree
            # /F = force, /T = include children
            ...
            # Because start_new_session=True, pgid == main process pid
            ...
            try:
                ...
                logger.warning(f"Process group did not respond to SIGTERM, sending SIGKILL: {simulation_id}")
    ...
    def _read_actions_from_file(
        ...
        """
        Read agent actions from a single action log file.

        Args:
            file_path: Path to the action log file.
            default_platform: Fallback platform when the record has no platform field.
            platform_filter: If set, only return actions for this platform.
            agent_id: If set, only return actions for this agent.
            round_num: If set, only return actions from this round.
        """
    ...
    def get_all_actions(
        ...
        Returns:
            Full action list sorted by timestamp descending (newest first).
        """
        sim_dir = os.path.join(cls.RUN_STATE_DIR, simulation_id)
        actions = []
        ...
        # Fall back to legacy single-file format if per-platform files don't exist
        if not actions:
    ...
    def get_agent_stats(cls, simulation_id: str) -> List[Dict[str, Any]]:
        """
        Return per-agent action statistics.

        Returns:
            List of per-agent stat dicts sorted by total action count descending.
        """
        actions = cls.get_actions(simulation_id, limit=10000)
    ...
    def cleanup_simulation_logs(cls, simulation_id: str) -> Dict[str, Any]:
        ...
        Does NOT remove simulation_config.json or persona files.

        Args:
            simulation_id: Simulation ID.

        Returns:
            Dict with cleaned_files list and any errors.
        """
        import shutil
                ...
                if process.poll() is None:
                    ...
                    try:
                        ...
                        if os.path.exists(state_file):
                            ...
                            logger.info(f"state.json status set to stopped: {simulation_id}")
                        else:
    ...
    def register_cleanup(cls):
        ...
        # (the process that actually runs the app). WERKZEUG_RUN_MAIN=true identifies it.
        ...
        def cleanup_handler(signum=None, frame=None):
            """Signal handler: clean up simulation processes then invoke the original handler."""
    ...
    def check_env_alive(cls, simulation_id: str) -> bool:
        """Check whether the simulation environment is alive (able to accept Interview commands).
    ...
    def get_env_status_detail(cls, simulation_id: str) -> Dict[str, Any]:
        ...
        Returns:
            Dict with keys: status, twitter_available, reddit_available, timestamp.
        """
        sim_dir = os.path.join(cls.RUN_STATE_DIR, simulation_id)
    ...
    def interview_agent(
        ...
        Args:
            ...
            platform: Target platform (optional).
                ...
                - None: In a dual-platform simulation, interview on both platforms and
                        return a merged result.
            timeout: Response timeout in seconds.

        Returns:
            Dict containing interview result.
        ...
        if not ipc_client.check_env_alive():
            raise ValueError(
                f"Simulation environment is not running or has shut down; "
                f"cannot execute Interview: {simulation_id}"
            )
    ...
    def interview_agents_batch(
        ...
        """Interview multiple agents in a single batch request.

        Args:
            simulation_id: Simulation ID.
            interviews: List of interview specs, each containing
                {"agent_id": int, "prompt": str, "platform": str (optional)}.
            platform: Default platform (optional; overridden per-interview if specified).
        ...
        Returns:
            Dict containing batch interview results.
    ...
    def interview_all_agents(
        ...
        Args:
            simulation_id: Simulation ID.
            prompt: Interview question (identical for all agents).
            platform: Target platform (optional).
        ...
        Returns:
            Dict containing the global interview result.
        """
        sim_dir = os.path.join(cls.RUN_STATE_DIR, simulation_id)
        if not os.path.exists(sim_dir):
            raise ValueError(f"Simulation not found: {simulation_id}")
    ...
    def close_simulation_env(
        ...
        Sends a close-environment command so the simulation gracefully exits its
    ...
    def _get_interview_history_from_db(
        ...
        """Retrieve interview history from a single platform database."""
        import sqlite3
    ...
    def get_interview_history(
        ...
        """Retrieve interview history from the database.

        Args:
            simulation_id: Simulation ID.
            platform: Platform filter (reddit / twitter / None).
                - "reddit": Return Reddit history only.
                - "twitter": Return Twitter history only.

...
Implements a simple command/response pattern over the filesystem:
1. Flask writes a command file to the commands/ directory.
2. The simulation script polls the commands directory, executes the command,
   and writes the response to the responses/ directory.
3. Flask polls the responses directory to retrieve the result.
"""
...
class SimulationIPCClient:
    ...
    def send_interview(
        ...
        Args:
            ...
            platform: Target platform (optional).
                ...
                - None: In dual-platform mode, interview on both platforms;
                        in single-platform mode, interview on that platform.
            timeout: Timeout in seconds.
    ...
    def send_batch_interview(
        ...
        Args:
            interviews: List of interview items, each containing
                {"agent_id": int, "prompt": str, "platform": str (optional)}.
            platform: Default platform (optional; overridden per item if set).
                - "twitter": Default to Twitter only.
                - "reddit": Default to Reddit only.
                - None: In dual-platform mode, each agent is interviewed on both platforms.
            timeout: Timeout in seconds.
...
class SimulationIPCServer:
    ...
    def _update_env_status(self, status: str):
        """Write the current environment status to env_status.json."""
        status_file = os.path.join(self.simulation_dir, "env_status.json")
    ...
    def send_response(self, response: IPCResponse):
        ...
        Args:
            response: IPC response to write.
        """
        response_file = os.path.join(self.responses_dir, f"{response.command_id}.json")
        with open(response_file, 'w', encoding='utf-8') as f:
            json.dump(response.to_dict(), f, ensure_ascii=False, indent=2)

        # Remove the corresponding command file

"""
OASIS Agent ProfileGerar
ZepGrafoEntidadeOASISSimulaçãoAgent Profile
...
class OasisProfileGenerator:
    ...
    def generate_profile_from_entity(
        ...
        """
        ZepEntidadeGerarOASIS Agent Profile
    ...
    def _search_zep_for_entity(self, entity: EntityNode) -> Dict[str, Any]:
        ...
        if not self.graph_id:
            logger.debug(f"ZepBuscagraph_id")
            return results
            ...
            for attempt in range(max_retries):
                try:
                    ...
                    if attempt < max_retries - 1:
                        logger.debug(f"Zep {attempt + 1} Falhou: {str(e)[:80]}, ...")
                        time.sleep(delay)
                        delay *= 2
                    else:
                        logger.debug(f"Zep {max_retries} Falhou: {e}")
            return None
        ...
        try:
            ...
            if node_result and hasattr(node_result, 'nodes') and node_result.nodes:
                for node in node_result.nodes:
                    ...
                    if hasattr(node, 'name') and node.name and node.name != entity_name:
                        all_summaries.add(f"Entidade: {node.name}")
            ...
            if results["node_summaries"]:
                context_parts.append("Entidade:\n" + "\n".join(f"- {s}" for s in results["node_summaries"][:10]))
            results["context"] = "\n\n".join(context_parts)
            
            logger.info(f"ZepBusca: {entity_name},  {len(results['facts'])} , {len(results['node_summaries'])} ")
            
        except concurrent.futures.TimeoutError:
            logger.warning(f"ZepBusca ({entity_name})")
        except Exception as e:
            logger.warning(f"ZepBuscaFalhou ({entity_name}): {e}")
            ...
            for edge in entity.related_edges:  # 
                ...
                if fact:
                    ...
                    if direction == "outgoing":
                        relationships.append(f"- {entity.name} --[{edge_name}]--> (Entidade)")
                    else:
                        relationships.append(f"- (Entidade) --[{edge_name}]--> {entity.name}")
            
            if relationships:
                context_parts.append("### Relacionamento\n" + "\n".join(relationships))
        ...
        for attempt in range(max_attempts):
            try:
                ...
                try:
                    ...
                    logger.warning(f"JSONFalhou (attempt {attempt+1}): {str(je)[:80]}")
                ...
                logger.warning(f"LLMFalhou (attempt {attempt+1}): {str(e)[:80]}")
                last_error = e
                import time
                time.sleep(1 * (attempt + 1))  # 
        
        logger.warning(f"LLMGerarFalhou{max_attempts}: {last_error}, Gerar")
...
"""
    ...
    def generate_profiles_from_entities(
        ...
        """
        EntidadeGerarAgent ProfileGerar
        
        Args:
            entities: Entidade
            use_llm: LLMGerar
            progress_callback: Função callback de progresso (current, total, message)
            graph_id: ID do grafoZepBusca
            parallel_count: Gerar5
            realtime_output_path: Gerar
        ...
        def generate_single_profile(idx: int, entity: EntityNode) -> tuple:
            ...
            try:
                ...
                logger.error(f"GerarEntidade {entity.name} Falhou: {str(e)}")
                # profile
        ...
        logger.info(f"Gerar {total} Agent: {parallel_count}...")
        print(f"\n{'='*60}")
        print(f"GerarAgent -  {total} Entidade: {parallel_count}")
        print(f"{'='*60}\n")
            ...
            for future in concurrent.futures.as_completed(future_to_entity):
                ...
                try:
                    ...
                    if error:
                        ...
                        logger.info(f"[{current}/{total}] Gerar: {entity.name} ({entity_type})")
                        
                except Exception as e:
                    logger.error(f"Entidade {entity.name} : {str(e)}")
                    with lock:
                        completed_count[0] += 1
        ...
        print(f"\n{'='*60}")
        print(f"GerarGerar {len([p for p in profiles if p])} Agent")
        print(f"{'='*60}\n")
        
        return profiles
    
    def _print_generated_profile(self, entity_name: str, entity_type: str, profile: OasisAgentProfile):
        """GerarConteúdo completo, sem truncar"""
        separator = "-" * 70

...
Converts entities from knowledge graph into OASIS simulation platform Agent Profile format

Optimizations:
1. Use Graphiti retrieval to enrich node information
...
class OasisAgentProfile:
    ...
    def to_reddit_format(self) -> Dict[str, Any]:
        ...
        profile = {
            ...
            "username": self.user_name,  # OASIS requires field name "username" (no underscore)
            "name": self.name,
            "bio": self.bio,
...
class OasisProfileGenerator:
    ...
    Converts entities from knowledge graph into Agent Profiles for OASIS simulation
    
    Optimized features:
    1. Use Graphiti retrieval for richer context
    ...
    # Group/institutional entity types (need representative persona generation)
    GROUP_ENTITY_TYPES = [
    ...
    def __init__(
        ...
        # Graphiti-backed search for retrieval to enrich context
        self.graph_id = graph_id
        self._graphiti_tools = None
    ...
    def _search_graphiti_for_entity(self, entity: EntityNode) -> Dict[str, Any]:
        """
        Use Graphiti search to get rich entity-related information.
        ...
        try:
            ...
            if self._graphiti_tools is None:
                self._graphiti_tools = GraphitiToolsService()

            search_result = self._graphiti_tools.search_graph(
                graph_id=self.graph_id,
                query=f"All information about {entity_name}",
                limit=30,
                scope="edges",
            )

            results["facts"] = search_result.facts[:20]

            node_search = self._graphiti_tools.search_graph(
                graph_id=self.graph_id,
                query=entity_name,
                limit=20,
                scope="nodes",
            ...
            summaries = []
            for n in node_search.nodes:
                summary = n.get("summary", "")
                name = n.get("name", "")
                if summary:
                    summaries.append(summary)
                if name and name != entity_name:
                    summaries.append(f"Related entity: {name}")
            results["node_summaries"] = summaries[:10]
            ...
            logger.info(
                f"Graphiti search complete: {entity_name}, "
                f"found {len(results['facts'])} facts, {len(results['node_summaries'])} related nodes"
            )
        except Exception as e:
            logger.warning(f"Graphiti search failed ({entity_name}): {e}")
    ...
    def _build_entity_context(self, entity: EntityNode) -> str:
        ...
        3. Rich information from Graphiti retrieval
        """
        context_parts = []
        ...
        # 3. Add detailed info for related nodes
        ...
        # 4. Use Graphiti retrieval for richer info
        ...
        if graphiti_results.get("facts"):
            ...
            if new_facts:
                context_parts.append("### Facts Retrieved via Graphiti\n" + "\n".join(f"- {f}" for f in new_facts[:15]))
        
        if graphiti_results.get("node_summaries"):
            context_parts.append("### Related Nodes Retrieved via Graphiti\n" + "\n".join(f"- {s}" for s in graphiti_results["node_summaries"][:10]))
    ...
    def _get_system_prompt(self, is_individual: bool) -> str:
        """Get system prompt"""
        base_prompt = "You are a social media user profile generation expert. Generate detailed, realistic personas for opinion simulation, maximizing fidelity to existing real-world situations. Must return valid JSON format. All string values must not contain unescaped newlines. Use English for all persona content."
        return base_prompt
    ...
    def _build_individual_persona_prompt(
        ...
        return f"""Generate detailed social media user persona for this entity, maximizing fidelity to existing real-world situations.
...
2. persona: Detailed persona description (2000-char plain text), must include:
   ...
   - Stance and viewpoints (attitude toward topics, content that may provoke/move them)
   - Unique traits (catchphrases, special experiences, personal hobbies)
   - Personal memory (important part of persona, introduce this individual's connection to events and their existing actions and reactions)
3. age: Age number (must be integer)
4. gender: Gender, must be English: "male" or "female"
5. mbti: MBTI type (e.g., INTJ, ENFP)
6. country: Country (in English, e.g., "United States", "Iran")
...
- Use English for all content (including persona, bio, profession, topics)
...
"""
    ...
    def _build_group_persona_prompt(
        ...
        """Build detailed persona prompt for group/institutional entity"""
        
        attrs_str = json.dumps(entity_attributes, ensure_ascii=False) if entity_attributes else "None"
        context_str = context[:3000] if context else "No additional context"
        
        return f"""Generate detailed social media account settings for an institutional/group entity, maximizing fidelity to existing real-world situations.
...
1. bio: Official account bio, 200 chars, professional and appropriate
2. persona: Detailed account settings description (2000-char plain text), must include:
   - Basic org info (formal name, nature, founding background, main functions)
   ...
   - Institutional memory (important part of org persona, introduce this org's connection to events and existing actions and reactions)
...
Important:
- All field values must be strings or numbers, null not allowed
...
- Institutional account speech must match its identity positioning"""
    ...
    def set_graph_id(self, graph_id: str):
        """Set graph ID for Graphiti retrieval"""
        self.graph_id = graph_id
    ...
    def generate_profiles_from_entities(
        ...
        """
        Batch generate Agent Profiles from entities (with parallel support)
        
        Args:
            entities: Entity list
            use_llm: Whether to use LLM for detailed persona generation
            progress_callback: Progress callback function (current, total, message)
            graph_id: Graph ID for Graphiti retrieval to get richer context
        ...
        # Set graph_id for Graphiti retrieval
        if graph_id:
            self.graph_id = graph_id
    ...
    def _save_twitter_csv(self, profiles: List[OasisAgentProfile], file_path: str):
        """
        Save Twitter Profile as CSV format (per OASIS official requirements)

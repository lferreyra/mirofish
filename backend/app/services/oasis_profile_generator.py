...
Converts entities from Zep graph into Agent Profile format required by the OASIS simulation platform.
...
class OasisProfileGenerator:
    ...
    2. Generates very detailed personas (including basic info, career, personality traits, social media behavior, etc.)
    3. Distinguishes between individual entities and abstract group entities
    """
    ...
    def _search_zep_for_entity(self, entity: EntityNode) -> Dict[str, Any]:
        """
        Use Zep graph hybrid search to retrieve rich information about the entity.

        Zep does not have a built-in hybrid search interface, so edges and nodes
        are searched separately and results are merged. Parallel requests are used
        for efficiency.
        ...
        def _search_with_retry(scope: str, limit: int):
            """Search Zep graph with retry mechanism"""
            max_retries = 3
            delay = 2.0
            ...
            for attempt in range(max_retries):
                try:
                    ...
                    if attempt < max_retries - 1:
                        logger.debug(f"Zep {scope} search attempt {attempt + 1} failed: {str(e)[:80]}, retrying...")
                        time.sleep(delay)
                        delay *= 2
                    else:
                        logger.debug(f"Zep {scope} search failed after {max_retries} attempts: {e}")
            return None

        try:
            # Execute edges and nodes search in parallel
            with concurrent.futures.ThreadPoolExecutor(max_workers=2) as executor:
                edge_future = executor.submit(_search_with_retry, "edges", 30)
                node_future = executor.submit(_search_with_retry, "nodes", 20)
    ...
    def _build_entity_context(self, entity: EntityNode) -> str:
        ...
        2. Detailed info about related nodes
        ...
        # 4. Use Zep hybrid retrieval for richer information
    ...
    def _generate_profile_with_llm(
        ...
        logger.warning(f"LLM persona generation failed ({max_attempts} attempts): {last_error}, falling back to rule-based")
    ...
    def _get_system_prompt(self, is_individual: bool) -> str:
        ...
        base_prompt = "You are a social media user profile generation expert. Generate detailed, realistic personas for opinion simulation, maximally faithful to existing real-world information. You must return valid JSON format. All string values must not contain unescaped newline characters. Use English."
        return base_prompt
    ...
    def _build_individual_persona_prompt(
        ...
        return f"""Generate a detailed social media user persona for the entity, maximally faithful to existing real-world information.
...
2. persona: Detailed persona description (2000 words of plain text), must include:
   ...
   - Background (important experiences, connections to events, social relationships)
   ...
   - Stances and opinions (attitudes toward topics, content that may anger/move them)
   - Unique characteristics (catchphrases, special experiences, personal hobbies)
   - Personal memories (important part of the persona, describe the individual's connection to events, and their existing actions and reactions in those events)
...
"""
    ...
    def _build_group_persona_prompt(
        ...
        return f"""Generate a detailed social media account profile for an institutional/group entity, maximally faithful to existing real-world information.
...
2. persona: Detailed account profile description (2000 words of plain text), must include:
   ...
   - Stances and attitudes (official position on core topics, how controversies are handled)
   - Special notes (representative group profile, operational habits)
   - Institutional memories (important part of the persona, describe the institution's connection to events, and the institution's existing actions and reactions in those events)
...
- age must be integer 30, gender must be the string "other"
- Institutional account communication must match its identity and positioning"""

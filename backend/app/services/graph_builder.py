...
class GraphBuilderService:
    """
    Graph construction service.
    Calls the Zep API to build a knowledge graph.
    """
    ...
    def build_graph_async(
        ...
        Args:
            ...
            batch_size: Number of chunks per batch sent to Zep.
    ...
    def _build_graph_worker(
        ...
        """Background worker thread for graph construction."""
        try:
    ...
    def create_graph(self, name: str) -> str:
        """Create a Zep graph and return its ID."""
    ...
    def set_ontology(self, graph_id: str, ontology: Dict[str, Any]):
        """Apply an ontology to a Zep graph."""
        import warnings
        ...
        # This is the pattern required by the Zep SDK; warnings from dynamic class
        # creation can be safely ignored.
        warnings.filterwarnings('ignore', category=UserWarning, module='pydantic')

        # Names reserved by Zep that cannot be used as attribute names
        RESERVED_NAMES = {'uuid', 'name', 'group_id', 'name_embedding', 'summary', 'created_at'}

        def safe_attr_name(attr_name: str) -> str:
            """Prefix reserved names to avoid Zep API conflicts."""
            if attr_name.lower() in RESERVED_NAMES:
                return f"entity_{attr_name}"
            return attr_name

        # Dynamically create entity type classes
        entity_types = {}
        for entity_def in ontology.get("entity_types", []):
            ...
            for attr_def in entity_def.get("attributes", []):
                ...
                # Zep API requires a Field description
        ...
        # Dynamically create edge type classes
        edge_definitions = {}
        ...
        # Apply the ontology via the Zep API
    ...
    def add_text_batches(
        ...
        """Send text chunks to the graph in batches; return all episode UUIDs."""
        episode_uuids = []
        total_chunks = len(chunks)
    ...
    def _wait_for_episodes(
        ...
        """Wait for all episodes to finish processing (polls each episode's processed flag)."""
        ...
        while pending_episodes:
            if time.time() - start_time > timeout:
                if progress_callback:
                    progress_callback(
                        f"Timed out — {completed_count}/{total_episodes} completed",
            ...
            if progress_callback:
                progress_callback(
                    f"Zep processing... {completed_count}/{total_episodes} done, "
    ...
    def _get_graph_info(self, graph_id: str) -> GraphInfo:
        """Fetch summary metadata for a graph."""
        # Fetch all nodes (paginated)
        nodes = fetch_all_nodes(self.client, graph_id)

        # Fetch all edges (paginated)
        edges = fetch_all_edges(self.client, graph_id)

        # Collect distinct entity types
        entity_types = set()
        for node in nodes:
            if node.labels:
    ...
    def get_graph_data(self, graph_id: str) -> Dict[str, Any]:
        """
        Fetch full graph data including detailed node and edge information.

        Args:
            graph_id: Graph ID.

        Returns:
            Dict with nodes and edges including timestamps, attributes, and other details.
        """
        nodes = fetch_all_nodes(self.client, graph_id)
        edges = fetch_all_edges(self.client, graph_id)

        # Build a UUID-to-name lookup for resolving edge endpoints
        node_map = {}
        for node in nodes:

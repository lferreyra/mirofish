...
- We need to simulate reactions and information propagation paths of all parties in public opinion events
...
```json
{
    ...
    "analysis_summary": "Brief analysis summary of text content"
}
```
...
A. **Fallback types (must include, placed as last 2 in list)**:
   - `Person`: Fallback type for any individual. When a person does not fit other more specific person types, classify here.
   - `Organization`: Fallback type for any organization. When an organization does not fit other more specific types, classify here.
...
- Various people appear in text, such as "school teacher", "passerby", "some netizen"
...
**Individual (fallback)**:
- Person: Any individual (used when not fitting specific types above)
...
class OntologyGenerator:
    ...
    # Maximum text length for LLM (50k characters)
    ...
    def _build_user_message(
        ...
        # If text exceeds 50k chars, truncate (only affects LLM input, not graph building)

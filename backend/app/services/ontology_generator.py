...
A. **Fallback types (must be included, placed as the last 2 in the list)**:
   - `Person`: Fallback type for any individual person. When a person does not belong to any more specific person type, they are classified here.
...
- Various people appear in the text, such as "elementary school teachers", "random passersby", "some netizen"
...
- Person: Any individual (used when not matching the above specific types)
...
- Organization: Any organization (used when not matching the above specific types)

...
# 本体生成的系统提示词
ONTOLOGY_SYSTEM_PROMPT = """You are a professional knowledge graph ontology design expert. Your task is to analyze the provided text content and simulation requirements, and design entity types and relationship types suitable for **social media opinion simulation**.
...
A. **Fallback types (required, place at the last 2 positions)**:
   - `Person`: Fallback type for any natural person. When an individual does not belong to a more specific person type, classify them here.
...
- Various individuals appear in the text, such as "school teachers", "passersby", "a certain netizen"
- If there is no specific type match, they should be classified as `Person`
- Similarly, small organizations, ad-hoc groups, etc. should be classified as `Organization`
...
- Ensure that source_targets cover the entity types you defined
...
- Person: Any natural person (use when not fitting specific types above)
...
- Organization: Any organization (use when not fitting specific types above)
...
"""
        ...
        message += """
Based on the above content, please design entity types and relationship types suitable for social media opinion simulation.
...
4. All entity types must be real-world agents that can speak and interact — no abstract concepts
5. Attribute names must not use reserved words such as name, uuid, group_id, etc. Use full_name, org_name, etc. instead
"""

...
Wraps graph search, node reads, edge queries, and related tools for use by the Report Agent.

Core retrieval tools (optimized):
1. InsightForge (deep-insight retrieval) — the most powerful hybrid retrieval; auto-generates
   sub-queries and retrieves across multiple dimensions.
2. PanoramaSearch (breadth search) — fetches the full picture, including expired content.
...
class InsightForgeResult:
    ...
    Contains retrieval results across multiple sub-queries and dimensions.
    """
    query: str
...
class PanoramaResult:
    ...
    def to_text(self) -> str:
        ...
        if self.active_facts:
            text_parts.append(f"\n### [Active facts] (verbatim simulation results)")
    ...
    def to_text(self) -> str:
        ...
        if self.key_quotes:
            ...
            for quote in self.key_quotes:
                ...
                if skip:
                    continue
                # Trim at a sentence boundary rather than hard-cutting
...
class InterviewResult:
    ...
    selection_reasoning: str = ""  # Rationale for agent selection
    summary: str = ""              # Consolidated interview summary
...
class ZepToolsService:
    ...
    1. insight_forge      — deep-insight retrieval (most powerful; auto sub-queries, multi-dimensional)
    2. panorama_search    — breadth search (full picture, including expired content)
    3. quick_search       — lightweight fast retrieval
    4. interview_agents   — deep interview (interview simulated agents, multi-perspective views)
    ...
    - get_node_edges      — fetch edges connected to a node
    - get_entities_by_type — fetch entities by type label
    ...
    def _call_with_retry(self, func, operation_name: str, max_retries: int = None):
        """Execute an API call with automatic retries."""
    ...
    def search_graph(
        ...
        """
        Semantic search over a graph using hybrid search (semantic + BM25).
        ...
        Returns:
            SearchResult with matching facts, edges, and nodes.
        """
    ...
    def _local_search(
        ...
        """
        Local keyword-matching search — fallback when the Zep Search API is unavailable.
        ...
        Returns:
            SearchResult with matched facts, edges, and nodes.
        """
    ...
    def get_all_nodes(self, graph_id: str) -> List[NodeInfo]:
        """
        Fetch all nodes in a graph (paginated).
    ...
    def get_all_edges(self, graph_id: str, include_temporal: bool = True) -> List[EdgeInfo]:
        """
        Fetch all edges in a graph (paginated), including temporal metadata.
        ...
        Returns:
            List of EdgeInfo objects with created_at, valid_at, invalid_at, expired_at.
        """
    ...
    def get_node_edges(self, graph_id: str, node_uuid: str) -> List[EdgeInfo]:
        ...
        Retrieves all graph edges and filters to those involving the specified node.
        ...
        Returns:
            List of EdgeInfo objects connected to the node.
        """
    ...
    def get_entity_summary(
        ...
        """
        Fetch a relationship summary for the specified entity.
        ...
        Returns:
            Dictionary with entity info, related facts, edges, and relation count.
        """
    ...
    def get_graph_statistics(self, graph_id: str) -> Dict[str, Any]:
        """
        Fetch aggregate statistics for a graph.
    ...
    def get_simulation_context(
        ...
        """
        Fetch all context information relevant to a simulation requirement.
        ...
        Returns:
            Dictionary with related facts, graph statistics, and entity list.
        """
        ...
        # Keep only nodes that have a meaningful type label
        entities = []
        for node in all_nodes:
    ...
    def insight_forge(
        ...
        """
        InsightForge — deep-insight hybrid retrieval.

        The most powerful retrieval method. Automatically decomposes the query and
        retrieves across multiple dimensions:
        1. Uses an LLM to split the query into sub-questions.
        2. Runs semantic search for each sub-question.
        3. Extracts relevant entities and fetches their details.
        4. Traces relationship chains.
        5. Consolidates all results into a rich InsightForgeResult.

        Args:
            graph_id: Graph ID.
            query: User question or topic.
            simulation_requirement: Simulation requirement description.
            report_context: Optional report context for more precise sub-query generation.
            max_sub_queries: Maximum number of sub-queries to generate.

        Returns:
            InsightForgeResult with facts, entities, and relationship chains.
        """
        ...
        # Step 3: Extract entity UUIDs from edges and fetch only those entities
        entity_uuids = set()
        ...
        # Step 4: Build relationship chains from all edges
        relationship_chains = []
        for edge_data in all_edges:
            if isinstance(edge_data, dict):
                source_uuid = edge_data.get('source_node_uuid', '')
...
Return a JSON list of sub-questions."""
    ...
    def panorama_search(
        ...
        """
        PanoramaSearch — breadth search for a full panoramic view.

        Retrieves all related content including historical/expired information:
        1. Fetches all relevant nodes.
        2. Fetches all edges (including expired/invalidated ones).
        3. Classifies results into active and historical facts.

        Suitable for understanding the full picture of an event or tracing its evolution.
        ...
        Returns:
            PanoramaResult with active facts, historical facts, and entities.
        """
    ...
    def quick_search(
        ...
        Directly calls Zep semantic search and returns the most relevant results.
        Suitable for simple, direct information lookups.
        ...
        Returns:
            SearchResult with the top matching facts.
        """
    ...
    def interview_agents(
        ...
        """
        InterviewAgents — conduct real interviews with simulated agents via the OASIS API.

        Workflow:
        1. Reads persona files to discover all simulated agents.
        2. Uses an LLM to select the most relevant agents for the interview topic.
        3. Generates interview questions via LLM (or uses custom_questions if provided).
        4. Calls the /api/simulation/interview/batch endpoint on both platforms simultaneously.
        5. Consolidates all responses into an InterviewResult.
        ...
        Args:
            ...
            interview_requirement: Unstructured description of the interview goal
                (e.g. "understand how students feel about the event").
            simulation_requirement: Optional simulation background context.
            max_agents: Maximum number of agents to interview.
            custom_questions: Optional list of questions; auto-generated if not provided.

        Returns:
            InterviewResult with per-agent responses, key quotes, and a summary.
        """
        ...
        try:
            ...
            if not api_result.get("success", False):
                ...
                result.summary = f"Interview API call failed: {error_msg}. Check OASIS environment status."
                return result
            ...
            result.summary = (
                ...
                "The simulation environment may have shut down. Ensure OASIS is running."
            )
            return result
        except Exception as e:
    ...
    def _clean_tool_call_response(response: str) -> str:
        """Strip JSON tool-call wrappers from an agent response and return the actual content."""
    ...
    def _select_agents_for_interview(
        ...
        """
        Use an LLM to select the most relevant agents for the interview.

        Returns:
            tuple: (selected_agents, selected_indices, reasoning)
                - selected_agents: Full profile dicts for selected agents.
                - selected_indices: Integer indices of selected agents (used for API calls).
...
Select up to {max_agents} agents most suitable for this interview and explain your reasoning."""
        ...
        try:
            ...
            return selected, indices, "Default selection strategy used"
...
Generate 3-5 interview questions."""
    ...
    def _generate_interview_summary(
        ...
        """Generate a summary of the interview results."""
        if not interviews:

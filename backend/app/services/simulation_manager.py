...
Uses preset scripts combined with LLM-generated configuration parameters.
"""
...
class SimulationManager:
    ...
    3. Use an LLM to intelligently generate simulation configuration parameters.
    ...
    def _load_simulation_state(self, simulation_id: str) -> Optional[SimulationState]:
        """Load simulation state from disk (with in-memory cache)."""
    ...
    def prepare_simulation(
        ...
        Steps:
        1. Read and filter entities from the Zep graph.
        2. Generate an OASIS Agent Profile for each entity
           (optional LLM enrichment, supports parallel execution).
        3. Use an LLM to generate simulation config parameters
           (timing, activity levels, posting frequency, etc.).
        4. Save configuration and profile files.
        5. Copy preset scripts to the simulation directory.

        Args:
            simulation_id: Simulation ID.
            simulation_requirement: Natural-language description of the simulation
                goal (used by the LLM to generate config).
            document_text: Raw document content (provides background context for the LLM).
            defined_entity_types: Predefined entity types to filter by (optional).
            use_llm_for_profiles: Whether to use the LLM for detailed persona generation.
        ...
        try:
            ...
            profiles = generator.generate_profiles_from_entities(
                ...
                graph_id=state.graph_id,          # used for Zep retrieval
            ...
            # Reddit profiles were already written incrementally; this ensures completeness.
            if progress_callback:
                progress_callback(
            ...
            if state.enable_twitter:
                # Twitter requires CSV format (OASIS requirement)
                generator.save_profiles(
                    profiles=profiles,
            ...
            # Note: run scripts remain in backend/scripts/; they are not copied to the
            # simulation directory. simulation_runner launches them from scripts/ directly.
    ...
    def get_simulation(self, simulation_id: str) -> Optional[SimulationState]:
        """Return the state for a simulation."""
    ...
    def get_simulation_config(self, simulation_id: str) -> Optional[Dict[str, Any]]:
        """Return the configuration for a simulation."""
        sim_dir = self._get_simulation_dir(simulation_id)
        config_path = os.path.join(sim_dir, "simulation_config.json")
    ...
    def get_run_instructions(self, simulation_id: str) -> Dict[str, str]:
        """Return run instructions for a simulation."""
        sim_dir = self._get_simulation_dir(simulation_id)
        config_path = os.path.join(sim_dir, "simulation_config.json")
        ...
        return {
            ...
            "instructions": (
                ...
                f"2. Run the simulation (scripts located in {scripts_dir}):\n"

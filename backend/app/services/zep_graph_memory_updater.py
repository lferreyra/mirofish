...
class ZepGraphMemoryUpdater:
    ...
    def __init__(self, graph_id: str, api_key: Optional[str] = None):
        ...
        Args:
            ...
            api_key: Zep API KeyConfiguração
        """
        self.graph_id = graph_id
        self.api_key = api_key or Config.ZEP_API_KEY
        
        if not self.api_key:
            raise ValueError("ZEP_API_KEYConfiguração")
    ...
    def _send_batch_activities(self, activities: List[AgentActivity], platform: str):
        ...
        for attempt in range(self.MAX_RETRIES):
            try:
                ...
                logger.info(f" {len(activities)} {display_name}Grafo {self.graph_id}")
                logger.debug(f"Conteúdo: {combined_text[:200]}...")
                return
                
            except Exception as e:
                if attempt < self.MAX_RETRIES - 1:
                    logger.warning(f"ZepFalhou ( {attempt + 1}/{self.MAX_RETRIES}): {e}")
                    time.sleep(self.RETRY_DELAY * (attempt + 1))
                else:
                    logger.error(f"ZepFalhou{self.MAX_RETRIES}: {e}")
    ...
    def get_stats(self) -> Dict[str, Any]:
        ...
        return {
            ...
            "failed_count": self._failed_count,          # Falhou
            ...
            if cls._updaters:
                for simulation_id, updater in list(cls._updaters.items()):
                    try:
                        ...
                        logger.error(f"Falhou: simulation_id={simulation_id}, error={e}")
                cls._updaters.clear()
            logger.info("Grafo")

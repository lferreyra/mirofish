...
Dynamically updates agent activities from simulation into the Graphiti knowledge graph
"""
...
class AgentActivity:
    ...
    def to_episode_text(self) -> str:
        """Convert activity to a text description for Graphiti"""
        action_descriptions = {
...
class GraphitiMemoryUpdater:
    """
    Graphiti graph memory updater

    Monitors simulation action log files and updates new agent activities
    to the Graphiti knowledge graph in real time.
    """
    ...
    def __init__(self, graph_id: str):
        ...
        logger.info(f"GraphitiMemoryUpdater initialized: graph_id={graph_id}, batch_size={self.BATCH_SIZE}")
    ...
    def start(self):
        ...
        self._worker_thread = threading.Thread(
            ...
            daemon=True,
            name=f"GraphitiMemUpdater-{self.graph_id[:8]}",
        )
        self._worker_thread.start()
        logger.info(f"GraphitiMemoryUpdater started: graph_id={self.graph_id}")
    ...
    def stop(self):
        ...
        logger.info(
            f"GraphitiMemoryUpdater stopped: graph_id={self.graph_id}, "
    ...
    def _send_batch_activities(self, activities: List[AgentActivity], platform: str):
        ...
        combined_text = "\n".join(episode_texts)
        round_num = activities[0].round_num if activities else 0

        for attempt in range(self.MAX_RETRIES):
            try:
                _run_async(self._add_episode(combined_text, platform, round_num))
                self._total_sent += 1
                self._total_items_sent += len(activities)
                display_name = self._get_platform_display_name(platform)
                logger.info(
                    f"Successfully batch-sent {len(activities)} {display_name} activities to graph {self.graph_id}"
                )
                return
            except Exception as e:
                if attempt < self.MAX_RETRIES - 1:
                    logger.warning(f"Batch send failed (attempt {attempt + 1}): {e}")
    ...
    async def _add_episode(self, text: str, platform: str, round_num: int):
        ...
        await graphiti.add_episode(
            name=f"sim_{round_num}_{platform}",
            episode_body=text,
            source=EpisodeType.text,
            source_description=f"MiroFish {platform} simulation round {round_num}",
...
class GraphitiMemoryManager:
    """Manages Graphiti graph memory updaters for multiple simulations"""

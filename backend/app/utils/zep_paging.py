...
The Zep node/edge list API uses UUID-cursor-based pagination. This module
wraps the auto-pagination logic (including per-page retries) and returns
the full list transparently to the caller.
"""
...
def _fetch_page_with_retry(
    ...
    **kwargs: Any,
    ...
    """Fetch a single page, retrying transient network/IO errors with exponential backoff."""
    if max_retries < 1:
...
def fetch_all_nodes(
    ...
    """Fetch all graph nodes with pagination, up to max_items (default 2000). Each page request includes retry logic."""
    all_nodes: list[Any] = []

"""
SimulaçãoAPI
Step2: ZepEntidadeOASISSimulação
"""
...
# Interview prompt
# AgentFerramenta
INTERVIEW_PROMPT_PREFIX = "Ferramenta"


def optimize_interview_prompt(prompt: str) -> str:
    """
    InterviewAgentFerramenta
...
def get_graph_entities(graph_id: str):
    ...
    EntidadeLabelsEntity
    
    Query
        entity_types: Entidade
        enrich: true
    """
    try:
        ...
        logger.info(f"GrafoEntidade: graph_id={graph_id}, entity_types={entity_types}, enrich={enrich}")
        ...
        logger.error(f"GrafoEntidadeFalhou: {str(e)}")
        return jsonify({
            "success": False,
            "error": str(e),
...
def get_entity_detail(graph_id: str, entity_uuid: str):
    ...
    try:
        ...
        logger.error(f"EntidadeFalhou: {str(e)}")
        return jsonify({
            "success": False,
            "error": str(e),
            "traceback": traceback.format_exc()
...
def create_simulation():
    ...
    try:
        ...
        logger.error(f"SimulaçãoFalhou: {str(e)}")
        return jsonify({
            "success": False,
            "error": str(e),
            "traceback": traceback.format_exc()
...
def _check_simulation_prepared(simulation_id: str) -> tuple:
    ...
    (run_*.py) backend/scripts/ Simulação
    ...
    try:
        ...
        logger.debug(f"Simulação: {simulation_id}, status={status}, config_generated={config_generated}")
        
        # config_generated=True
        # Concluído
        # - ready:
        # - preparing:  config_generated=True Concluído
        # - running:
        # - completed:
        # - stopped:
        # - failed: Falhou
        ...
        if status in prepared_statuses and config_generated:
            ...
            if status == "preparing":
                try:
                    ...
                    logger.info(f"Simulação: {simulation_id} preparing -> ready")
                    status = "ready"
                except Exception as e:
                    logger.warning(f"Falhou: {e}")
            
            logger.info(f"Simulação {simulation_id} Resultado:  (status={status}, config_generated={config_generated})")
            return True, {
                "status": status,
            ...
            logger.warning(f"Simulação {simulation_id} Resultado:  (status={status}, config_generated={config_generated})")
            return False, {
        ...
        return False, {"reason": f"Falhou: {str(e)}"}
...
def prepare_simulation():
    ...
    - ConcluídoGerar
    - Resultado
    - Gerarforce_regenerate=true
    
    1. 
    2. ZepGrafoEntidade
    3. EntidadeGerarOASIS Agent Profile
    4. LLMGerarConfiguração da simulação
    5. Configuração
    
    JSON
        {
            "simulation_id": "sim_xxxx",                   // Obrigatório, ID da simulação
            "entity_types": ["Student", "PublicFigure"],  // Entidade
            "use_llm_for_profiles": true,                 // LLMGerar
            "parallel_profile_count": 5,                  // Gerar5
            "force_regenerate": false                     // Gerarfalse
        }
    ...
    try:
        ...
        if not force_regenerate:
            logger.debug(f"Simulação {simulation_id} ...")
            is_prepared, prepare_info = _check_simulation_prepared(simulation_id)
            logger.debug(f"Resultado: is_prepared={is_prepared}, prepare_info={prepare_info}")
            if is_prepared:
                logger.info(f"Simulação {simulation_id} Gerar")
                return jsonify({
                    "success": True,
                    "data": {
                ...
                logger.info(f"Simulação {simulation_id} ")
        ...
        # prepareAgent
        try:
            logger.info(f"Entidade: graph_id={state.graph_id}")
            reader = ZepEntityReader()
            # Entidade
            ...
            logger.info(f"Entidade: {filtered_preview.filtered_count}, : {filtered_preview.entity_types}")
        except Exception as e:
            logger.warning(f"EntidadeFalhou: {e}")
            # Falhou
        ...
        def run_prepare():
            ...
            try:
                ...
                # SimulaçãoCallback de progresso
                stage_details = {}
        ...
        return jsonify({
            ...
            "data": {
                ...
                "entity_types": state.entity_types  # Entidade
            }
        })
    ...
    JSON
        {
            ...
            "simulation_id": "sim_xxxx"      // ID da simulaçãoConcluído
        }
...
def list_simulations():
    """
    Simulaçãoenriquecido com runner_status, current_round, total_rounds e report_id)
...
def _get_report_id_for_simulation(simulation_id: str) -> str:
    ...
    try:
        ...
        logger.warning(f" simulation {simulation_id}  report Falhou: {e}")
        return None
...
def get_simulation_history():
    ...
    try:
        ...
        for sim in simulations:
            ...
            # Configuração da simulação simulation_config.json  simulation_requirement
            config = manager.get_simulation_config(sim.simulation_id)
            if config:
                sim_dict["simulation_requirement"] = config.get("simulation_requirement", "")
...
def get_simulation_profiles(simulation_id: str):
    ...
    try:
        ...
        logger.error(f"ProfileFalhou: {str(e)}")
        return jsonify({
            "success": False,
            "error": str(e),
            "traceback": traceback.format_exc()
    ...
    try:
        ...
        if file_exists:
            ...
            try:
                ...
                logger.warning(f" profiles Falhou: {e}")
                profiles = []
...
def get_simulation_config_realtime(simulation_id: str):
    """
    Configuração da simulaçãoGerar
    ...
    - ConfiguraçãoGerar
    
        {
            ...
            "data": {
                ...
                "file_modified_at": "2025-12-04T18:20:00",
                "is_generating": true,  // Gerar
                "generation_stage": "generating_config",  // Gerar
                "config": {...}  // ConfiguraçãoConteúdo
            }
        }
    """
    import json
    ...
    try:
        ...
        if file_exists:
            ...
            try:
                ...
                logger.warning(f" config Falhou: {e}")
                config = None
        ...
        logger.error(f"ConfigFalhou: {str(e)}")
        return jsonify({
            "success": False,
            "error": str(e),
            "traceback": traceback.format_exc()
...
def get_simulation_config(simulation_id: str):
    """
    Configuração da simulaçãoLLMGerarConfiguração
    
        - time_config: ConfiguraçãoSimulação/
        - agent_configs: AgentConfiguração
        - event_config: Configuração
        - platform_configs: Configuração
        - generation_reasoning: LLMConfiguração
    """
    try:
        manager = SimulationManager()
        ...
        logger.error(f"ConfiguraçãoFalhou: {str(e)}")
        return jsonify({
            "success": False,
            "error": str(e),
            "traceback": traceback.format_exc()
...
def generate_profiles():
    """
    GrafoGerarOASIS Agent ProfileSimulação
    ...
    try:
        ...
        logger.error(f"GerarProfileFalhou: {str(e)}")
        return jsonify({
            "success": False,
            "error": str(e),
            "traceback": traceback.format_exc()
...
def start_simulation():
    ...
    JSON
        {
            ...
            "max_rounds": 100,                     // : SimulaçãoSimulação
            "enable_graph_memory_update": false,   // : AgentZepGrafo
            "force": false                         // : Simulação
        }

     force 
        - SimulaçãoConcluído
        - Conteúdorun_state.json, actions.jsonl, simulation.log 
        - Configuraçãosimulation_config.json profile 
        - Simulação

     enable_graph_memory_update
        - SimulaçãoAgentZepGrafo
        - Grafo""SimulaçãoAnáliseAI
        - Simulação graph_id
        - API

        {
            ...
            "data": {
                ...
                "graph_memory_update_enabled": true,  // Grafo
    ...
    try:
        ...
        max_rounds = data.get('max_rounds')  # Simulação
        enable_graph_memory_update = data.get('enable_graph_memory_update', False)  # Grafo
        ...
        if state.status != SimulationStatus.READY:
            ...
            if is_prepared:
                    ...
                    if run_state and run_state.runner_status.value == "running":
                        if force:
                            ...
                            try:
                                ...
                                logger.warning(f"Simulação: {str(e)}")
                        else:
                            return jsonify({
                                "success": False,
                ...
                # ready
                logger.info(f"Simulação {simulation_id} Concluído ready: {state.status.value}")
        ...
        if enable_graph_memory_update:
            ...
            logger.info(f"Grafo: simulation_id={simulation_id}, graph_id={graph_id}")
...
def get_agent_stats(simulation_id: str):
    ...
    try:
        ...
        logger.error(f"AgentFalhou: {str(e)}")
        return jsonify({
            "success": False,
            "error": str(e),
            "traceback": traceback.format_exc()
...
def interview_agent():
    ...
    try:
        ...
        logger.error(f"InterviewFalhou: {str(e)}")
        return jsonify({
            "success": False,
            "error": str(e),
            "traceback": traceback.format_exc()

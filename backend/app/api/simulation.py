...
# Adding this prefix prevents Agent from calling tools and forces text-only replies
...
def _check_simulation_prepared(simulation_id: str) -> tuple:
    ...
    try:
        ...
        if status in prepared_statuses and config_generated:
            ...
            # If status is preparing but files are complete, auto-update to ready
            if status == "preparing":
                try:
...
def prepare_simulation():
    ...
    Features:
    - Auto-detects completed preparations, avoiding duplicate generation
    ...
    try:
            ...
            if is_prepared:
                ...
                return jsonify({
                    ...
                    "data": {
                        ...
                        "status": "ready",
                        "message": "Existing preparation found, no duplicate generation needed",
        ...
        # This allows frontend to immediately get the expected Agent count after calling prepare
        try:
            ...
            filtered_preview = reader.filter_defined_entities(
                ...
                enrich_with_edges=False  # No edge info needed, faster
            )
...
def get_prepare_status():
    ...
    2. Check for completed preparation by simulation_id
    ...
    Returns:
        {
            ...
            "data": {
                ...
                "prepare_info": {...}            // detailed info when already prepared
            }
        }
    """
    ...
    try:
            ...
            if simulation_id:
                ...
                return jsonify({
                    ...
                    "data": {
                        ...
                        "status": "not_started",
                        "progress": 0,
                        "message": "Not started yet, call /api/simulation/prepare to begin",
                        "already_prepared": False
                    }
                })
            return jsonify({
        ...
        if not task:
            # Task not found, but if simulation_id given, check if already prepared
            if simulation_id:
                is_prepared, prepare_info = _check_simulation_prepared(simulation_id)
                if is_prepared:
                    return jsonify({
...
def _get_report_id_for_simulation(simulation_id: str) -> str:
    ...
    Iterate through reports directory to find reports matching simulation_id,
...
def get_simulation_history():
    ...
    For homepage history display, returns simulation list with project name, description, and other details
            ...
            "data": [
                {
                    ...
                    "simulation_requirement": "If university publishes...",
                    "status": "completed",
                    "entities_count": 68,
                    "profiles_count": 68,
    ...
    try:
        ...
        for sim in simulations:
            ...
            if run_state:
                ...
                # Use user-set total_rounds, or recommended rounds if not set
...
def get_simulation_profiles_realtime(simulation_id: str):
    ...
    - Returns extra metadata (file modification time, whether generation is in progress, etc.)
...
def get_simulation_config(simulation_id: str):
    ...
    Returns:
        ...
        - agent_configs: Activity config per Agent (activity level, posting frequency, stance, etc.)
        - event_config: Event config (initial posts, hot topics)
        - platform_configs: Platform configuration
        - generation_reasoning: LLM config generation reasoning
    """
    try:
        manager = SimulationManager()
...
def download_simulation_script(script_name: str):
    """
    Download simulation run script files (shared scripts in backend/scripts/)
...
def start_simulation():
    ...
    Request (JSON):
        {
            ...
            "max_rounds": 100,                     // optional: max simulation rounds, used to truncate long simulations
    ...
    About the force parameter:
        - When enabled, stops running simulation and cleans run logs if running or completed
...
def get_run_status_detail(simulation_id: str):
    ...
    For frontend real-time dynamics display
    ...
    try:
        ...
        # recent_actions shows only the latest round content from both platforms
...
def get_simulation_timeline(simulation_id: str):
    ...
    For frontend progress bar and timeline view display
...
def get_agent_stats(simulation_id: str):
    ...
    For frontend Agent activity rankings, action distribution, etc.
    """
    try:
...
def interview_agent():
    ...
    Note: This feature requires the simulation environment to be running (entered command-wait mode after completing simulation loop)
...
def interview_agents_batch():
    ...
    Request (JSON):
        {
            ...
            "interviews": [                    // required, interview list
                ...
                {
                    "agent_id": 1,
                    "prompt": "What do you think about B?"  // uses default if platform not specified
                }
            ],
    ...
    try:
        ...
        # Optimize prompt for each interview item, add prefix to prevent Agent tool calls
    ...
    Request (JSON):
        {
            ...
            "platform": "reddit",          // optional, platform type (reddit/twitter)
                                           // returns all history from both platforms if not specified
...
def close_simulation_env():
    ...
    Note: This differs from the /stop endpoint, which forcefully terminates the process.
    This endpoint allows the simulation to gracefully close the environment and exit.

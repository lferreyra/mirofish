...
def create_simulation():
    ...
    Note: Parameters like max_rounds are intelligently generated by LLM, no manual setting required
...
def _check_simulation_prepared(simulation_id: str) -> tuple:
    ...
    Note: Run scripts (run_*.py) remain in the backend/scripts/ directory and are not copied to the simulation directory
    ...
    try:
        ...
        # - running: currently running, meaning preparation was completed earlier
        # - completed: run complete, meaning preparation was completed earlier
        # - stopped: stopped, meaning preparation was completed earlier
...
def prepare_simulation():
    ...
    try:
            ...
            if is_prepared:
                ...
                return jsonify({
                    ...
                    "data": {
                        ...
                        "status": "ready",
                        "message": "Preparation already complete, no need for redundant generation",
        ...
        # ========== Synchronously get entity count (before launching background task) ==========
        ...
        try:
            ...
            logger.warning(f"Synchronous entity count retrieval failed (will retry in background task): {e}")
            # Failure doesn't affect the subsequent flow; background task will re-fetch
...
def _get_report_id_for_simulation(simulation_id: str) -> str:
    ...
    If multiple exist, returns the most recent (sorted by created_at).
...
def get_simulation_history():
    ...
    try:
        ...
        for sim in simulations:
            ...
            # Get run state (read actual round count set by user from run_state.json)
...
def download_simulation_script(script_name: str):
    """
    Download simulation run script file (shared scripts in backend/scripts/)
...
def start_simulation():
    ...
    About the force parameter:
        - When enabled, if simulation is running or completed, it will be stopped first and run logs cleaned
        - Cleaned content includes: run_state.json, actions.jsonl, simulation.log, etc.
        - Config files (simulation_config.json) and profile files are NOT cleaned
        - Suitable for scenarios requiring simulation re-run

    About enable_graph_memory_update:
        - When enabled, all Agent activities (posts, comments, likes, etc.) during simulation are updated to Zep graph in real-time
    ...
    Response:
        {
            ...
            "data": {
                ...
                "force_restarted": true               // Whether force restarted
            }
        }
    """
    try:
        ...
        if state.status != SimulationStatus.READY:
            ...
            if is_prepared:
                # Preparation is complete, check if there's a running process
...
def get_run_status_detail(simulation_id: str):
    ...
    try:
        ...
        # recent_actions only shows content from the latest round across both platforms
...
def get_interview_history():
    ...
    Request (JSON):
        {
            ...
            "platform": "reddit",          // Optional, platform type (reddit/twitter)
                                           // When not specified, returns history for both platforms
    ...
    try:
        ...
        simulation_id = data.get('simulation_id')
        platform = data.get('platform')  # When not specified, returns history for both platforms
        agent_id = data.get('agent_id')
...
def close_simulation_env():
    ...
    Send a close-environment command to the simulation for graceful exit from waiting-for-command mode.

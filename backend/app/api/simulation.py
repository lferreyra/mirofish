...
def create_simulation():
    ...
    Note: Parameters such as max_rounds are intelligently generated by LLM; no manual setup is needed.
    ...
    try:
        ...
        if not graph_id:
            return jsonify({
                "success": False,
                "error": "Project has no graph yet. Please call /api/graph/build first"
            }), 400
...
def _check_simulation_prepared(simulation_id: str) -> tuple:
    ...
    # Required files list (scripts are excluded; they reside in backend/scripts/)
    required_files = [
        "state.json",
    ...
    try:
        ...
        # The following statuses all indicate preparation has been completed:
    ...
    try:
        ...
        try:
            ...
            filtered_preview = reader.filter_defined_entities(
                ...
                enrich_with_edges=False  # Skip edge info for faster response
            )
...
def get_prepare_status():
    ...
    try:
        ...
        # If simulation_id is provided, check if preparation is already complete first
        if simulation_id:
            is_prepared, prepare_info = _check_simulation_prepared(simulation_id)
            if is_prepared:
                return jsonify({
                    ...
                    "data": {
                        ...
                        "status": "ready",
                        "progress": 100,
                        "message": "Preparation work is already complete",
                        "already_prepared": True,
                        "prepare_info": prepare_info
                    }
                })
        ...
        if not task:
            # Task does not exist, but if simulation_id is present, check if preparation is complete
            if simulation_id:
                is_prepared, prepare_info = _check_simulation_prepared(simulation_id)
                if is_prepared:
                    return jsonify({
...
def _get_report_id_for_simulation(simulation_id: str) -> str:
    ...
    If multiple exist, returns the newest one (sorted by created_at).
    ...
    try:
        ...
        # Sort by creation time descending, return the newest
...
def get_simulation_history():
    ...
    Used for homepage historical project display, returns a simulation list with rich
    information including project name, description, etc.
...
def get_simulation_config(simulation_id: str):
    ...
    try:
        ...
        if not config:
            return jsonify({
                "success": False,
                "error": "Simulation config does not exist. Please call the /prepare endpoint first"
            }), 404
...
def start_simulation():
    ...
    About the force parameter:
        - When enabled, if the simulation is running or completed, it will first stop and clear run logs
        ...
        - Suitable for scenarios where re-running the simulation is needed

    About enable_graph_memory_update:
        - When enabled, all Agent activity (posting, commenting, liking, etc.) is updated to the Zep graph in real-time
        - This allows the graph to "remember" the simulation for subsequent analysis or AI conversations
    ...
    try:
        ...
        if state.status != SimulationStatus.READY:
            # Check if preparation work is already complete
            is_prepared, prepare_info = _check_simulation_prepared(simulation_id)
...
def get_run_status_detail(simulation_id: str):
    ...
    try:
        ...
        # recent_actions shows only the latest round's content from both platforms
...
def interview_agent():
    ...
    Note: This feature requires the simulation environment to be running (in command-waiting mode after completing the simulation loop)
    ...
    Response (without platform specified, dual-platform mode):
        {
            "success": true,
            "data": {
                "agent_id": 0,
...
def close_simulation_env():
    ...
    Sends a close command to the simulation for a graceful exit from command-waiting mode.

    Note: This differs from /stop. /stop forcefully terminates the process,
    while this endpoint lets the simulation gracefully close and exit.

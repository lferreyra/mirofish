...
def optimize_interview_prompt(prompt: str) -> str:
    """
    Optimize Interview questions by adding a prefix to prevent the Agent from calling tools.
    ...
    Request (JSON):
        {
            ...
            "graph_id": "mirofish_xxxx",    // optional; if not provided, fetched from the project
...
def _check_simulation_prepared(simulation_id: str) -> tuple:
    ...
    # List of required files (scripts excluded; scripts are located in backend/scripts/)
    required_files = [
        "state.json",
    ...
    try:
        ...
        # The following statuses all indicate that preparation has already finished:
...
def prepare_simulation():
    ...
    Steps:
    1. Check whether preparation work already exists
    2. Read and filter entities from the Zep graph
    3. Generate an OASIS Agent Profile for each entity (with retry logic)
    4. LLM intelligently generates simulation configuration (with retry logic)
...
def get_prepare_status():
    ...
    2. Check whether preparation work already exists via simulation_id
    ...
    Returns:
        {
            ...
            "data": {
                ...
                "progress": 45,
                "message": "...",
                "already_prepared": true|false,  // whether preparation work already exists
    ...
    try:
        ...
        if not task_id:
            ...
            return jsonify({
                "success": False,
                "error": "Please provide a task_id or simulation_id"
            }), 400
        ...
        if not task:
            # Task does not exist; if simulation_id is provided, check whether preparation is already complete
            if simulation_id:
                is_prepared, prepare_info = _check_simulation_prepared(simulation_id)
                if is_prepared:
                    return jsonify({
...
def _get_report_id_for_simulation(simulation_id: str) -> str:
    ...
    Iterates through the reports directory to find reports that match the simulation_id.
    If multiple matches exist, returns the most recent one (sorted by created_at).
...
def get_simulation_history():
    ...
    Used for the home-page history display. Returns a simulation list enriched with
    project names, descriptions, and other details.
            ...
            "data": [
                {
                    "simulation_id": "sim_xxxx",
                    "project_id": "proj_xxxx",
                    "project_name": "Wuda Sentiment Analysis",
    ...
    try:
        ...
        for sim in simulations:
            ...
            # Get run state (read the user-configured total_rounds from run_state.json)
            run_state = SimulationRunner.get_run_state(sim.simulation_id)
            if run_state:
                sim_dict["current_round"] = run_state.current_round
                sim_dict["runner_status"] = run_state.runner_status.value
                # Use the user-configured total_rounds; fall back to recommended count if not set
            ...
            # Get the file list for the associated project (up to 3 files)
...
def get_simulation_profiles_realtime(simulation_id: str):
    """
    Get Agent Profiles for the simulation in real time (for viewing progress during generation).

    Differences from the /profiles endpoint:
    - Reads files directly without going through SimulationManager
    - Suitable for real-time inspection during generation
    ...
    try:
        ...
        if file_exists:
            ...
            try:
                ...
                logger.warning(f"Failed to read profiles file (may be in the middle of being written): {e}")
                profiles = []
...
def get_simulation_config_realtime(simulation_id: str):
    ...
    - Suitable for real-time inspection during generation
    - Returns extra metadata (e.g. file modification time, whether generation is in progress)
    - Can return partial information even if config generation is not yet complete
    ...
    try:
        ...
        if file_exists:
            ...
            try:
                ...
                logger.warning(f"Failed to read config file (may be in the middle of being written): {e}")
                config = None
...
def get_simulation_config(simulation_id: str):
    """
    Get simulation configuration (the complete config intelligently generated by the LLM).
    ...
    Returns:
        ...
        - generation_reasoning: LLM explanation of how the config was generated
    """
    try:
        manager = SimulationManager()
...
def download_simulation_script(script_name: str):
    """
    Download simulation run script files (generic scripts located in backend/scripts/).
...
def generate_profiles():
    ...
    try:
        ...
        if not graph_id:
            return jsonify({
                "success": False,
                "error": "Please provide a graph_id"
            }), 400
...
def start_simulation():
    ...
    Request (JSON):
        {
            ...
            "force": false                         // Optional: force restart (will stop a running simulation and clean up logs)
        }
    ...
    About the force parameter:
        ...
        - Config files (simulation_config.json) and profile files will NOT be cleaned
        - Suitable for scenarios where the simulation needs to be re-run

    About enable_graph_memory_update:
        - When enabled, all Agent activities (posting, commenting, liking, etc.) during the simulation are updated to the Zep graph in real time
    ...
    try:
        ...
        # Smart state handling: if preparation is complete, allow restart
        if state.status != SimulationStatus.READY:
                ...
                if state.status == SimulationStatus.RUNNING:
                    ...
                    if run_state and run_state.runner_status.value == "running":
                        ...
                        if force:
                            ...
                            return jsonify({
                                "success": False,
                                "error": f"Simulation is running. Please call /stop first, or use force=true to force a restart"
                            }), 400
...
def get_run_status_detail(simulation_id: str):
    ...
    For displaying real-time activity in the frontend
    ...
    try:
        ...
        # recent_actions shows both platforms' content for the current latest round only
...
def get_simulation_timeline(simulation_id: str):
    ...
    For displaying progress bars and timeline views in the frontend
...
def get_agent_stats(simulation_id: str):
    ...
    For displaying Agent activity rankings and action distribution in the frontend
    """
    try:
...
def interview_agent():
    ...
    (after completing the simulation loop and entering the command-waiting mode).
    ...
    Request (JSON):
        {
            ...
            "platform": "twitter",             // optional, specify platform (twitter/reddit)
                                               // if not specified: both platforms are interviewed simultaneously
    ...
    try:
        ...
        if agent_id is None:
            return jsonify({
                "success": False,
                "error": "Please provide an agent_id"
            }), 400
        ...
        if platform and platform not in ("twitter", "reddit"):
            return jsonify({
                "success": False,
                "error": "The platform parameter must be either 'twitter' or 'reddit'"
            }), 400
        ...
        if not SimulationRunner.check_env_alive(simulation_id):
            return jsonify({
                "success": False,
                "error": "The simulation environment is not running or has been closed. Please ensure the simulation has completed and entered command-waiting mode."
            }), 400
    ...
    Request (JSON):
        {
            ...
            "interviews": [                    // required, list of interviews
                ...
                {
                    "agent_id": 1,
                    "prompt": "What do you think about B?"  // if platform not specified, the default is used
                }
            ],
            "platform": "reddit",              // optional, default platform (overridden by each item's platform)
                                               // if not specified: both platforms are interviewed for each Agent
    ...
    try:
        ...
        for i, interview in enumerate(interviews):
            ...
            if item_platform and item_platform not in ("twitter", "reddit"):
                return jsonify({
                    "success": False,
                    "error": f"The platform for interview list item {i+1} must be either 'twitter' or 'reddit'"
                }), 400
...
def close_simulation_env():
    ...
    Sends a close-environment command to the simulation, causing it to gracefully exit wait-for-command mode.

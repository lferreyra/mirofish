"""
Report API routes
Provides interfaces for simulation report generation, retrieval, and chat
"""
...
def generate_report():
    ...
    try:
        ...
        # Generate report_id in advance so it can be returned to frontend immediately
        import uuid
...
def get_generate_status():
    ...
    try:
        ...
        # If simulation_id provided, first check if report already exists
        if simulation_id:
            existing_report = ReportManager.get_report_by_simulation(simulation_id)
            if existing_report and existing_report.status == ReportStatus.COMPLETED:
                return jsonify({
...
def get_report_sections(report_id: str):
    ...
    Frontend can poll this interface to get generated section content without waiting for entire report
...
def get_agent_log(report_id: str):
    ...
    Real-time acquisition of every step during report generation:
    - Report start, planning start/complete
    - Each section start, tool call, LLM response, complete
    - Report complete or failed

    Query parameters:
        from_line: Start from which line (optional, default 0, for incremental fetch)
...
def get_console_log(report_id: str):
    """
    Get console output log of Report Agent

    Real-time acquisition of console output during report generation (INFO, WARNING, etc.),
    This is different from the structured JSON log returned by agent-log interface,
    it's pure text format console-style log.

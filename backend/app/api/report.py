...
def chat_with_report_agent():
    ...
    Request (JSON):
        {
            "simulation_id": "sim_xxxx",            // required, simulation ID
            "message": "Please explain the opinion trends",  // required, user message
...
def get_agent_log(report_id: str):
    ...
    Retrieves every action taken during report generation in real time, including:
    - Report start, planning start/complete
    - Each section's start, tool calls, LLM response, completion
    - Report completion or failure

    Query parameters:
        from_line: Line number to start reading from (optional, default 0, for incremental fetch)

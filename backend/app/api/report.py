    ...
    Request (JSON):
        {
            ...
            "force_regenerate": false       // optional: regenerate even if a report exists
        }
...
def get_generate_status():
    ...
    Request (JSON):
        {
            "task_id": "task_xxxx",      // optional — from the generate endpoint
...
def get_report_by_simulation(simulation_id: str):
    """
    Return the report associated with a simulation.
...
def chat_with_report_agent():
    ...
    The agent autonomously calls retrieval tools to answer questions about the simulation.
...
def get_report_progress(report_id: str):
    """
    Return real-time report generation progress.
...
def get_report_sections(report_id: str):
    """
    Return the list of already-generated report sections.

    Clients can poll this endpoint to display sections as they complete,
    without waiting for the full report to finish.
...
def get_single_section(report_id: str, section_index: int):
    """
    Return the content of a single report section.
...
def check_report_status(simulation_id: str):
    """
    Check whether a simulation has a report and what its status is.
...
def get_agent_log(report_id: str):
    ...
    Provides real-time step-by-step agent actions during report generation, including:
    - Report start, planning start/complete
    - Per-section: start, tool calls, LLM responses, completion
    - Report complete or failed

    Query params:
        from_line: Start reading from this line (default 0; use for incremental polling).
...
def stream_agent_log(report_id: str):
    """
    Return the complete Agent log in a single request.
    ...
    try:
        ...
        logger.error(f"Failed to fetch agent log stream: {str(e)}")
        return jsonify({
            "success": False,
            "error": str(e),
            "traceback": traceback.format_exc()
...
def get_console_log(report_id: str):
    """
    Return the console output log from the Report Agent.

    This is plain-text console-style output (INFO, WARNING, etc.),
    distinct from the structured JSON logs returned by agent-log.
...
def stream_console_log(report_id: str):
    """
    Return the complete console log in a single request.
    ...
    try:
        ...
        logger.error(f"Failed to fetch console log stream: {str(e)}")
        return jsonify({
            "success": False,
            "error": str(e),
            "traceback": traceback.format_exc()

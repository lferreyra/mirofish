...
Uses project context mechanism with server-side persisted state
"""
...
# ============== Endpoint 1.5: Research Mode Ontology Generation ==============
...
def generate_ontology_from_research():
    ...
    try:
        ...
        if not topic:
            return jsonify({"success": False, "error": "Please provide a research topic (topic)"}), 400
        if not simulation_requirement:
            return jsonify({"success": False, "error": "Please provide a simulation requirement (simulation_requirement)"}), 400

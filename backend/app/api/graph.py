...
Uses project context with server-side persistent state.
"""
...
def reset_project(project_id: str):
    ...
    # Reset to ontology-generated if ontology exists
    if project.ontology:
...
def generate_ontology():
    """
    Endpoint 1: upload files and generate an ontology definition.

    Request: multipart/form-data

    Fields:
        files: uploaded files (PDF/MD/TXT), multiple allowed
        simulation_requirement: simulation requirement text (required)
    ...
    try:
        ...
        logger.info(f"Text extraction done, {len(all_text)} characters")
...
def build_graph():
    """
    Endpoint 2: build graph for a project_id.
    ...
    try:
        ...
        if project.status == ProjectStatus.GRAPH_BUILDING and not force:
            return jsonify({
                "success": False,
                "error": "Graph is already building; do not submit again. To force rebuild, set force: true",

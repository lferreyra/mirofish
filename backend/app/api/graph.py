...
def delete_project(project_id: str):
    ...
    if not success:
        return jsonify({
            "success": False,
            "error": f"Project not found or could not be deleted: {project_id}"
        }), 404
...
def build_graph():
    ...
    try:
        ...
        if project.status == ProjectStatus.CREATED:
            return jsonify({
                "success": False,
                "error": "The project does not have an ontology yet. Call /ontology/generate first."
            }), 400
        
        if project.status == ProjectStatus.GRAPH_BUILDING and not force:
            return jsonify({
                "success": False,
                "error": "A graph build is already in progress. To force a rebuild, set force: true.",
        ...
        def build_task():
            ...
            try:
                ...
                task_manager.update_task(
                    ...
                    message="Initializing the graph build service..."
                )
                ...
                task_manager.update_task(
                    task_id,
                    message="Applying the ontology definition...",
                    progress=15
                )

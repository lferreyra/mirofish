<template>
  ...
  >
    <!-- Background decoration: tech grid (shown only with projects) -->
    ...
    <!-- Card container (shown only with projects) -->
        ...
        <div v-if="selectedProject" class="modal-overlay" @click.self="closeModal">
          <div class="modal-content">
            ...
            <div class="modal-playback-hint">
              <span class="hint-text">Step3 "Start Simulation" and Step5 "Deep Interaction" must be started while running and cannot be replayed</span>
            </div>
          </div>
        </div>
      </Transition>
...
const containerStyle = computed(() => {
  ...
  // Expanded: dynamically calculate height by card count
  ...
  // Calculate actual height: rows * card height + (rows-1) * gap + padding
...
const getCardStyle = (index) => {
  ...
  if (isExpanded.value) {
    ...
    // Calculate cards in current row, center each row
    ...
    // Expand downward with title spacing
    ...
    const x = offset * 35
    // Adjust start position, near title with proper spacing
  ...
  observer = new IntersectionObserver(
    (entries) => {
      entries.forEach((entry) => {
        ...
        // If animating, just record state, handle after animation
        if (isAnimating) return
        
        // If target state equals current state, no action needed
        ...
        // Short delay for expand (50ms), longer for collapse (200ms) for stability
        ...
        expandDebounceTimer = setTimeout(() => {
          ...
          // Check if pending state still needs execution (may be overridden by subsequent scroll)
          ...
          setTimeout(() => {
            ...
            if (pendingState !== null && pendingState !== isExpanded.value) {
              // Delay briefly to avoid switching too fast
              expandDebounceTimer = setTimeout(() => {
                if (pendingState !== null && pendingState !== isExpanded.value) {
                  isAnimating = true
                  isExpanded.value = pendingState

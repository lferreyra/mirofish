        ...
        <div v-if="selectedProject" class="modal-overlay" @click.self="closeModal">
          <div class="modal-content">
            ...
            <div class="modal-playback-hint">
              <span class="hint-text">Step3 "Start Simulation" and Step5 "Deep Interaction" must be started while running and do not support history playback.</span>
            </div>
          </div>
        </div>
      </Transition>
...
let pendingState = null  // Tracks the pending target state

// Card layout config - adjusted to a wider aspect ratio
...
const containerStyle = computed(() => {
  ...
  // Calculate actual required height: rows * card height + (rows-1) * gap + small bottom padding
...
const getCardStyle = (index) => {
  ...
  if (isExpanded.value) {
    ...
    // Calculate card count in current row to center each row
    ...
    // Expand downward with extra spacing from title
    ...
    const x = offset * 35
    // Adjust start position: close to title but with appropriate spacing
...
// Get CSS class based on round progress
...
// Navigate to knowledge graph build page (Project)
const goToProject = () => {
  ...
  observer = new IntersectionObserver(
    (entries) => {
      entries.forEach((entry) => {
        ...
        // Debounced state switch to prevent rapid flickering.
        // Short delay when expanding (50ms), longer when collapsing (200ms) for stability.
        ...
        expandDebounceTimer = setTimeout(() => {
          ...
          // Check if pending state still needs to be applied (may have been overridden)
          ...
          // Release lock after animation ends, then check for any pending state change
          setTimeout(() => {
            isAnimating = false

            // After animation ends, check if there is a new pending state
            if (pendingState !== null && pendingState !== isExpanded.value) {
              // Delay briefly to avoid switching too quickly
              expandDebounceTimer = setTimeout(() => {
                if (pendingState !== null && pendingState !== isExpanded.value) {
                  isAnimating = true
                  isExpanded.value = pendingState
    ...
    },
    {
      ...
      // Adjust rootMargin: shrink bottom of viewport upward, requiring more scrolling to trigger expansion
...
onMounted(async () => {
  ...
  // Initialize observer after DOM is rendered
  setTimeout(() => {
    initObserver()
  }, 100)
})
...
.grid-pattern {
  ...
  /* Anchor from top-left so height changes only expand at the bottom, not affecting existing grid positions */
...
.cards-container {
  ...
  /* min-height is dynamically calculated by JS based on card count */
}
...
.status-icon:nth-child(1).available { color: #3B82F6; } /* Knowledge Graph - blue */

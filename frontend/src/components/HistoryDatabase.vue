...
const getCardStyle = (index) => {
  ...
  if (isExpanded.value) {
    ...
    const x = offset * 35
    // Adjust start position, close to title with proper spacing
  ...
  observer = new IntersectionObserver(
    (entries) => {
      entries.forEach((entry) => {
        ...
        // If target state is same as current state, no processing needed
        ...
        // Shorter delay (50ms) for expand, longer (200ms) for collapse for stability
        ...
        expandDebounceTimer = setTimeout(() => {
          ...
          setTimeout(() => {
            ...
            // After animation ends, check for new pending states
            if (pendingState !== null && pendingState !== isExpanded.value) {
              // Delay a short time before executing to avoid switching too fast
              expandDebounceTimer = setTimeout(() => {
                if (pendingState !== null && pendingState !== isExpanded.value) {
                  isAnimating = true
                  isExpanded.value = pendingState

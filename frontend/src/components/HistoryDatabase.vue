        ...
        <div v-if="selectedProject" class="modal-overlay" @click.self="closeModal">
          <div class="modal-content">
            ...
            <div class="modal-playback-hint">
              <span class="hint-text">Step3 "Start Simulation" and Step5 "Deep Interaction" must be started during runtime and do not support history replay</span>
            </div>
          </div>
        </div>
      </Transition>
...
const getCardStyle = (index) => {
  ...
  if (isExpanded.value) {
    ...
    // Calculate cards per row to center each row
  ...
  observer = new IntersectionObserver(
    (entries) => {
      entries.forEach((entry) => {
        ...
        // Update pending target state (record latest target regardless of animation)
        ...
        // Use debounce to delay state transitions, preventing rapid flickering
        ...
        expandDebounceTimer = setTimeout(() => {
          ...
          setTimeout(() => {
            ...
            if (pendingState !== null && pendingState !== isExpanded.value) {
              // Brief delay before executing to avoid rapid switching
              expandDebounceTimer = setTimeout(() => {
                if (pendingState !== null && pendingState !== isExpanded.value) {
                  isAnimating = true
                  isExpanded.value = pendingState
...
/* Create fixed-spacing square grid with CSS background pattern */
.grid-pattern {
  position: absolute;
  top: 0;
  left: 0;
  ...
  /* Position from top-left, height changes only extend bottom, preserving existing grid position */

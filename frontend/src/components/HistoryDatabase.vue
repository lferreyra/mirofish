        ...
        <div v-if="selectedProject" class="modal-overlay" @click.self="closeModal">
          <div class="modal-content">
            ...
            <div class="modal-playback-hint">
              <span class="hint-text">Step 3 "Simulation" and Step 5 "Interaction" must be started while running and do not support historical replay</span>
            </div>
          </div>
        </div>
      </Transition>
...
const getCardStyle = (index) => {
  ...
  if (isExpanded.value) {
    ...
    // Calculate number of cards in current row, ensure each row is centered
        ...
        expandDebounceTimer = setTimeout(() => {
          ...
          setTimeout(() => {
            ...
            if (pendingState !== null && pendingState !== isExpanded.value) {
              // Brief delay before executing to avoid too-rapid switching
              expandDebounceTimer = setTimeout(() => {
                if (pendingState !== null && pendingState !== isExpanded.value) {
                  isAnimating = true
                  isExpanded.value = pendingState
...
/* CSS background pattern creating fixed-spacing square grid */
.grid-pattern {
  position: absolute;
  top: 0;
  left: 0;

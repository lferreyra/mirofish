        ...
        <div v-if="selectedProject" class="modal-overlay" @click.self="closeModal">
          <div class="modal-content">
            ...
            <div class="modal-playback-hint">
              <span class="hint-text">Step 3 "Run Simulation" and Step 5 "Deep Interaction" must be started during runtime and don't support history playback</span>
            </div>
          </div>
        </div>
      </Transition>
  ...
  observer = new IntersectionObserver(
    (entries) => {
      entries.forEach((entry) => {
        ...
        // If animating, just record state, handle after animation ends
        ...
        expandDebounceTimer = setTimeout(() => {
          ...
          setTimeout(() => {
            ...
            if (pendingState !== null && pendingState !== isExpanded.value) {
              // Brief delay before executing to avoid too-rapid transitions
              expandDebounceTimer = setTimeout(() => {
                if (pendingState !== null && pendingState !== isExpanded.value) {
                  isAnimating = true
                  isExpanded.value = pendingState
    ...
    },
    {
      ...
      // Adjust rootMargin, shrink viewport bottom upward, require more scrolling to trigger expand
...
/* Create fixed-spacing square grid using CSS background pattern */
.grid-pattern {
  position: absolute;
  top: 0;
  left: 0;

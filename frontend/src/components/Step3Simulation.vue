...
// Track each platform's previous round to detect changes and output logs
...
const checkPlatformsCompleted = (data) => {
  // Return false if no platform data
  ...
  // Determine if a platform is enabled via actions_count (count > 0 or running was true)
...
const fetchRunStatusDetail = async () => {
  ...
  try {
    ...
    if (res.success && res.data) {
      // Get the full action list via all_actions
    ...
    console.warn('Failed to fetch detail status:', err)
  }
}

...
const doStartSimulation = async () => {
  ...
  // Reset all state first, ensure no influence from previous simulation
  resetAllState()
...
const fetchRunStatus = async () => {
  ...
  try {
    ...
    if (res.success && res.data) {
      ...
      // Detect if simulation is complete (via runner_status or platform completion)
      const isCompleted = data.runner_status === 'completed' || data.runner_status === 'stopped'
      
      // Extra check: if backend has not yet updated runner_status but platforms report completion
...
const fetchRunStatusDetail = async () => {
  ...
  try {
    ...
    if (res.success && res.data) {
      ...
      // No auto-scroll, let user freely browse timeline

    ...
    <div class="control-bar">
      <div class="status-group">
        ...
        <div class="platform-status twitter" :class="{ active: runStatus.twitter_running, completed: runStatus.twitter_completed }">
          ...
          <!-- Suggerimento azioni disponibili -->
      ...
      <div class="action-controls">
        ...
        >
          ...
          {{ isGeneratingReport ? 'Avvio in corso...' : 'Genera Report dei Risultati' }}
          ...
          >
            ...
            <div class="timeline-card">
              ...
              <div class="card-body">
                ...
                <!-- CREATE_COMMENT: Pubblica commento -->
                ...
                <!-- DO_NOTHING: Nessuna azione (silenzioso) -->
                ...
                <!-- Fallback generico: tipo sconosciuto o content non gestito sopra -->
...
const props = defineProps({
  simulationId: String,
  maxRounds: Number, // Numero massimo di turni passato da Step2
  minutesPerRound: {
    type: Number,
    default: 30 // Predefinito 30 minuti per turno
  },
  projectData: Object,
  graphData: Object,
  systemLogs: Array
...
const phase = ref(0) // 0: non iniziato, 1: in esecuzione, 2: completato
const isStarting = ref(false)
const isStopping = ref(false)
const startError = ref(null)
const runStatus = ref({})
const allActions = ref([]) // Tutte le azioni (accumulo incrementale)
const actionIds = ref(new Set()) // Set di ID azioni per deduplicazione
const scrollContainer = ref(null)

// Computed
// Visualizza azioni in ordine cronologico (le pi recenti in basso)
const chronologicalActions = computed(() => {
  return allActions.value
})

// Conteggio azioni per piattaforma
...
// Formatta il tempo simulato trascorso (calcolato dai turni e minuti per turno)
...
// Tempo simulato trascorso sulla piattaforma Twitter
const twitterElapsedTime = computed(() => {
  return formatElapsedTime(runStatus.value.twitter_current_round || 0)
})

// Tempo simulato trascorso sulla piattaforma Reddit
...
// Resetta tutti gli stati (per riavviare la simulazione)
const resetAllState = () => {
  phase.value = 0
  runStatus.value = {}
  allActions.value = []
  ...
  stopPolling()  // Ferma eventuali polling precedenti
}
...
const doStartSimulation = async () => {
  ...
  // Resetta tutti gli stati per evitare interferenze dalla simulazione precedente
  resetAllState()

  isStarting.value = true
  startError.value = null
  addLog('Avvio simulazione parallela su doppia piattaforma...')
  ...
  try {
    const params = {
      ...
      platform: 'parallel',
      force: true,  // Forza riavvio
      enable_graph_memory_update: true  // Attiva aggiornamento dinamico del grafo
    }
    
    if (props.maxRounds) {
      params.max_rounds = props.maxRounds
      addLog(`Numero massimo turni simulazione impostato: ${props.maxRounds}`)
    }
    
    addLog('Modalita aggiornamento dinamico del grafo attivata')
    
    const res = await startSimulation(params)
    
    if (res.success && res.data) {
      if (res.data.force_restarted) {
        addLog('✓ Log di simulazione precedenti eliminati, simulazione riavviata')
      }
      addLog('✓ Motore di simulazione avviato con successo')
      ...
      startError.value = res.error || 'Avvio fallito'
      addLog(`✗ Avvio fallito: ${res.error || 'Errore sconosciuto'}`)
      emit('update-status', 'error')
    }
  } catch (err) {
    startError.value = err.message
    addLog(`✗ Eccezione all'avvio: ${err.message}`)
    emit('update-status', 'error')
  } finally {
    isStarting.value = false
  }
...
const handleStopSimulation = async () => {
  ...
  addLog('Arresto simulazione in corso...')
  ...
  try {
    ...
    if (res.success) {
      ...
      addLog(`Arresto fallito: ${res.error || 'Errore sconosciuto'}`)
    }
  } catch (err) {
    addLog(`Eccezione all'arresto: ${err.message}`)
  } finally {
    isStopping.value = false
  }
}
...
// Traccia il turno precedente di ogni piattaforma per rilevare cambiamenti e generare log
...
const fetchRunStatus = async () => {
  ...
  try {
    ...
    if (res.success && res.data) {
      ...
      // Rileva i cambiamenti di turno per ogni piattaforma e genera log
      ...
      // Rileva se la simulazione e completata (tramite runner_status o stato completamento piattaforme)
      const isCompleted = data.runner_status === 'completed' || data.runner_status === 'stopped'
      
      // Controllo aggiuntivo: se il backend non ha ancora aggiornato runner_status, ma le piattaforme hanno gia riportato il completamento
      // Verifica tramite gli stati twitter_completed e reddit_completed
      const platformsCompleted = checkPlatformsCompleted(data)
      
      if (isCompleted || platformsCompleted) {
        if (platformsCompleted && !isCompleted) {
          addLog('✓ Rilevato completamento simulazione su tutte le piattaforme')
        }
        addLog('✓ Simulazione completata')
        phase.value = 2
        stopPolling()
        emit('update-status', 'completed')
      }
    ...
    console.warn('Recupero stato esecuzione fallito:', err)
  }
}

// Verifica se tutte le piattaforme attive sono completate
const checkPlatformsCompleted = (data) => {
  // Se non ci sono dati piattaforma, restituisci false
  if (!data) return false

  // Verifica lo stato di completamento di ogni piattaforma
  const twitterCompleted = data.twitter_completed === true
  const redditCompleted = data.reddit_completed === true
  
  // Se almeno una piattaforma e completata, verifica se tutte le piattaforme attive sono completate
  // Determina se una piattaforma e attiva tramite actions_count (se count > 0 o running era true)
  const twitterEnabled = (data.twitter_actions_count > 0) || data.twitter_running || twitterCompleted
  const redditEnabled = (data.reddit_actions_count > 0) || data.reddit_running || redditCompleted
  
  // Se nessuna piattaforma e attiva, restituisci false
...
const fetchRunStatusDetail = async () => {
  ...
  try {
    ...
    if (res.success && res.data) {
      // Usa all_actions per ottenere la lista completa delle azioni
      const serverActions = res.data.all_actions || []
      
      // Aggiungi nuove azioni in modo incrementale (con deduplicazione)
      ...
      // Nessuno scroll automatico, l'utente puo esplorare liberamente la timeline
      // Le nuove azioni vengono aggiunte in fondo
    }
  } catch (err) {
    console.warn('Recupero stato dettagliato fallito:', err)
  }
}
...
const handleNextStep = async () => {
  ...
  if (isGeneratingReport.value) {
    addLog('Richiesta di generazione report gia inviata, attendere...')
    return
  }

  isGeneratingReport.value = true
  addLog('Avvio generazione report...')
  ...
  try {
    ...
    if (res.success && res.data) {
      ...
      addLog(`✓ Attivita di generazione report avviata: ${reportId}`)

      // Naviga alla pagina del report
      router.push({ name: 'Report', params: { reportId } })
    } else {
      addLog(`✗ Avvio generazione report fallito: ${res.error || 'Errore sconosciuto'}`)
      isGeneratingReport.value = false
    }
  } catch (err) {
    addLog(`✗ Eccezione avvio generazione report: ${err.message}`)
    isGeneratingReport.value = false
  }
}
...
onMounted(() => {
  addLog('Step3 Inizializzazione esecuzione simulazione')
  if (props.simulationId) {
    doStartSimulation()
  }
})

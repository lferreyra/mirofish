      ...
      <div class="status-group">
        ...
        <div class="platform-status twitter" :class="{ active: runStatus.twitter_running, completed: runStatus.twitter_completed }">
          <div class="platform-header">
            ...
            <span class="platform-name">{{ $t('home.platform1Name') }}</span>
          ...
          <div class="platform-stats">
            <span class="stat">
              <span class="stat-label">{{ $t('common.roundLabel') }}</span>
              <span class="stat-value mono">{{ runStatus.twitter_current_round || 0 }}<span class="stat-total">/{{ runStatus.total_rounds || maxRounds || '-' }}</span></span>
            </span>
            <span class="stat">
              <span class="stat-label">{{ $t('common.timeLabel') }}</span>
              <span class="stat-value mono">{{ twitterElapsedTime }}</span>
            </span>
            <span class="stat">
              <span class="stat-label">{{ $t('common.actsLabel') }}</span>
              <span class="stat-value mono">{{ runStatus.twitter_actions_count || 0 }}</span>
            </span>
          </div>
          <!-- 可用动作提示 -->
          <div class="actions-tooltip">
            <div class="tooltip-title">{{ $t('common.availableActions') }}</div>
        ...
        <div class="platform-status reddit" :class="{ active: runStatus.reddit_running, completed: runStatus.reddit_completed }">
          <div class="platform-header">
            ...
            <span class="platform-name">{{ $t('home.platform2Name') }}</span>
    ...
    <div class="main-content-area" ref="scrollContainer">
      ...
      <div class="timeline-header" v-if="allActions.length > 0">
        <div class="timeline-stats">
          <span class="total-count">{{ $t('common.totalEventsLabel') }}: <span class="mono">{{ allActions.length }}</span></span>
              ...
              <div class="card-body">
                ...
                <template v-if="action.action_type === 'QUOTE_POST'">
                  ...
                  <div v-if="action.action_args?.original_content" class="quoted-block">
                    <div class="quote-header">
                      ...
                      <span class="quote-label">@{{ action.action_args.original_author_name || $t('common.defaultUser') }}</span>
                    </div>
                ...
                <template v-if="action.action_type === 'REPOST'">
                  <div class="repost-info">
                    ...
                    <span class="repost-label">{{ $t('common.repostedFrom') }} @{{ action.action_args?.original_author_name || $t('common.defaultUser') }}</span>
                  </div>
                ...
                <template v-if="action.action_type === 'LIKE_POST'">
                  <div class="like-info">
                    ...
                    <span class="like-label">{{ $t('common.likedPostBy') }} @{{ action.action_args?.post_author_name || $t('common.defaultUser') }}</span>
                  </div>
                ...
                <template v-if="action.action_type === 'CREATE_COMMENT'">
                  ...
                  <div v-if="action.action_args?.post_id" class="comment-context">
                    ...
                    <span>{{ $t('common.replyToPost') }} #{{ action.action_args.post_id }}</span>
                  </div>
                </template>
                ...
                <template v-if="action.action_type === 'SEARCH_POSTS'">
                  <div class="search-info">
                    ...
                    <span class="search-label">{{ $t('common.searchQueryLabel') }}:</span>
                ...
                <template v-if="action.action_type === 'FOLLOW'">
                  <div class="follow-info">
                    ...
                    <span class="follow-label">Followed @{{ action.action_args?.target_user || action.action_args?.user_id || $t('common.defaultUser') }}</span>
                  </div>
                </template>
                ...
                <template v-if="action.action_type === 'UPVOTE_POST' || action.action_type === 'DOWNVOTE_POST'">
                  <div class="vote-info">
                    ...
                    <span class="vote-label">{{ action.action_type === 'UPVOTE_POST' ? $t('common.upvoted') : $t('common.downvoted') }} Post</span>
                  </div>
                ...
                <template v-if="action.action_type === 'DO_NOTHING'">
                  <div class="idle-info">
                    ...
                    <span class="idle-label">{{ $t('common.actionSkipped') }}</span>
                  </div>
                </template>
  ...
  try {
    ...
    if (res.success && res.data) {
      ...
      if (data.twitter_current_round > prevTwitterRound.value) {
        addLog(`[${t('common.plazaLabel')}] ${t('common.roundLabel')} ${data.twitter_current_round}/${data.total_rounds} | ${t('common.timeLabel')}:${data.twitter_simulated_hours || 0}h | ${t('common.actsLabel')}:${data.twitter_actions_count}`)
        prevTwitterRound.value = data.twitter_current_round
      }
      
      if (data.reddit_current_round > prevRedditRound.value) {
        addLog(`[${t('common.communityLabel')}] ${t('common.roundLabel')} ${data.reddit_current_round}/${data.total_rounds} | ${t('common.timeLabel')}:${data.reddit_simulated_hours || 0}h | ${t('common.actsLabel')}:${data.reddit_actions_count}`)
...
const getActionTypeLabel = (type) => {
  const labels = {
    'CREATE_POST': t('common.actionTypes.post'),
    'REPOST': t('common.actionTypes.repost'),
    'LIKE_POST': t('common.actionTypes.like'),
    'CREATE_COMMENT': t('common.actionTypes.comment'),
    'LIKE_COMMENT': t('common.actionTypes.like'),
    'DO_NOTHING': t('common.actionTypes.idle'),
    'FOLLOW': t('common.actionTypes.follow'),
    'SEARCH_POSTS': t('common.actionTypes.search'),
    'QUOTE_POST': t('common.actionTypes.quote'),
    'UPVOTE_POST': t('common.actionTypes.upvote'),
    'DOWNVOTE_POST': t('common.actionTypes.downvote')
  }
  return labels[type] || type || t('common.actionTypes.unknown')
}

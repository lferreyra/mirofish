    ...
    <div class="control-bar">
      ...
      <div class="action-controls">
        ...
        >
          ...
          {{ isGeneratingReport ? 'Starting...' : 'Generate Final Report' }} 
...
const doStartSimulation = async () => {
  ...
  try {
    ...
    if (props.maxRounds) {
      ...
      addLog(`Max rounds set to: ${props.maxRounds}`)
    }
    
    addLog('Graph memory live updates enabled')
    
    const res = await startSimulation(params)
    
    if (res.success && res.data) {
      if (res.data.force_restarted) {
        addLog('✓ Cleared previous simulation logs and restarted')
      }
    ...
    if (res.success && res.data) {
      ...
      if (isCompleted || platformsCompleted) {
        if (platformsCompleted && !isCompleted) {
          addLog('✓ Detected that all platform runs have ended')
        }
...
const handleNextStep = async () => {
  ...
  try {
    ...
    addLog(`✗ Report generation start error: ${err.message}`)
    isGeneratingReport.value = false
  }
}

    ...
    <div class="control-bar">
      ...
      <div class="action-controls">
        ...
        >
          ...
          {{ isGeneratingReport ? $t('step3.startReportBtnLoading') : $t('step3.startReportBtn') }}
...
const doStartSimulation = async () => {
  if (!props.simulationId) {
    addLog(t('step3.errorMissingSimId'))
    return
  }
  ...
  startError.value = null
  addLog(t('step3.launchingSimulation'))
  ...
  try {
    ...
    if (props.maxRounds) {
      ...
      addLog(t('step3.setMaxRounds', { n: props.maxRounds }))
    }

    addLog(t('step3.dynamicGraphEnabled'))
    ...
    if (res.success && res.data) {
      ...
      addLog(t('step3.startFailedMsg', { msg: res.error || t('common.unknownError') }))
      emit('update-status', 'error')
    }
  } catch (err) {
    startError.value = err.message
    addLog(t('step3.startError', { msg: err.message }))
    emit('update-status', 'error')
  } finally {
    isStarting.value = false
  }
...
const handleStopSimulation = async () => {
  ...
  try {
    ...
    if (res.success) {
      ...
      addLog(t('step3.stopFailed', { msg: res.error || t('common.unknownError') }))
    }
  } catch (err) {
    addLog(t('step3.stopError', { msg: err.message }))
  } finally {
    isStopping.value = false
  }
}
...
const handleNextStep = async () => {
  ...
  if (isGeneratingReport.value) {
    addLog(t('step3.reportRequested'))
    return
  }

  isGeneratingReport.value = true
  addLog(t('step3.launchingReport'))
  ...
  try {
    ...
    if (res.success && res.data) {
      ...
      addLog(t('step3.reportStartFailed', { msg: res.error || t('common.unknownError') }))
      isGeneratingReport.value = false
    }
  } catch (err) {
    addLog(t('step3.reportStartError', { msg: err.message }))
    isGeneratingReport.value = false
  }
}

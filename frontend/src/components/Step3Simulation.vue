...
const actionIds = ref(new Set()) // Set for deduplicating action IDs
const scrollContainer = ref(null)

// Computed
// Show actions chronologically (newest at bottom)
  ...
  try {
    ...
    if (res.success && res.data) {
      if (res.data.force_restarted) {
        addLog('Done: Cleared old simulation logs, restarting simulation')
      }
      addLog('Done: Simulation engine started successfully')
      ...
      addLog(`Failed: Start failed: ${res.error || 'Unknown error'}`)
      emit('update-status', 'error')
    }
  } catch (err) {
    startError.value = err.message
    addLog(`Failed: Start error: ${err.message}`)
    emit('update-status', 'error')
  } finally {
    isStarting.value = false
  }
...
const handleStopSimulation = async () => {
  ...
  try {
    ...
    if (res.success) {
      addLog('Done: Simulation stopped')
      phase.value = 2
      stopPolling()
...
// Track previous round per platform for change detection and logging
...
const fetchRunStatus = async () => {
  ...
  try {
    ...
    if (res.success && res.data) {
      ...
      // Detect per-platform round changes and output logs
      ...
      // Check if simulation is completed (via runner_status or platform completion)
      ...
      if (isCompleted || platformsCompleted) {
        if (platformsCompleted && !isCompleted) {
          addLog('Done: All platform simulations completed')
        }
        addLog('Done: Simulation completed')
        phase.value = 2
        stopPolling()
        emit('update-status', 'completed')
      }
...
const checkPlatformsCompleted = (data) => {
  ...
  // If no platforms enabled, return false
...
const handleNextStep = async () => {
  ...
  try {
    ...
    if (res.success && res.data) {
      ...
      addLog(`Done: Report generation task started: ${reportId}`)
      
      // Navigate to report page
      router.push({ name: 'Report', params: { reportId } })
    } else {
      addLog(`Failed: Report generation failed: ${res.error || 'Unknown error'}`)
      isGeneratingReport.value = false
    }
  } catch (err) {
    addLog(`Failed: Report generation error: ${err.message}`)
    isGeneratingReport.value = false
  }
}

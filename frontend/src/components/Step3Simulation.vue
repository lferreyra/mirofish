    ...
    <div class="control-bar">
      <div class="status-group">
        ...
        <div class="platform-status twitter" :class="{ active: runStatus.twitter_running, completed: runStatus.twitter_completed }">
          <div class="platform-header">
            ...
            <span class="platform-name">{{ $t('step3.control.plaza_name') }}</span>
          ...
          <div class="platform-stats">
            <span class="stat">
              <span class="stat-label">{{ $t('step3.control.round') }}</span>
              <span class="stat-value mono">{{ runStatus.twitter_current_round || 0 }}<span class="stat-total">/{{ runStatus.total_rounds || maxRounds || '-' }}</span></span>
            </span>
            <span class="stat">
              <span class="stat-label">{{ $t('step3.control.elapsed') }}</span>
              <span class="stat-value mono">{{ twitterElapsedTime }}</span>
            </span>
            <span class="stat">
              <span class="stat-label">{{ $t('step3.control.acts') }}</span>
        ...
        <div class="platform-status reddit" :class="{ active: runStatus.reddit_running, completed: runStatus.reddit_completed }">
          <div class="platform-header">
            ...
            <span class="platform-name">{{ $t('step3.control.community_name') }}</span>
      ...
      <div class="action-controls">
        ...
        >
          ...
          {{ isGeneratingReport ? $t('step3.control.starting') : $t('step3.control.start_report') }} 
    ...
    <div class="main-content-area" ref="scrollContainer">
      ...
      <div class="timeline-header" v-if="allActions.length > 0">
        <div class="timeline-stats">
          <span class="total-count">{{ $t('step3.timeline.total_events') }}: <span class="mono">{{ allActions.length }}</span></span>
              ...
              <div class="card-body">
                ...
                <template v-if="action.action_type === 'REPOST'">
                  <div class="repost-info">
                    ...
                    <span class="repost-label">{{ $t('step3.timeline.reposted_from', { author: action.action_args?.original_author_name || 'User' }) }}</span>
                  </div>
                ...
                <template v-if="action.action_type === 'LIKE_POST'">
                  <div class="like-info">
                    ...
                    <span class="like-label">{{ $t('step3.timeline.liked_post', { author: action.action_args?.post_author_name || 'User' }) }}</span>
                  </div>
                ...
                <template v-if="action.action_type === 'CREATE_COMMENT'">
                  ...
                  <div v-if="action.action_args?.post_id" class="comment-context">
                    ...
                    <span>{{ $t('step3.timeline.reply_to', { id: action.action_args.post_id }) }}</span>
                  </div>
                </template>
                ...
                <template v-if="action.action_type === 'SEARCH_POSTS'">
                  <div class="search-info">
                    ...
                    <span class="search-label">{{ $t('step3.timeline.search_query') }}</span>
                ...
                <template v-if="action.action_type === 'FOLLOW'">
                  <div class="follow-info">
                    ...
                    <span class="follow-label">{{ $t('step3.timeline.followed_user', { user: action.action_args?.target_user || action.action_args?.user_id || 'User' }) }}</span>
                  </div>
                </template>
                ...
                <template v-if="action.action_type === 'DO_NOTHING'">
                  <div class="idle-info">
                    ...
                    <span class="idle-label">{{ $t('step3.timeline.action_skipped') }}</span>
                  </div>
                </template>
...
const doStartSimulation = async () => {
  if (!props.simulationId) {
    addLog(t('step3.logs.err_no_sim_id'))
    return
  }
  ...
  startError.value = null
  addLog(t('step3.logs.starting_sim'))
  ...
  try {
    ...
    if (props.maxRounds) {
      ...
      addLog(t('step3.logs.set_max_rounds', { rounds: props.maxRounds }))
    }
    
    addLog(t('step3.logs.dynamic_graph_enabled'))
    
    const res = await startSimulation(params)
    
    if (res.success && res.data) {
      if (res.data.force_restarted) {
        addLog(t('step3.logs.old_logs_cleaned'))
      }
      addLog(t('step3.logs.engine_started'))
      ...
      startError.value = res.error || t('common.unknown_error')
      addLog(t('step3.logs.report_failed', { error: res.error || t('common.unknown_error') }))
      emit('update-status', 'error')
    }
  } catch (err) {
    startError.value = err.message
    addLog(t('step3.logs.report_exception', { error: err.message }))
    emit('update-status', 'error')
  } finally {
    isStarting.value = false
  }
...
const handleStopSimulation = async () => {
  ...
  addLog(t('step3.logs.stop_sim'))
  
  try {
    const res = await stopSimulation({ simulation_id: props.simulationId })
    
    if (res.success) {
      addLog(t('step3.logs.sim_stopped'))
      phase.value = 2
      stopPolling()
      emit('update-status', 'completed')
    } else {
      addLog(t('step3.logs.stop_failed', { error: res.error || t('common.unknown_error') }))
    }
  } catch (err) {
    addLog(t('step3.logs.stop_exception', { error: err.message }))
  } finally {
    isStopping.value = false
  }
}
    ...
    if (res.success && res.data) {
      ...
      if (isCompleted || platformsCompleted) {
        if (platformsCompleted && !isCompleted) {
          addLog(t('step3.logs.all_platforms_ended'))
        }
        addLog(t('step3.logs.sim_completed'))
        phase.value = 2
        stopPolling()
        emit('update-status', 'completed')
      }
...
const handleNextStep = async () => {
  ...
  if (isGeneratingReport.value) {
    addLog(t('step3.logs.report_pending'))
    return
  }
  
  isGeneratingReport.value = true
  addLog(t('step3.logs.starting_report'))
  ...
  try {
    ...
    if (res.success && res.data) {
      ...
      addLog(t('step3.logs.report_task_started', { id: reportId }))
...
onMounted(() => {
  addLog(t('step3.logs.step3_init'))
  if (props.simulationId) {
    doStartSimulation()
  }
})

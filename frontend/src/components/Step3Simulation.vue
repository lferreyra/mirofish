  ...
  <div class="simulation-panel">
    ...
    <div class="control-bar">
      <div class="status-group">
        ...
        <div class="platform-status twitter" :class="{ active: runStatus.twitter_running, completed: runStatus.twitter_completed }">
          <div class="platform-header">
            ...
            <span class="platform-name">{{ $t('step3.platform.infoPlaza') }}</span>
          ...
          <div class="platform-stats">
            <span class="stat">
              <span class="stat-label">{{ $t('step3.stats.round') }}</span>
              <span class="stat-value mono">{{ runStatus.twitter_current_round || 0 }}<span class="stat-total">/{{ runStatus.total_rounds || maxRounds || '-' }}</span></span>
            </span>
            <span class="stat">
              <span class="stat-label">{{ $t('step3.stats.elapsedTime') }}</span>
              <span class="stat-value mono">{{ twitterElapsedTime }}</span>
            </span>
            <span class="stat">
              <span class="stat-label">{{ $t('step3.stats.acts') }}</span>
              <span class="stat-value mono">{{ runStatus.twitter_actions_count || 0 }}</span>
            </span>
          </div>
          <!-- Available actions tooltip -->
          <div class="actions-tooltip">
            <div class="tooltip-title">{{ $t('step3.platform.tooltipTitle') }}</div>
        ...
        <div class="platform-status reddit" :class="{ active: runStatus.reddit_running, completed: runStatus.reddit_completed }">
          <div class="platform-header">
            ...
            <span class="platform-name">{{ $t('step3.platform.topicCommunity') }}</span>
      ...
      <div class="action-controls">
        ...
        >
          ...
          {{ isGeneratingReport ? $t('step3.btn.starting') : $t('step3.btn.generateReport') }}
    ...
    <div class="main-content-area" ref="scrollContainer">
      ...
      <div class="timeline-header" v-if="allActions.length > 0">
        <div class="timeline-stats">
          <span class="total-count">{{ $t('step3.timeline.totalEvents') }} <span class="mono">{{ allActions.length }}</span></span>
      ...
      <div class="timeline-feed">
              ...
              <div class="card-body">
                ...
                <template v-if="action.action_type === 'REPOST'">
                  <div class="repost-info">
                    ...
                    <span class="repost-label">{{ $t('step3.actions.repostedFrom', { author: action.action_args?.original_author_name || 'User' }) }}</span>
                  </div>
                ...
                <template v-if="action.action_type === 'LIKE_POST'">
                  <div class="like-info">
                    ...
                    <span class="like-label">{{ $t('step3.actions.likedPost', { author: action.action_args?.post_author_name || 'User' }) }}</span>
                  </div>
                ...
                <template v-if="action.action_type === 'CREATE_COMMENT'">
                  ...
                  <div v-if="action.action_args?.post_id" class="comment-context">
                    ...
                    <span>{{ $t('step3.actions.replyToPost', { id: action.action_args.post_id }) }}</span>
                  </div>
                </template>
                ...
                <template v-if="action.action_type === 'SEARCH_POSTS'">
                  <div class="search-info">
                    ...
                    <span class="search-label">{{ $t('step3.actions.searchQuery') }}</span>
                ...
                <template v-if="action.action_type === 'FOLLOW'">
                  <div class="follow-info">
                    ...
                    <span class="follow-label">{{ $t('step3.actions.followed', { user: action.action_args?.target_user || action.action_args?.user_id || 'User' }) }}</span>
                  </div>
                </template>
                ...
                <template v-if="action.action_type === 'UPVOTE_POST' || action.action_type === 'DOWNVOTE_POST'">
                  <div class="vote-info">
                    ...
                    <span class="vote-label">{{ action.action_type === 'UPVOTE_POST' ? $t('step3.actions.upvoted') : $t('step3.actions.downvoted') }} Post</span>
                  </div>
                ...
                <template v-if="action.action_type === 'DO_NOTHING'">
                  <div class="idle-info">
                    ...
                    <span class="idle-label">{{ $t('step3.actions.actionSkipped') }}</span>
                  </div>
                </template>
        ...
        <div v-if="allActions.length === 0" class="waiting-state">
          ...
          <span>{{ $t('step3.timeline.waitingForActions') }}</span>
        </div>
      </div>
    </div>
    ...
    <div class="system-logs">
      <div class="log-header">
        <span class="log-title">{{ $t('step3.monitor.title') }}</span>
        <span class="log-id">{{ simulationId || $t('step3.monitor.noSimulation') }}</span>
      </div>
...
// Format simulated elapsed time (computed from round count and minutes per round)
...
// Simulated elapsed time for Info Plaza
const twitterElapsedTime = computed(() => {
  return formatElapsedTime(runStatus.value.twitter_current_round || 0)
})

// Simulated elapsed time for Topic Community
...
const doStartSimulation = async () => {
  ...
  // Reset state first to avoid contamination from a previous run
  resetAllState()
...
const fetchRunStatus = async () => {
  ...
  try {
    ...
    if (res.success && res.data) {
      ...
      // Check if simulation is complete via runner_status or platform completion flags
      const isCompleted = data.runner_status === 'completed' || data.runner_status === 'stopped'

      // Extra check: backend may not have updated runner_status yet but platforms report done
      const platformsCompleted = checkPlatformsCompleted(data)

      if (isCompleted || platformsCompleted) {
        if (platformsCompleted && !isCompleted) {
          addLog('✓ All platforms detected as finished')
        }
...
const checkPlatformsCompleted = (data) => {
  ...
  // Determine if a platform is enabled: actions_count > 0, was running, or already completed
...
const fetchRunStatusDetail = async () => {
  ...
  try {
    ...
    if (res.success && res.data) {
      ...
      // No auto-scroll — user can freely browse the timeline

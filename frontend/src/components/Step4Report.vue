  ...
  <div class="report-panel">
    ...
    <div class="main-split-layout">
      ...
      <div class="left-panel report-style" ref="leftPanel">
        ...
        <!-- Waiting State -->
        <div v-if="!reportOutline && !errorMessage" class="waiting-placeholder">
        ...
        <div v-if="!reportOutline && errorMessage" class="waiting-placeholder waiting-placeholder--error">
          <div class="waiting-animation waiting-animation--error">
            <div class="waiting-ring"></div>
          </div>
          <span class="waiting-text">Report generation failed</span>
          <p class="waiting-error">{{ errorMessage }}</p>
        </div>
      </div>
...
// Parse functions (English markers + legacy Chinese for old logs)
...
const parsePanorama = (text) => {
  ...
  try {
    ...
    const activeSection = text.match(/(?:### Current active facts[^\n]*|### 【当前有效事实】[^\n]*)\n([\s\S]*?)(?=\n###|$)/)
    if (activeSection) {
...
const parseInterview = (text) => {
  ...
  try {
    ...
    const countMatch = text.match(/\*\*(?:Agents interviewed|采访人数):\*\*\s*(\d+)\s*\/\s*(\d+)/)
    if (countMatch) {
    ...
    const reasonMatch = text.match(/### (?:Rationale for agent selection|采访对象选择理由)\n([\s\S]*?)(?=\n---\n|\n### (?:Interview transcript|采访实录))/)
    if (reasonMatch) {
    ...
    const parseIndividualReasons = (reasonText) => {
      ...
      for (const line of lines) {
        ...
        if (!headerMatch) {
          headerMatch = line.match(/^-\s*(?:Select|选择)\s*([^（(]+)(?:[（(]index\s*=?\s*\d+[)）])?[：:]\s*(.*)/)
          if (headerMatch) {
            name = headerMatch[1].trim()
            reasonStart = headerMatch[2]
          }
        ...
        } else if (currentName && line.trim() && !line.match(/^未选|^综上|^最终选择|^Not selected|^In summary|^Final selection/i)) {
          // 理由的续行（排除结尾总结段落）
    ...
    interviewBlocks.forEach((block, index) => {
      ...
      if (answerMatch) {
        ...
        const twitterMatch = answerText.match(/(?:\[Twitter response\]|【Twitter平台回答】)\n?([\s\S]*?)(?=(?:\[Reddit response\]|【Reddit平台回答】)|$)/)
        const redditMatch = answerText.match(/(?:\[Reddit response\]|【Reddit平台回答】)\n?([\s\S]*?)$/)
        ...
        if (!twitterMatch && redditMatch) {
          if (interview.redditAnswer && interview.redditAnswer !== '(No reply on this platform)' && interview.redditAnswer !== '（该平台未获得回复）') {
            interview.twitterAnswer = interview.redditAnswer
          }
        } else if (twitterMatch && !redditMatch) {
          if (interview.twitterAnswer && interview.twitterAnswer !== '(No reply on this platform)' && interview.twitterAnswer !== '（该平台未获得回复）') {
    ...
    // 提取采访摘要
    const summaryMatch = text.match(/### (?:Summary and key takeaways|采访摘要与核心观点)\n([\s\S]*?)$/)
    if (summaryMatch) {
...
const parseQuickSearch = (text) => {
  ...
  try {
    ...
    const countMatch = text.match(/(?:Found (\d+) results|找到\s*(\d+)\s*条)/)
      ...
      h('div', { class: 'insight-content' }, [
        ...
        activeTab.value === 'facts' && props.result.facts.length > 0 && h('div', { class: 'facts-panel' }, [
          h('div', { class: 'panel-header' }, [
            h('span', { class: 'panel-title' }, 'Latest key facts from retrieval'),
        ...
        activeTab.value === 'subqueries' && props.result.subQueries.length > 0 && h('div', { class: 'subqueries-panel' }, [
          h('div', { class: 'panel-header' }, [
            h('span', { class: 'panel-title' }, 'Sub-questions generated for analysis'),
        ...
        // Empty state
        activeTab.value === 'facts' && props.result.facts.length === 0 && h('div', { class: 'empty-state' }, 'No key facts yet'),
        activeTab.value === 'entities' && props.result.entities.length === 0 && h('div', { class: 'empty-state' }, 'No core entities yet'),
        activeTab.value === 'relations' && props.result.relations.length === 0 && h('div', { class: 'empty-state' }, 'No relationship chains yet')
      ])
    ])
  }
}
    ...
    return () => h('div', { class: 'panorama-display' }, [
      ...
      h('div', { class: 'panorama-tabs' }, [
        ...
        h('button', {
          ...
          h('span', { class: 'tab-label' }, `Historical (${props.result.historicalFacts.length})`)
        ]),
        h('button', {
      ...
      h('div', { class: 'panorama-content' }, [
        ...
        activeTab.value === 'active' && h('div', { class: 'facts-panel active-facts' }, [
          ...
          ) : h('div', { class: 'empty-state' }, 'No active facts yet'),
        ...
        activeTab.value === 'historical' && h('div', { class: 'facts-panel historical-facts' }, [
          ...
          ) : h('div', { class: 'empty-state' }, 'No historical facts yet'),
...
const InterviewDisplay = {
  ...
  setup(props) {
    ...
    const isPlaceholderText = (text) => {
      if (!text) return true
      const t = text.trim()
      return t === '(No reply on this platform)' || t === '（该平台未获得回复）' || t === '(该平台未获得回复)' || t === '[无回复]'
    }
    ...
    return () => h('div', { class: 'quick-search-display' }, [
      ...
      h('div', { class: 'quicksearch-header' }, [
        ...
        props.result.query && h('div', { class: 'header-query' }, [
          h('span', { class: 'query-label' }, 'Query: '),
      ...
      h('div', { class: ['quicksearch-content', { 'no-tabs': !showTabs.value }] }, [
        ...
        ((!showTabs.value) || activeTab.value === 'facts') && h('div', { class: 'facts-panel' }, [
          ...
          ) : h('div', { class: 'empty-state' }, 'No matching results'),
          props.result.facts.length > INITIAL_SHOW_COUNT && h('button', {
            class: 'expand-btn',
            onClick: () => { expandedFacts.value = !expandedFacts.value }
          }, expandedFacts.value ? `Collapse ▲` : `Show all ${props.result.facts.length} ▼`)
...
.waiting-animation--error .waiting-ring {
  border-color: #FCA5A5;
  animation: none;
}

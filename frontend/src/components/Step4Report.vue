  ...
  try {
    ...
    const parseIndividualReasons = (reasonText) => {
      ...
      for (const line of lines) {
        ...
        // Format 2: - 选择name（index X）: reason
    ...
    interviewBlocks.forEach((block, index) => {
      ...
      if (qMatch) {
        ...
        if (questions.length > 0) {
          // If the first question is preceded by "1.", handle specially
      ...
      if (answerMatch) {
        ...
        // Platform fallback logic (backward compat: only one platform marker present)
        if (!twitterMatch && redditMatch) {
          // Only Reddit answer available; copy as default display if not placeholder text
      ...
      // Extract key quotes (supports multiple quote mark formats)
...
const InterviewDisplay = {
  ...
  setup(props) {
    ...
    const splitAnswerByQuestions = (answerText, questionCount) => {
      ...
      // If no numbering found or only one, return as a whole
      if (matches.length <= 1) {
    ...
    // Check if a question has dual-platform answers (filtering out placeholder text)
...
const renderMarkdown = (content) => {
  ...
  // Remove leading h2 title (## xxx), since section title is already shown in the outer container
  ...
  // Process lists - supports nested lists
  ...
  // Clean up <br> between block element closing tags and paragraph opening tags
  html = html.replace(/(<\/ol>|<\/ul>|<\/blockquote>)<br>(<p|<div)/g, '$1$2')

  // Fix non-contiguous ordered list numbering: when single-item <ol>s are separated by paragraph content, maintain incrementing numbers
      ...
      if (newLogs.length > 0) {
        newLogs.forEach(log => {
          ...
          if (log.action === 'report_complete') {
            ...
            stopPolling()
            // Scroll logic is handled uniformly in the nextTick after the loop
          }
...
const extractFinalContent = (response) => {
  ...
  // If it starts with ## or # or >, it's likely direct markdown content

...
const toggleRawResult = (timestamp, event) => {
  ...
  // After DOM update, adjust scroll position to keep button at same viewport position
...
const parseInsightForge = (text) => {
  ...
  try {
    ...
    // Extract sub-questions - full extraction, unlimited count
    ...
    // Extract key facts - full extraction, unlimited count
    ...
    // Extract relation chains - full extraction, unlimited count
...
const parsePanorama = (text) => {
  ...
  try {
    ...
    // Extract current active facts - full extraction, unlimited count
    ...
    // Extract historical/expired facts - full extraction, unlimited count
    ...
    // Extract involved entities - full extraction, unlimited count
  ...
  try {
    ...
    const parseIndividualReasons = (reasonText) => {
      ...
      for (const line of lines) {
        ...
        // e.g.: 1. **alumni_345（index=1）**: As an alumni...
        ...
        if (name) {
          ...
          // Continuation of reason (exclude summary paragraphs at end)
    ...
    interviewBlocks.forEach((block, index) => {
      ...
      if (qMatch) {
        ...
        if (questions.length > 0) {
          // If the first question has "1." prefix, needs special handling
      ...
      if (answerMatch) {
        ...
        // Platform fallback logic (backward compatible: only one platform marker)
...
const parseQuickSearch = (text) => {
  ...
  try {
    ...
    // Extract related facts - full extraction, unlimited count
...
const renderMarkdown = (content) => {
  ...
  // Remove leading h2 title (## xxx), since section title is already displayed externally
  ...
  // Clean up <br> before paragraph start tag after block-level elements
  html = html.replace(/(<\/ol>|<\/ul>|<\/blockquote>)<br>(<p|<div)/g, '$1$2')

  // Fix non-consecutive ordered list numbering: when single-item <ol> is separated by paragraph content, maintain incrementing numbers
      ...
      if (newLogs.length > 0) {
        newLogs.forEach(log => {
          ...
          if (log.action === 'report_complete') {
            ...
            stopPolling()
            // Scroll logic is unified in nextTick after loop ends
          }
...
const extractFinalContent = (response) => {
  ...
  // Try to find content after Chinese "最终答案:" marker
  const chineseFinalMatch = response.match(/最终答案[:：]\s*\n*([\s\S]*)$/i)
  if (chineseFinalMatch) {
    return chineseFinalMatch[1].trim()
  }
  
  // If it starts with ## or # or >, it might be direct markdown content
  const trimmedResponse = response.trim()
  if (trimmedResponse.match(/^[#>]/)) {
    return trimmedResponse
  }
  
  // If content is long and contains markdown formatting, try removing thinking process

...
const parseInsightForge = (text) => {
  ...
  try {
    ...
    const subQSection = text.match(/### Sub-queries analyzed\n([\s\S]*?)(?=\n###|$)/)
    if (subQSection) {
...
const parsePanorama = (text) => {
  ...
  try {
    // Extract query
    const queryMatch = text.match(/(?:Analysis query|Search query|Query):\s*(.+?)(?:\n|$)/)
    if (queryMatch) result.query = queryMatch[1].trim()
    ...
    // Extract currently valid facts - full extraction, no count limit
  ...
  try {
    ...
    const parseIndividualReasons = (reasonText) => {
      ...
      for (const line of lines) {
        ...
        // e.g.: - Select parent_601 (index 0): as a representative of parent groups...
        if (!headerMatch) {
        ...
        // e.g.: - **parent_601 (index 0)**: as a representative of parent groups...
        if (!headerMatch) {
    ...
    return () => h('div', { class: 'insight-display' }, [
      ...
      h('div', { class: 'insight-content' }, [
        ...
        activeTab.value === 'subqueries' && props.result.subQueries.length > 0 && h('div', { class: 'subqueries-panel' }, [
          h('div', { class: 'panel-header' }, [
            h('span', { class: 'panel-title' }, 'Drift query generated sub-queries'),
    ...
    return () => h('div', { class: 'panorama-display' }, [
      ...
      h('div', { class: 'panorama-tabs' }, [
        h('button', {
          ...
          h('span', { class: 'tab-label' }, `Currently valid memories (${props.result.activeFacts.length})`)
        ]),
        h('button', {
      ...
      h('div', { class: 'panorama-content' }, [
        ...
        activeTab.value === 'active' && h('div', { class: 'facts-panel active-facts' }, [
          h('div', { class: 'panel-header' }, [
            h('span', { class: 'panel-title' }, 'Currently valid memories'),
          ...
          ) : h('div', { class: 'empty-state' }, 'No currently valid memories'),
...
const InterviewDisplay = {
  ...
  setup(props) {
    ...
    const splitAnswerByQuestions = (answerText, questionCount) => {
      ...
      // 1. "Question N:" or "问题N：" (English or Chinese format, new backend format)
      // 2. "1. " or "\n1. " (number+period, old format compat)
      let matches = []
      let match

      // Prefer "Question N:" / "问题N：" format (both Chinese and English)
      const cnPattern = /(?:^|[\r\n]+)(?:问题|Question)\s*(\d+)[：:]\s*/g

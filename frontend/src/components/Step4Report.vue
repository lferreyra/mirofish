    ...
    <div class="main-split-layout">
            ...
            >
              ...
              <div class="section-body" v-show="!collapsedSections.has(idx)">
                ...
                <div v-else-if="currentSectionIndex === idx + 1" class="loading-state">
                  ...
                  <span class="loading-text">In generazione {{ section.title }}...</span>
                </div>
              </div>
            </div>
          </div>
      ...
      <div class="right-panel" ref="rightPanel">
        ...
        <div class="workflow-overview" v-if="agentLogs.length > 0 || reportOutline">
          ...
          <!-- Pulsante passo successivo - mostrato al completamento -->
          <button v-if="isComplete" class="next-step-btn" @click="goToInteraction">
            <span>Interazione Approfondita</span>
            ...
            >
              ...
              <div class="timeline-content">
                ...
                <div class="timeline-body" :class="{ 'collapsed': isLogCollapsed(log) }" @click="toggleLogExpand(log)">
                  ...
                  <!-- Contenuto Sezione Generato (il contenuto e generato, ma l'intera sezione potrebbe non essere ancora completata) -->
                  ...
                  <!-- Sezione Completata (generazione sezione completata) -->
                  ...
                  <template v-if="log.action === 'llm_response'">
                    ...
                    <!-- Quando e la risposta finale, mostra avviso speciale -->
...
const toggleRawResult = (timestamp, event) => {
  // Salva posizione del pulsante relativa al viewport
  ...
  // Dopo l'aggiornamento del DOM, adatta la posizione di scroll per mantenere il pulsante nella stessa posizione
...
const toggleSectionCollapse = (idx) => {
  // Solo le sezioni completate possono essere compresse
  if (!generatedSections.value[idx + 1]) return
...
const toolConfig = {
  'insight_forge': {
    name: 'Deep Insight',
    color: 'purple',
    icon: 'lightbulb' // Icona lampadina - rappresenta insight
  },
  'panorama_search': {
    name: 'Panorama Search',
    color: 'blue',
    icon: 'globe' // Icona globo - rappresenta ricerca panoramica
  },
  'interview_agents': {
    name: 'Agent Interview',
    color: 'green',
    icon: 'users' // Icona utenti - rappresenta interviste
  },
  'quick_search': {
    name: 'Quick Search',
    color: 'orange',
    icon: 'zap' // Icona fulmine - rappresenta velocita
  },
  'get_graph_statistics': {
    name: 'Graph Stats',
    color: 'cyan',
    icon: 'chart' // Icona grafico - rappresenta statistiche
  },
  'get_entities_by_type': {
    name: 'Entity Query',
    color: 'pink',
    icon: 'database' // Icona database - rappresenta entita
  }
}
...
const parseInsightForge = (text) => {
  ...
  try {
    // Estrai domanda di analisi
    const queryMatch = text.match(/分析问题:\s*(.+?)(?:\n|$)/)
    if (queryMatch) result.query = queryMatch[1].trim()
    
    // Estrai scenario di previsione
    const reqMatch = text.match(/预测场景:\s*(.+?)(?:\n|$)/)
    if (reqMatch) result.simulationRequirement = reqMatch[1].trim()
    
    // Estrai dati statistici - corrisponde al formato "相关预测事实: X条"
    ...
    // Estrai sotto-domande - estrazione completa, senza limite di quantita
    ...
    // Estrai fatti chiave - estrazione completa, senza limite di quantita
    ...
    // Estrai entita principali - estrazione completa, incluso riepilogo e conteggio fatti correlati
    const entitySection = text.match(/### 【核心实体】\n([\s\S]*?)(?=\n###|$)/)
    if (entitySection) {
      const entityText = entitySection[1]
      // Dividi blocchi entita per "- **"
    ...
    // Estrai catene di relazioni - estrazione completa, senza limite di quantita
...
const parsePanorama = (text) => {
  ...
  try {
    ...
    // Estrai fatti attualmente validi - estrazione completa, senza limite di quantita
    const activeSection = text.match(/### 【当前有效事实】[\s\S]*?\n([\s\S]*?)(?=\n###|$)/)
    if (activeSection) {
      const lines = activeSection[1].split('\n').filter(l => l.match(/^\d+\./))
      result.activeFacts = lines.map(l => {
        // Rimuovi numerazione e virgolette
        const factText = l.replace(/^\d+\.\s*/, '').replace(/^"|"$/g, '').trim()
        return factText
      }).filter(Boolean)
    }
    
    // Estrai fatti storici/scaduti - estrazione completa, senza limite di quantita
    ...
    // Estrai entita coinvolte - estrazione completa, senza limite di quantita
...
const parseInterview = (text) => {
  ...
  try {
    // Estrai tema dell'intervista
    const topicMatch = text.match(/\*\*采访主题:\*\*\s*(.+?)(?:\n|$)/)
    if (topicMatch) result.topic = topicMatch[1].trim()
    
    // Estrai numero intervistati (es. "5 / 9 位模拟Agent")
    ...
    // Estrai motivazione della selezione degli intervistati
    const reasonMatch = text.match(/### 采访对象选择理由\n([\s\S]*?)(?=\n---\n|\n### 采访实录)/)
    if (reasonMatch) {
      result.selectionReason = reasonMatch[1].trim()
    }
    
    // Analizza la motivazione di selezione per ogni persona
    const parseIndividualReasons = (reasonText) => {
      ...
      for (const line of lines) {
        ...
        // Formato 1: numero. **nome（index=X）**：motivazione
        // Esempio: 1. **校友_345（index=1）**：作为武大校友...
        ...
        // Formato 2: - 选择nome（index X）：motivazione
        // Esempio: - 选择家长_601（index 0）：作为家长群体代表...
        if (!headerMatch) {
        ...
        // Formato 3: - **nome（index X）**：motivazione
        // Esempio: - **家长_601（index 0）**：作为家长群体代表...
        if (!headerMatch) {
        ...
        if (name) {
          // Salva motivazione della persona precedente
          if (currentName && currentReason.length > 0) {
            reasons[currentName] = currentReason.join(' ').trim()
          }
          // Inizia nuova persona
          currentName = name
          currentReason = reasonStart ? [reasonStart.trim()] : []
        } else if (currentName && line.trim() && !line.match(/^未选|^综上|^最终选择/)) {
          // Continuazione della motivazione (escludendo paragrafi riassuntivi finali)
          currentReason.push(line.trim())
        }
      }
      
      // Salva motivazione dell'ultima persona
      if (currentName && currentReason.length > 0) {
        reasons[currentName] = currentReason.join(' ').trim()
      }
    ...
    // Estrai ogni registrazione di intervista
    ...
    interviewBlocks.forEach((block, index) => {
      ...
      // Estrai titolo (es. "studente", "educatore", ecc.)
      ...
      if (nameRoleMatch) {
        ...
        // Imposta la motivazione di selezione per questa persona
      ...
      // Estrai lista domande
      const qMatch = block.match(/\*\*Q:\*\*\s*([\s\S]*?)(?=\n\n\*\*A:\*\*|\*\*A:\*\*)/)
      if (qMatch) {
        const qText = qMatch[1].trim()
        // Dividi domande per numerazione
        const questions = qText.split(/\n\d+\.\s+/).filter(q => q.trim())
        if (questions.length > 0) {
          // Se la prima domanda ha "1." davanti, necessita gestione speciale
      ...
      // Estrai risposte - divise per Twitter e Reddit
      const answerMatch = block.match(/\*\*A:\*\*\s*([\s\S]*?)(?=\*\*关键引言|$)/)
      if (answerMatch) {
        const answerText = answerMatch[1].trim()
        
        // Separa risposte Twitter e Reddit
        ...
        // Logica di fallback piattaforma (compatibile con formato precedente: solo un marcatore piattaforma)
        if (!twitterMatch && redditMatch) {
          // Solo risposta Reddit, copia come visualizzazione predefinita solo se non e testo segnaposto
          ...
          // Nessun marcatore piattaforma (formato molto vecchio), usa come risposta intera
          interview.twitterAnswer = answerText
        }
      }
      
      // Estrai citazioni chiave (compatibile con vari formati di virgolette)
      const quotesMatch = block.match(/\*\*关键引言:\*\*\n([\s\S]*?)(?=\n---|\n####|$)/)
      if (quotesMatch) {
        const quotesText = quotesMatch[1]
        // Dai priorita al formato > "text"
        let quoteMatches = quotesText.match(/> "([^"]+)"/g)
        // Fallback: corrisponde > "text" o > \u201Ctext\u201D (virgolette cinesi)
        if (!quoteMatches) {
...
const parseQuickSearch = (text) => {
  ...
  try {
    ...
    // Estrai conteggio risultati
    const countMatch = text.match(/找到\s*(\d+)\s*条/)
    if (countMatch) result.count = parseInt(countMatch[1])
    
    // Estrai fatti correlati - estrazione completa, senza limite di quantita
    ...
    // Tenta di estrarre informazioni sugli archi (se presenti)
    ...
    // Tenta di estrarre informazioni sui nodi (se presenti)
    ...
    return () => h('div', { class: 'insight-display' }, [
      ...
      h('div', { class: 'insight-header' }, [
        ...
        props.result.simulationRequirement && h('div', { class: 'header-scenario' }, [
          h('span', { class: 'scenario-label' }, 'Scenario di previsione: '),
      ...
      h('div', { class: 'insight-tabs' }, [
        h('button', {
          ...
          h('span', { class: 'tab-label' }, `Memorie chiave attuali (${props.result.facts.length})`)
        ]),
        h('button', {
          class: ['insight-tab', { active: activeTab.value === 'entities' }],
          onClick: () => { activeTab.value = 'entities' }
        }, [
          h('span', { class: 'tab-label' }, `Entita principali (${props.result.entities.length})`)
        ]),
        h('button', {
          class: ['insight-tab', { active: activeTab.value === 'relations' }],
          onClick: () => { activeTab.value = 'relations' }
        }, [
          h('span', { class: 'tab-label' }, `Catene di relazioni (${props.result.relations.length})`)
        ]),
        props.result.subQueries.length > 0 && h('button', {
          ...
          h('span', { class: 'tab-label' }, `Sotto-domande (${props.result.subQueries.length})`)
        ])
      ]),
      ...
      h('div', { class: 'insight-content' }, [
        ...
        activeTab.value === 'facts' && props.result.facts.length > 0 && h('div', { class: 'facts-panel' }, [
          h('div', { class: 'panel-header' }, [
            h('span', { class: 'panel-title' }, 'Fatti chiave piu recenti dalla memoria temporale'),
            h('span', { class: 'panel-count' }, `${props.result.facts.length} elementi`)
          ]),
          ...
          }, expandedFacts.value ? `Comprimi ▲` : `Mostra tutti ${props.result.facts.length} elementi ▼`)
        ]),
        
        // Entities Tab
        activeTab.value === 'entities' && props.result.entities.length > 0 && h('div', { class: 'entities-panel' }, [
          h('div', { class: 'panel-header' }, [
            h('span', { class: 'panel-title' }, 'Entita principali'),
            h('span', { class: 'panel-count' }, `${props.result.entities.length} elementi`)
          ]),
          h('div', { class: 'entities-grid' },
            (expandedEntities.value ? props.result.entities : props.result.entities.slice(0, 12)).map((entity, i) => 
              h('div', { class: 'entity-tag', key: i, title: entity.summary || '' }, [
                ...
                entity.relatedFactsCount > 0 && h('span', { class: 'entity-fact-count' }, `${entity.relatedFactsCount} fatti`)
              ])
            )
          ),
          ...
          }, expandedEntities.value ? `Comprimi ▲` : `Mostra tutte ${props.result.entities.length} entita ▼`)
        ]),
        
        // Relations Tab
        activeTab.value === 'relations' && props.result.relations.length > 0 && h('div', { class: 'relations-panel' }, [
          h('div', { class: 'panel-header' }, [
            h('span', { class: 'panel-title' }, 'Catene di relazioni'),
            h('span', { class: 'panel-count' }, `${props.result.relations.length} elementi`)
          ]),
          ...
          }, expandedRelations.value ? `Comprimi ▲` : `Mostra tutti ${props.result.relations.length} elementi ▼`)
        ]),
        
        // Sub-queries Tab
        activeTab.value === 'subqueries' && props.result.subQueries.length > 0 && h('div', { class: 'subqueries-panel' }, [
          h('div', { class: 'panel-header' }, [
            h('span', { class: 'panel-title' }, 'Sotto-domande generate da query a deriva'),
            h('span', { class: 'panel-count' }, `${props.result.subQueries.length} elementi`)
          ]),
        ...
        // Empty state
        activeTab.value === 'facts' && props.result.facts.length === 0 && h('div', { class: 'empty-state' }, 'Nessuna memoria chiave attuale'),
        activeTab.value === 'entities' && props.result.entities.length === 0 && h('div', { class: 'empty-state' }, 'Nessuna entita principale'),
        activeTab.value === 'relations' && props.result.relations.length === 0 && h('div', { class: 'empty-state' }, 'Nessuna catena di relazioni')
      ])
    ])
  }
}
    ...
    return () => h('div', { class: 'panorama-display' }, [
      ...
      h('div', { class: 'panorama-tabs' }, [
        h('button', {
          ...
          h('span', { class: 'tab-label' }, `Memorie valide attuali (${props.result.activeFacts.length})`)
        ]),
        h('button', {
          class: ['panorama-tab', { active: activeTab.value === 'historical' }],
          onClick: () => { activeTab.value = 'historical' }
        }, [
          h('span', { class: 'tab-label' }, `Memorie storiche (${props.result.historicalFacts.length})`)
        ]),
        h('button', {
          class: ['panorama-tab', { active: activeTab.value === 'entities' }],
          onClick: () => { activeTab.value = 'entities' }
        }, [
          h('span', { class: 'tab-label' }, `Entita coinvolte (${props.result.entities.length})`)
        ])
      ]),
      ...
      h('div', { class: 'panorama-content' }, [
        ...
        activeTab.value === 'active' && h('div', { class: 'facts-panel active-facts' }, [
          h('div', { class: 'panel-header' }, [
            h('span', { class: 'panel-title' }, 'Memorie valide attuali'),
            h('span', { class: 'panel-count' }, `${props.result.activeFacts.length} elementi`)
          ]),
          ...
          ) : h('div', { class: 'empty-state' }, 'Nessuna memoria valida attuale'),
          props.result.activeFacts.length > INITIAL_SHOW_COUNT && h('button', {
            class: 'expand-btn',
            onClick: () => { expandedActive.value = !expandedActive.value }
          }, expandedActive.value ? `Comprimi ▲` : `Mostra tutti ${props.result.activeFacts.length} elementi ▼`)
        ]),
        
        // Historical Facts Tab
        activeTab.value === 'historical' && h('div', { class: 'facts-panel historical-facts' }, [
          h('div', { class: 'panel-header' }, [
            h('span', { class: 'panel-title' }, 'Memorie storiche'),
            h('span', { class: 'panel-count' }, `${props.result.historicalFacts.length} elementi`)
          ]),
            ...
            (expandedHistorical.value ? props.result.historicalFacts : props.result.historicalFacts.slice(0, INITIAL_SHOW_COUNT)).map((fact, i) => 
              h('div', { class: 'fact-item historical', key: i }, [
                ...
                h('div', { class: 'fact-content' }, [
                  // Tenta di estrarre informazioni temporali [time - time]
                  (() => {
          ...
          ) : h('div', { class: 'empty-state' }, 'Nessuna memoria storica'),
          props.result.historicalFacts.length > INITIAL_SHOW_COUNT && h('button', {
            class: 'expand-btn',
            onClick: () => { expandedHistorical.value = !expandedHistorical.value }
          }, expandedHistorical.value ? `Comprimi ▲` : `Mostra tutti ${props.result.historicalFacts.length} elementi ▼`)
        ]),
        
        // Entities Tab
        activeTab.value === 'entities' && h('div', { class: 'entities-panel' }, [
          h('div', { class: 'panel-header' }, [
            h('span', { class: 'panel-title' }, 'Entita coinvolte'),
            h('span', { class: 'panel-count' }, `${props.result.entities.length} elementi`)
          ]),
          ...
          ) : h('div', { class: 'empty-state' }, 'Nessuna entita coinvolta'),
...
const InterviewDisplay = {
  ...
  setup(props) {
    ...
    // Mantieni stato di selezione piattaforma indipendente per ogni coppia domanda-risposta
    const platformTabs = reactive({}) // { 'agentIdx-qIdx': 'twitter' | 'reddit' }
    
    // Ottieni la selezione piattaforma corrente per una domanda
    const getPlatformTab = (agentIdx, qIdx) => {
      const key = `${agentIdx}-${qIdx}`
      return platformTabs[key] || 'twitter'
    }
    
    // Imposta la selezione piattaforma per una domanda
    ...
    // Verifica se e testo segnaposto piattaforma
    ...
    // Tenta di dividere le risposte per numero di domanda
    const splitAnswerByQuestions = (answerText, questionCount) => {
      ...
      // Supporta due formati di numerazione:
      // 1. "问题X：" o "问题X:" (formato cinese, nuovo formato backend)
      // 2. "1. " o "\n1. " (numero+punto, compatibilita formato precedente)
      let matches = []
      let match

      // Priorita al formato "问题X："
      const cnPattern = /(?:^|[\r\n]+)问题(\d+)[：:]\s*/g
      ...
      // Se nessuna corrispondenza, fallback al formato "numero."
      if (matches.length === 0) {
        const numPattern = /(?:^|[\r\n]+)(\d+)\.\s+/g
      ...
      // Se nessuna numerazione trovata o solo una, restituisci intero
      if (matches.length <= 1) {
    ...
    // Ottieni la risposta corrispondente a una domanda
    const getAnswerForQuestion = (interview, qIdx, platform) => {
      ...
      // Divisione riuscita e indice valido
      if (answers.length > 1 && qIdx < answers.length) {
        return answers[qIdx] || ''
      }

      // Divisione fallita: la prima domanda restituisce la risposta completa, le altre vuote
      return qIdx === 0 ? answer : ''
    }
    
    // Verifica se una domanda ha risposte su doppia piattaforma (filtrando testi segnaposto)
    const hasMultiplePlatforms = (interview, qIdx) => {
      ...
      // Entrambe le piattaforme hanno risposte reali (non segnaposto) e contenuto diverso
              ...
              answerText && h('div', { class: ['qa-answer', { 'answer-placeholder': isPlaceholder }] }, [
                ...
                h('div', { class: 'qa-content' }, [
                  h('div', { class: 'qa-answer-header' }, [
                    ...
                    // Pulsanti cambio piattaforma (mostrati solo con risposte reali su doppia piattaforma)
                  ...
                  // Pulsante Espandi/Comprimi (non mostrato per testi segnaposto)
    ...
    return () => h('div', { class: 'quick-search-display' }, [
      ...
      h('div', { class: 'quicksearch-header' }, [
        ...
        props.result.query && h('div', { class: 'header-query' }, [
          h('span', { class: 'query-label' }, 'Ricerca: '),
      ...
      h('div', { class: ['quicksearch-content', { 'no-tabs': !showTabs.value }] }, [
        ...
        ((!showTabs.value) || activeTab.value === 'facts') && h('div', { class: 'facts-panel' }, [
          !showTabs.value && h('div', { class: 'panel-header' }, [
            h('span', { class: 'panel-title' }, 'Risultati ricerca'),
            h('span', { class: 'panel-count' }, `${props.result.facts.length} elementi`)
          ]),
          ...
          ) : h('div', { class: 'empty-state' }, 'Nessun risultato trovato'),
          props.result.facts.length > INITIAL_SHOW_COUNT && h('button', {
            class: 'expand-btn',
            onClick: () => { expandedFacts.value = !expandedFacts.value }
          }, expandedFacts.value ? `Comprimi ▲` : `Mostra tutti ${props.result.facts.length} elementi ▼`)
        ...
        activeTab.value === 'edges' && hasEdges.value && h('div', { class: 'edges-panel' }, [
          h('div', { class: 'panel-header' }, [
            h('span', { class: 'panel-title' }, 'Relazioni correlate'),
            h('span', { class: 'panel-count' }, `${props.result.edges.length} elementi`)
          ]),
        ...
        activeTab.value === 'nodes' && hasNodes.value && h('div', { class: 'nodes-panel' }, [
          h('div', { class: 'panel-header' }, [
            h('span', { class: 'panel-title' }, 'Nodi correlati'),
            h('span', { class: 'panel-count' }, `${props.result.nodes.length} elementi`)
          ]),
...
// Passo attualmente attivo (per visualizzazione in alto)
const activeStep = computed(() => {
  const steps = workflowSteps.value
  // Trova il passo attualmente attivo
  const active = steps.find(s => s.status === 'active')
  if (active) return active
  
  // Se nessun passo attivo, restituisci l'ultimo completato
  const doneSteps = steps.filter(s => s.status === 'done')
  if (doneSteps.length > 0) return doneSteps[doneSteps.length - 1]
  
  // Altrimenti restituisci il primo passo
  return steps[0] || { noLabel: '--', title: 'In attesa di avvio', status: 'todo', meta: '' }
})
...
const renderMarkdown = (content) => {
  ...
  // Rimuovi titolo di secondo livello iniziale (## xxx), perche il titolo della sezione e gia mostrato esternamente
  let processedContent = content.replace(/^##\s+.+\n+/, '')
  
  // Elabora blocchi di codice
  let html = processedContent.replace(/```(\w*)\n([\s\S]*?)```/g, '<pre class="code-block"><code>$2</code></pre>')
  
  // Elabora codice inline
  ...
  // Elabora blocchi citazione
  html = html.replace(/^> (.+)$/gm, '<blockquote class="md-quote">$1</blockquote>')
  
  // Elabora liste - supporta sotto-liste
  ...
  // Avvolgi liste non ordinate
  html = html.replace(/(<li class="md-li"[^>]*>.*?<\/li>\s*)+/g, '<ul class="md-ul">$&</ul>')
  // Avvolgi liste ordinate
  html = html.replace(/(<li class="md-oli"[^>]*>.*?<\/li>\s*)+/g, '<ol class="md-ol">$&</ol>')

  // Pulisci tutti gli spazi tra gli elementi della lista
  html = html.replace(/<\/li>\s+<li/g, '</li><li')
  // Pulisci spazi dopo il tag di apertura lista
  html = html.replace(/<ul class="md-ul">\s+/g, '<ul class="md-ul">')
  html = html.replace(/<ol class="md-ol">\s+/g, '<ol class="md-ol">')
  // Pulisci spazi prima del tag di chiusura lista
  html = html.replace(/\s+<\/ul>/g, '</ul>')
  html = html.replace(/\s+<\/ol>/g, '</ol>')
  
  // Elabora grassetto e corsivo
  html = html.replace(/\*\*(.+?)\*\*/g, '<strong>$1</strong>')
  html = html.replace(/\*(.+?)\*/g, '<em>$1</em>')
  html = html.replace(/_(.+?)_/g, '<em>$1</em>')
  
  // Elabora linee separatrici
  html = html.replace(/^---$/gm, '<hr class="md-hr">')
  
  // Elabora a capo - righe vuote diventano separatori di paragrafo, singolo a capo diventa <br>
  html = html.replace(/\n\n/g, '</p><p class="md-p">')
  html = html.replace(/\n/g, '<br>')
  
  // Avvolgi in paragrafi
  ...
  // Pulisci tag <br> prima e dopo elementi a livello di blocco
  html = html.replace(/<br>\s*(<ul|<ol|<blockquote)/g, '$1')
  html = html.replace(/(<\/ul>|<\/ol>|<\/blockquote>)\s*<br>/g, '$1')
  // Pulisci <p><br> seguito da elementi a livello di blocco (causato da righe vuote extra)
  html = html.replace(/<p class="md-p">(<br>\s*)+(<ul|<ol|<blockquote|<pre|<hr)/g, '$2')
  // Pulisci tag <br> consecutivi
  html = html.replace(/(<br>\s*){2,}/g, '<br>')
  // Pulisci <br> tra elemento a livello di blocco e tag di apertura paragrafo successivo
  html = html.replace(/(<\/ol>|<\/ul>|<\/blockquote>)<br>(<p|<div)/g, '$1$2')

  // Correggi numerazione liste ordinate non consecutive: quando singoli <ol> sono separati da contenuto paragrafo, mantieni numerazione incrementale
...
const getLogLevelClass = (log) => {
  ...
  // INFO usa il colore predefinito, non contrassegnare come success
  return ''
}
    ...
    if (res.success && res.data) {
      ...
      if (newLogs.length > 0) {
        newLogs.forEach(log => {
          ...
          // section_complete - generazione sezione completata
          if (log.action === 'section_complete') {
            if (log.details?.content) {
              generatedSections.value[log.section_index] = log.details.content
              // Espandi automaticamente la sezione appena generata
          ...
          if (log.action === 'report_complete') {
            ...
            currentSectionIndex.value = null  // Assicurati di cancellare lo stato di caricamento
            emit('update-status', 'completed')
            stopPolling()
            // La logica di scorrimento viene gestita uniformemente nel nextTick dopo la fine del ciclo
          }
        ...
        nextTick(() => {
          if (rightPanel.value) {
            // Se il task e' completato, scorri in alto; altrimenti scorri in basso per seguire gli ultimi log
            if (isComplete.value) {
...
// Estrai il contenuto della risposta finale - estrae il contenuto della sezione dalla risposta LLM
const extractFinalContent = (response) => {
  if (!response) return null
  
  // Prova a estrarre il contenuto all'interno del tag <final_answer>
  ...
  // Prova a trovare il contenuto dopo Final Answer: (supporta diversi formati)
  // Formato 1: Final Answer:\n\ncontenuto
  // Formato 2: Final Answer: contenuto
  const finalAnswerMatch = response.match(/Final\s*Answer:\s*\n*([\s\S]*)$/i)
  if (finalAnswerMatch) {
    return finalAnswerMatch[1].trim()
  }
  
  // Prova a trovare il contenuto dopo "risposta finale:"
  const chineseFinalMatch = response.match(/最终答案[:：]\s*\n*([\s\S]*)$/i)
  if (chineseFinalMatch) {
    return chineseFinalMatch[1].trim()
  }
  
  // Se inizia con ## o # o >, potrebbe essere contenuto markdown diretto
  const trimmedResponse = response.trim()
  if (trimmedResponse.match(/^[#>]/)) {
    return trimmedResponse
  }
  
  // Se il contenuto e' lungo e contiene formattazione markdown, prova a rimuovere il processo di ragionamento e restituisci
  if (response.length > 300 && (response.includes('**') || response.includes('>'))) {
    // Rimuovi il processo di ragionamento che inizia con Thought:
...
.section-number {
  ...
  font-size: 16px;
  color: #9CA3AF; /* Grigio scuro, non cambia con lo stato */
  font-weight: 500;
}
...
/* Console Logs - Coerente con Step3Simulation.vue */
.console-logs {
  background: #000;
  color: #DDD;
  padding: 16px;

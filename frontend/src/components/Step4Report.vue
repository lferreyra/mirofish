            ...
            >
              ...
              <div class="timeline-content">
                ...
                <div class="timeline-body" :class="{ 'collapsed': isLogCollapsed(log) }" @click="toggleLogExpand(log)">
                  ...
                  <!-- Section Content Generated (content generation done, but full section may not be complete) -->
...
const toggleRawResult = (timestamp, event) => {
  ...
  // After DOM update, adjust scroll position to keep button at the same position
...
const parseInsightForge = (text) => {
  ...
  try {
    ...
    const factMatch = text.match(/Related Prediction Facts:\s*(\d+)/) || text.match(/相关预测事实:\s*(\d+)/)
    const entityMatch = text.match(/Entities Involved:\s*(\d+)/) || text.match(/涉及实体:\s*(\d+)/)
    const relMatch = text.match(/Relationship Chains:\s*(\d+)/) || text.match(/关系链:\s*(\d+)/)
    ...
    if (entitySection) {
      ...
      result.entities = entityBlocks.map(block => {
        ...
        const relatedMatch = block.match(/Related Facts:\s*(\d+)/) || block.match(/相关事实:\s*(\d+)/)
        return {
...
const parsePanorama = (text) => {
  ...
  try {
    ...
    // Extract statistics
    const nodesMatch = text.match(/Total Nodes:\s*(\d+)/) || text.match(/总节点数:\s*(\d+)/)
    const edgesMatch = text.match(/Total Edges:\s*(\d+)/) || text.match(/总边数:\s*(\d+)/)
    const activeMatch = text.match(/Current Active Facts:\s*(\d+)/) || text.match(/当前有效事实:\s*(\d+)/)
    const histMatch = text.match(/Historical\/Expired Facts:\s*(\d+)/) || text.match(/历史\/过期事实:\s*(\d+)/)
...
const parseInterview = (text) => {
  ...
  try {
    ...
    const countMatch = text.match(/\*\*Interviewees:\*\*\s*(\d+)\s*\/\s*(\d+)/) || text.match(/\*\*采访人数:\*\*\s*(\d+)\s*\/\s*(\d+)/)
    if (countMatch) {
    ...
    // Extract selection rationale for interviewees
    const reasonMatch = text.match(/### Selection Rationale\n([\s\S]*?)(?=\n---\n|\n### Interview Records)/) || text.match(/### 采访对象选择理由\n([\s\S]*?)(?=\n---\n|\n### 采访实录)/)
    if (reasonMatch) {
      result.selectionReason = reasonMatch[1].trim()
    }

    // Parse individual selection rationales
    const parseIndividualReasons = (reasonText) => {
      ...
      for (const line of lines) {
        ...
        // e.g.: - select parent_601（index 0）: as a representative of parent group...
        if (!headerMatch) {
        ...
        // e.g.: - **parent_601（index 0）**: as a representative of parent group...
        if (!headerMatch) {
        ...
        if (name) {
          ...
          // Continuation of reason (exclude trailing summary paragraphs)
          currentReason.push(line.trim())
        }
      }
      
      // Save reason for the last person
      if (currentName && currentReason.length > 0) {
        reasons[currentName] = currentReason.join(' ').trim()
      }
    ...
    interviewBlocks.forEach((block, index) => {
      ...
      if (qMatch) {
        ...
        // Split questions by numeric index
      ...
      if (answerMatch) {
        ...
        // Platform fallback logic (compatibility with old format: only one platform marker)
        if (!twitterMatch && redditMatch) {
          ...
          // No platform markers (very old format), use whole answer
...
const parseQuickSearch = (text) => {
  ...
  try {
    ...
    const countMatch = text.match(/Found\s*(\d+)\s*results/) || text.match(/找到\s*(\d+)\s*条/)
  ...
  setup(props) {
    ...
    return () => h('div', { class: 'insight-display' }, [
      ...
      h('div', { class: 'insight-content' }, [
        ...
        // Empty state
        activeTab.value === 'facts' && props.result.facts.length === 0 && h('div', { class: 'empty-state' }, 'No key memories available'),
        activeTab.value === 'entities' && props.result.entities.length === 0 && h('div', { class: 'empty-state' }, 'No core entities available'),
        activeTab.value === 'relations' && props.result.relations.length === 0 && h('div', { class: 'empty-state' }, 'No relationship chains available')
      ])
    ])
  }
}
...
const InterviewDisplay = {
  ...
  setup(props) {
    ...
    // Attempt to split answer by question number
    const splitAnswerByQuestions = (answerText, questionCount) => {
      ...
      // 2. "1. " or "\n1. " (number+dot, legacy format compatibility)
      let matches = []
      let match
      ...
      // If no numbering found or only one match, return whole answer
      if (matches.length <= 1) {
...
const renderMarkdown = (content) => {
  ...
  // Remove leading level-2 headings (## xxx) since section titles are displayed in outer container
  ...
  // Clean <br> before paragraph opening tags following block elements
  html = html.replace(/(<\/ol>|<\/ul>|<\/blockquote>)<br>(<p|<div)/g, '$1$2')

  // Fix numbering for non-consecutive ordered lists: when single-item <ol> is separated by paragraph content, keep numbering incrementing
      ...
      if (newLogs.length > 0) {
        newLogs.forEach(log => {
          ...
          if (log.action === 'report_complete') {
            ...
            stopPolling()
            // Scroll logic is handled in nextTick after the loop ends
          }
        ...
        nextTick(() => {
          if (rightPanel.value) {
            // If task is complete, scroll to top; otherwise scroll to bottom to follow latest log
            if (isComplete.value) {
...
const extractFinalContent = (response) => {
  ...
  // If content is long and contains markdown formatting, try removing thinking process first

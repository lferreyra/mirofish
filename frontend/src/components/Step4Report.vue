...
const toggleRawResult = (timestamp, event) => {
  ...
  // After DOM update, adjust scroll position to keep button in same place
...
const parseInsightForge = (text) => {
  ...
  try {
    ...
    const subQSection = text.match(/### Analysis Sub-queries\n([\s\S]*?)(?=\n###|$)/)
    if (subQSection) {
...
const parseInterview = (text) => {
  ...
  try {
    ...
    const reasonMatch = text.match(/### Interview Subject Selection Reason\n([\s\S]*?)(?=\n---\n|\n### Interview Records)/)
    if (reasonMatch) {
    ...
    const parseIndividualReasons = (reasonText) => {
      ...
      for (const line of lines) {
        ...
        if (name) {
          ...
          // Continuation of reason (excluding summary paragraphs)
    ...
    interviewBlocks.forEach((block, index) => {
      ...
      if (answerMatch) {
        ...
        // Platform fallback logic (backward compat: only one platform marker)
        if (!twitterMatch && redditMatch) {
          // Only Reddit answer, copy as default only when not placeholder
          if (interview.redditAnswer && interview.redditAnswer !== '(No reply received on this platform)') {
            interview.twitterAnswer = interview.redditAnswer
          }
        } else if (twitterMatch && !redditMatch) {
          if (interview.twitterAnswer && interview.twitterAnswer !== '(No reply received on this platform)') {
      ...
      if (quotesMatch) {
        ...
        // Fallback: match > "text" or > \u201Ctext\u201D (CJK quotes)
        if (!quoteMatches) {
    ...
    return () => h('div', { class: 'insight-display' }, [
      ...
      h('div', { class: 'insight-content' }, [
        ...
        activeTab.value === 'subqueries' && props.result.subQueries.length > 0 && h('div', { class: 'subqueries-panel' }, [
          h('div', { class: 'panel-header' }, [
            h('span', { class: 'panel-title' }, 'Drift query generated analysis sub-queries'),
...
const InterviewDisplay = {
  ...
  setup(props) {
    ...
    const isPlaceholderText = (text) => {
      if (!text) return true
      const t = text.trim()
      return t === '(No reply received on this platform)' || t === '(No reply on this platform)' || t === '[No reply]'
    }
    ...
    const splitAnswerByQuestions = (answerText, questionCount) => {
      ...
      // 1. "Question X:" (Chinese format, new backend format)
    ...
    const getAnswerForQuestion = (interview, qIdx, platform) => {
      ...
      // Split failed: first question gets full answer, rest get empty
            ...
            return h('div', { class: 'qa-pair', key: qIdx }, [
              ...
              answerText && h('div', { class: ['qa-answer', { 'answer-placeholder': isPlaceholder }] }, [
                ...
                h('div', { class: 'qa-content' }, [
                  ...
                  // Expand/Collapse Button (hidden for placeholders)
...
const renderMarkdown = (content) => {
  ...
  // Fix non-contiguous ordered list numbering: when single-item <ol> are separated by paragraph content, keep numbering incremental
...
const extractFinalContent = (response) => {
  ...
  // Try to find content after final answer marker
  ...
  // If content is long and contains markdown, try removing thought process before returning

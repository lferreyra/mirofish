            ...
            >
              ...
              <div class="timeline-content">
                ...
                <div class="timeline-body" :class="{ 'collapsed': isLogCollapsed(log) }" @click="toggleLogExpand(log)">
                  ...
                  <!-- Section Content Generated (content done but section may not be finalized) -->
...
const toolConfig = {
  ...
  'interview_agents': {
    ...
    color: 'green',
    icon: 'users' // Users icon - represents conversations
  },
  'quick_search': {
    name: 'Quick Search',
    color: 'orange',
...
const parseInsightForge = (text) => {
  ...
  try {
    ...
    // Extract core entities - full extraction with summaries and related fact counts
    ...
    return () => h('div', { class: 'insight-display' }, [
      ...
      h('div', { class: 'insight-content' }, [
        ...
        activeTab.value === 'facts' && props.result.facts.length > 0 && h('div', { class: 'facts-panel' }, [
          ...
          }, expandedFacts.value ? `Collapse ▲` : `Show all ${props.result.facts.length} items ▼`)
        ]),
        ...
        activeTab.value === 'entities' && props.result.entities.length > 0 && h('div', { class: 'entities-panel' }, [
          ...
          h('div', { class: 'entities-grid' },
            (expandedEntities.value ? props.result.entities : props.result.entities.slice(0, 12)).map((entity, i) => 
              h('div', { class: 'entity-tag', key: i, title: entity.summary || '' }, [
                ...
                entity.relatedFactsCount > 0 && h('span', { class: 'entity-fact-count' }, `${entity.relatedFactsCount} results`)
              ])
            )
          ),
          ...
          }, expandedEntities.value ? `Collapse ▲` : `Show all ${props.result.entities.length} items ▼`)
        ]),
        ...
        activeTab.value === 'relations' && props.result.relations.length > 0 && h('div', { class: 'relations-panel' }, [
          ...
          }, expandedRelations.value ? `Collapse ▲` : `Show all ${props.result.relations.length} items ▼`)
        ]),
        ...
        activeTab.value === 'entities' && props.result.entities.length === 0 && h('div', { class: 'empty-state' }, 'No re Entities'),
    ...
    return () => h('div', { class: 'panorama-display' }, [
      ...
      h('div', { class: 'panorama-content' }, [
        ...
        activeTab.value === 'active' && h('div', { class: 'facts-panel active-facts' }, [
          ...
          }, expandedActive.value ? `Collapse ▲` : `Show all ${props.result.activeFacts.length} items ▼`)
        ]),
        ...
        activeTab.value === 'historical' && h('div', { class: 'facts-panel historical-facts' }, [
          ...
          }, expandedHistorical.value ? `Collapse ▲` : `Show all ${props.result.historicalFacts.length} items ▼`)
        ]),
...
const InterviewDisplay = {
  ...
  setup(props) {
    ...
    const splitAnswerByQuestions = (answerText, questionCount) => {
      ...
      // If no numbers found or only one, return as whole
      if (matches.length <= 1) {
    ...
    const getAnswerForQuestion = (interview, qIdx, platform) => {
      ...
      // Split failed: first question returns full answer, rest empty
              ...
              answerText && h('div', { class: ['qa-answer', { 'answer-placeholder': isPlaceholder }] }, [
                ...
                h('div', { class: 'qa-content' }, [
                  h('div', { class: 'qa-answer-header' }, [
                    ...
                    // Dual-platform toggle buttons (shown only with real dual-platform answers)
...
// Currently active step (for header display)
...
const renderMarkdown = (content) => {
  ...
  // Remove leading H2 title (## xxx) since section title is shown in outer wrapper
  ...
  // Process line breaks - blank lines become paragraph breaks, single newlines become <br>
  html = html.replace(/\n\n/g, '</p><p class="md-p">')
  html = html.replace(/\n/g, '<br>')
  ...
  // Clean <p><br> immediately following block elements (from extra blank lines)
  ...
  // Fix non-consecutive ordered list numbering: maintain incrementing numbers when single-item <ol> lists are separated by paragraph content
      ...
      if (newLogs.length > 0) {
        newLogs.forEach(log => {
          ...
          if (log.action === 'report_complete') {
            ...
            stopPolling()
            // Scroll logic handled in nextTick after loop ends
          }
        ...
        nextTick(() => {
          if (rightPanel.value) {
            // If task completed, scroll to top; otherwise scroll to bottom following latest logs
            if (isComplete.value) {
...
const extractFinalContent = (response) => {
  ...
  // Try to find content after Final Answer: (multiple formats supported)
  ...
  // If content is long and contains markdown formatting, try removing thought process
  if (response.length > 300 && (response.includes('**') || response.includes('>'))) {
    // Remove Thought: prefix thought process

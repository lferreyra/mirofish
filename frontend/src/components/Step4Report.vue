...
const noResponsePlaceholders = new Set([
  '（该平台未获得回复）',
  '(该平台未获得回复)',
  '[无回复]',
...
const parseInsightForge = (text) => {
  ...
  try {
    ...
    const factMatch = text.match(/(?:Relevant Forecast Facts|相关预测事实)[:：]\s*(\d+)/)
    ...
    // 提取子问题 - 完整提取，不限制数量
    const subQSection = text.match(/### (?:Analysis Sub-questions|分析的子问题)\n([\s\S]*?)(?=\n###|$)/)
    if (subQSection) {
    ...
    // 提取关键事实 - 完整提取，不限制数量
    const factsSection = text.match(/### (?:Key Facts|\[Key Facts\]|【关键事实】)[\s\S]*?\n([\s\S]*?)(?=\n###|$)/)
    if (factsSection) {
    ...
    const entitySection = text.match(/### (?:Core Entities|\[Core Entities\]|【核心实体】)\n([\s\S]*?)(?=\n###|$)/)
    if (entitySection) {
    ...
    // 提取关系链 - 完整提取，不限制数量
    const relSection = text.match(/### (?:Relationship Chains|\[Relationship Chains\]|【关系链】)\n([\s\S]*?)(?=\n###|$)/)
    if (relSection) {
...
const parsePanorama = (text) => {
  ...
  try {
    ...
    const histMatch = text.match(/(?:Historical\s*\/\s*Expired Facts|历史\/过期事实)[:：]\s*(\d+)/)
    ...
    // 提取当前有效事实 - 完整提取，不限制数量
    const activeSection = text.match(/### (?:Active Facts|\[Active Facts\]|【当前有效事实】)[\s\S]*?\n([\s\S]*?)(?=\n###|$)/)
    if (activeSection) {
    ...
    const histSection = text.match(/### (?:Historical\s*\/\s*Expired Facts|\[Historical\s*\/\s*Expired Facts\]|【历史\/过期事实】)[\s\S]*?\n([\s\S]*?)(?=\n###|$)/)
    if (histSection) {
    ...
    // 提取涉及实体 - 完整提取，不限制数量
    const entitySection = text.match(/### (?:Entities Involved|\[Entities Involved\]|【涉及实体】)\n([\s\S]*?)(?=\n###|$)/)
    if (entitySection) {
...
const parseInterview = (text) => {
  ...
  try {
    ...
    const countMatch = text.match(/\*\*(?:Interview Count|采访人数)[:：]\*\*\s*(\d+)\s*\/\s*(\d+)/)
    if (countMatch) {
    ...
    // 提取采访对象选择理由
    const reasonMatch = text.match(/### (?:Why These Agents Were Selected|采访对象选择理由)\n([\s\S]*?)(?=\n---\n|\n### (?:Interview Transcript|采访实录))/)
    if (reasonMatch) {
    ...
    const parseIndividualReasons = (reasonText) => {
      ...
      for (const line of lines) {
        ...
        if (!headerMatch) {
          headerMatch = line.match(/^-\s*(?:Select\s+|选择)([^（(]+)(?:[（(]index\s*=?\s*\d+[)）])?[：:]\s*(.*)/)
          if (headerMatch) {
            name = headerMatch[1].trim()
            reasonStart = headerMatch[2]
          }
        ...
        } else if (currentName && line.trim() && !line.match(/^(?:未选|综上|最终选择|Not selected|Overall|Final selection)/)) {
          // 理由的续行（排除结尾总结段落）
    ...
    interviewBlocks.forEach((block, index) => {
      ...
      if (answerMatch) {
        ...
        if (!twitterMatch && redditMatch) {
          ...
          if (interview.redditAnswer && !noResponsePlaceholders.has(interview.redditAnswer.trim())) {
            interview.twitterAnswer = interview.redditAnswer
          }
        } else if (twitterMatch && !redditMatch) {
          if (interview.twitterAnswer && !noResponsePlaceholders.has(interview.twitterAnswer.trim())) {
    ...
    // 提取采访摘要
    const summaryMatch = text.match(/### (?:Interview Summary & Key Takeaways|采访摘要与核心观点)\n([\s\S]*?)$/)
    if (summaryMatch) {
...
const parseQuickSearch = (text) => {
  ...
  try {
    ...
    // 提取结果数量
    const countMatch = text.match(/(?:Found|找到)\s*(\d+)\b/i)
    ...
    return () => h('div', { class: 'insight-display' }, [
      ...
      h('div', { class: 'insight-tabs' }, [
        h('button', {
          ...
          h('span', { class: 'tab-label' }, `Current Facts (${props.result.facts.length})`)
        ]),
        h('button', {
      ...
      h('div', { class: 'insight-content' }, [
        ...
        activeTab.value === 'facts' && props.result.facts.length > 0 && h('div', { class: 'facts-panel' }, [
          h('div', { class: 'panel-header' }, [
            h('span', { class: 'panel-title' }, 'Latest key facts tied to this temporal memory'),
        ...
        activeTab.value === 'subqueries' && props.result.subQueries.length > 0 && h('div', { class: 'subqueries-panel' }, [
          h('div', { class: 'panel-header' }, [
            h('span', { class: 'panel-title' }, 'Generated Analysis Sub-questions'),
        ...
        // Empty state
        activeTab.value === 'facts' && props.result.facts.length === 0 && h('div', { class: 'empty-state' }, 'No current facts available'),
...
const InterviewDisplay = {
  ...
  setup(props) {
    ...
    const isPlaceholderText = (text) => {
      if (!text) return true
      return noResponsePlaceholders.has(text.trim())
    }

            ...
            >
              ...
              <div class="section-body" v-show="!collapsedSections.has(idx)">
                ...
                <div v-else-if="currentSectionIndex === idx + 1" class="loading-state">
                  ...
                  <span class="loading-text">{{ t('step4.generatingSection') }} {{ section.title }}...</span>
                </div>
              </div>
            </div>
          </div>
              ...
              <div class="timeline-content">
                ...
                <div class="timeline-body" :class="{ 'collapsed': isLogCollapsed(log) }" @click="toggleLogExpand(log)">
                  ...
                  <template v-if="log.action === 'llm_response'">
                    ...
                    <!-- Show special hint when final answer is given -->
    ...
    interviewBlocks.forEach((block, index) => {
      ...
      if (answerMatch) {
        ...
        if (!twitterMatch && redditMatch) {
          // Only Reddit answer, copy as default display when not placeholder
  ...
  setup(props) {
    ...
    return () => h('div', { class: 'insight-display' }, [
      ...
      h('div', { class: 'insight-content' }, [
        ...
        activeTab.value === 'relations' && props.result.relations.length === 0 && h('div', { class: 'empty-state' }, 'No relations yet')
      ])
    ])
  }
}
    ...
    return () => h('div', { class: 'panorama-display' }, [
      ...
      h('div', { class: 'panorama-content' }, [
        ...
        activeTab.value === 'active' && h('div', { class: 'facts-panel active-facts' }, [
          ...
          ) : h('div', { class: 'empty-state' }, 'No active memories yet'),
        ...
        activeTab.value === 'historical' && h('div', { class: 'facts-panel historical-facts' }, [
          ...
          ) : h('div', { class: 'empty-state' }, 'No historical memories yet'),
        ...
        activeTab.value === 'entities' && h('div', { class: 'entities-panel' }, [
          ...
          ) : h('div', { class: 'empty-state' }, 'No involved entities yet'),
...
const InterviewDisplay = {
  ...
  setup(props) {
    ...
    const splitAnswerByQuestions = (answerText, questionCount) => {
      ...
      // 1. "问题X:" (Chinese format, backend new format)
    ...
    // Check if a question has dual-platform answers (filtering placeholders)
...
const renderMarkdown = (content) => {
  ...
  // Remove leading H2 heading (## xxx), since section title is already displayed outside
  ...
  // Clean <br> tags before/after block-level elements
  ...
  // Clean <br> between block-level element end and following paragraph start
  html = html.replace(/(<\/ol>|<\/ul>|<\/blockquote>)<br>(<p|<div)/g, '$1$2')

  // Fix non-consecutive ordered list numbering: when single-item <ol> are separated by paragraph content, keep numbering incrementing
...
// Extract final answer content - from LLM response extract section content
const extractFinalContent = (response) => {
  ...
  // If content is long and contains markdown format, try removing thinking process

    ...
    <div class="scroll-container">
      ...
      <div class="step-card" :class="{ 'active': phase === 1, 'completed': phase > 1 }">
        ...
        <div class="card-content">
          ...
          <p class="description">
            Combining context, auto-invoke tools to organize entities and relations from knowledge graph, initialize simulation individuals, and assign unique behaviors and memories based on reality seeds
          </p>
          ...
          <div v-if="profiles.length > 0" class="stats-grid">
            ...
            <div class="stat-card">
              ...
              <span class="stat-label">Associated Topics</span>
            </div>
          </div>
      ...
      <div class="step-card" :class="{ 'active': phase === 2, 'completed': phase > 2 }">
        ...
        <div class="card-content">
          <p class="api-note">POST /api/simulation/prepare</p>
          <p class="description">
            LLM intelligently sets world time flow, recommendation algorithm, individual active hours, posting frequency, event triggers, and other parameters based on simulation needs and reality seeds
          </p>
      ...
      <div class="step-card" :class="{ 'active': phase === 3, 'completed': phase > 3 }">
        ...
        <div class="card-content">
          <p class="api-note">POST /api/simulation/prepare</p>
          <p class="description">
            Based on narrative direction, auto-generate initial activation events and trending topics to guide the initial state of the simulated world
          </p>
      ...
      <div class="step-card" :class="{ 'active': phase === 4 }">
        ...
        <div class="card-content">
          ...
          <!-- Simulation rounds config - shown only after config generation and rounds calculation -->
          <div v-if="simulationConfig && autoGeneratedRounds" class="rounds-config-section">
            <div class="rounds-header">
              <div class="header-left">
                <span class="section-title">Simulation Rounds Setting</span>
                <span class="section-desc">MiroFish auto-plans to simulate <span class="desc-highlight">{{ simulationConfig?.time_config?.total_simulation_hours || '-' }}</span> hours, each round represents <span class="desc-highlight">{{ simulationConfig?.time_config?.minutes_per_round || '-' }}</span> minutes of elapsed time</span>
              </div>
              ...
              <div v-else class="rounds-content auto" key="auto">
                <div class="auto-info-card">
                  ...
                  <div class="auto-content">
                    ...
                    <div class="auto-desc">
                      <p class="highlight-tip" @click="useCustomRounds = true">For first-time runs, strongly recommend switching to Custom mode to reduce rounds for a quick preview and lower error risk"action-group dual">
            <button 
...
watch(currentStage, (newStage) => {
  if (newStage === 'generating_profiles_cn' || newStage === 'generating_profiles') {
    phase.value = 1
  } else if (newStage === 'generating_config_cn' || newStage === 'generating_config') {
    phase.value = 2
    // Entering config generation phase, start config polling
    if (!configTimer) {
      addLog('Starting dual-platform simulation config generation...')
      startConfigPolling()
    }
  } else if (newStage === 'copying_scripts_cn' || newStage === 'copying_scripts') {
...
const autoGeneratedRounds = computed(() => {
  ...
  // Ensure max rounds >= 40 (recommended), avoid slider range issues
...
// Calculate total associated topics across all personas
  ...
  try {
    ...
    if (res.success && res.data) {
      if (res.data.already_prepared) {
        addLog('Detected existing preparation, reusing it')
      ...
      // Set expected total immediately (from prepare API response)
      ...
      startPolling()
      // Start realtime profiles polling
  ...
  try {
    ...
    if (res.success && res.data) {
      ...
      if (data.status === 'completed' || data.status === 'ready' || data.already_prepared) {
        addLog('Done: Preparation complete')
        stopPolling()
        stopProfilesPolling()
        await loadPreparedData()
      } else if (data.status === 'failed') {
        addLog(`Failed: Preparation failed: ${data.error || 'Unknown error'}`)
        stopPolling()
        stopProfilesPolling()
      }
    }
    ...
    if (res.success && res.data) {
      ...
      if (currentCount > 0 && currentCount !== lastLoggedProfileCount) {
        ...
        if (expectedTotal.value && currentCount >= expectedTotal.value) {
          addLog(`Done: All ${currentCount} Agent personas generated`)
        }
      }
    }
  } catch (err) {
  ...
  try {
    ...
    if (res.success && res.data) {
      ...
      if (data.config_generated && data.config) {
        ...
        addLog('Done: Simulation config generated')
        ...
        if (data.summary) {
          ...
          addLog(`  Platform config: Twitter ${data.summary.has_twitter_config ? 'Y' : 'N'}, Reddit ${data.summary.has_reddit_config ? '✓' : '✗'}`)
        }
        
        // Show Time Config details
        if (data.config.time_config) {
          const tc = data.config.time_config
          addLog(`Time Config: ${tc.minutes_per_round} min/round, ${Math.floor((tc.total_simulation_hours * 60) / tc.minutes_per_round)}rounds`)
        }
        ...
        stopConfigPolling()
        phase.value = 4
        addLog('Done: Environment setup complete, ready to simulate')
  ...
  try {
    ...
    if (res.success && res.data) {
      if (res.data.config_generated && res.data.config) {
        ...
        addLog('Done: Simulation config loaded')

    ...
    <div class="scroll-container">
      ...
      <div class="step-card" :class="{ 'active': phase === 1, 'completed': phase > 1 }">
        ...
        <div class="card-content">
          ...
          <p class="description">
            Automatically invokes tools to extract entities and relationships from the knowledge graph, initializes simulation individuals, and assigns them unique behaviors and memories based on real-world seeds
          </p>
          ...
          <div v-if="profiles.length > 0" class="stats-grid">
            ...
            <div class="stat-card">
              ...
              <span class="stat-label">Real-world Seed Topics</span>
            </div>
          </div>
      ...
      <div class="step-card" :class="{ 'active': phase === 3, 'completed': phase > 3 }">
        ...
        <div class="card-content">
          <p class="api-note">POST /api/simulation/prepare</p>
          <p class="description">
            Automatically generates initial activation events and trending topics based on narrative direction, guiding the initial state of the simulation world
          </p>
      ...
      <div class="step-card" :class="{ ‘active’: phase === 4 }">
        ...
        <div class="card-content">
          ...
          <!-- Simulation Rounds Config - only shown after config generation and rounds are calculated -->
          <div v-if="simulationConfig && autoGeneratedRounds" class="rounds-config-section">
            <div class="rounds-header">
              <div class="header-left">
                <span class="section-title">Simulation Rounds Setting</span>
                <span class="section-desc">MiroFish auto-plans to simulate <span class="desc-highlight">{{ simulationConfig?.time_config?.total_simulation_hours || ‘-’ }}</span> real-world hours, each round represents <span class="desc-highlight">{{ simulationConfig?.time_config?.minutes_per_round || ‘-’ }}</span> minutes of real-world time</span>
              </div>
              ...
              <div v-else class="rounds-content auto" key="auto">
                <div class="auto-info-card">
                  ...
                  <div class="auto-content">
                    ...
                    <div class="auto-desc">
                      <p class="highlight-tip" @click="useCustomRounds = true">For first-time runs, it is strongly recommended to switch to ‘Custom Mode’ to reduce rounds for a quick preview and lower error risk ➝</p>
                    </div>
                  </div>
                </div>
              </div>
      ...
      <div v-if="selectedProfile" class="profile-modal-overlay" @click.self="selectedProfile = null">
        ...
        <div class="modal-body">
          ...
          <div class="modal-section">
            <span class="section-label">Character Bio</span>
            <p class="section-bio">{{ selectedProfile.bio || 'No bio available' }}</p>
          </div>

          <!-- Interested Topics -->
          <div class="modal-section" v-if="selectedProfile.interested_topics?.length">
            <span class="section-label">Real-world Seed Associated Topics</span>
...
const useCustomRounds = ref(false) // default: use auto-generated rounds
const customMaxRounds = ref(40)   // default recommended: 40 rounds

// Watch stage to update phase
watch(currentStage, (newStage) => {
  if (newStage === 'generating_agent_profiles' || newStage === 'generating_profiles') {
    phase.value = 1
  ...
  } else if (newStage === 'preparing_simulation_script' || newStage === 'copying_scripts') {
...
const autoGeneratedRounds = computed(() => {
  ...
  // Ensure max rounds is not less than 40 (recommended), to avoid abnormal slider range
...
// Calculate total associated topics across all profiles
  ...
  try {
    ...
    if (res.success && res.data) {
      if (res.data.already_prepared) {
        addLog('Existing preparation detected, using directly')

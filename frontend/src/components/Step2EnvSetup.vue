    ...
    <div class="scroll-container">
      ...
      <div class="step-card" :class="{ 'active': phase === 0, 'completed': phase > 0 }">
        <div class="card-header">
          <div class="step-info">
            ...
            <span class="step-title">シミュレーションインスタンス初期化</span>
          </div>
          <div class="step-status">
            <span v-if="phase > 0" class="badge success">完了</span>
            <span v-else class="badge processing">初期化</span>
          </div>
        </div>
        ...
        <div class="card-content">
          ...
          <p class="description">
            シミュレーションインスタンスを新規作成し、シミュレーションパラメータテンプレートを取得
          </p>
          ...
          <div v-if="simulationId" class="info-card">
            ...
            <div class="info-row">
              ...
              <span class="info-value mono">{{ taskId || '非同期タスク完了' }}</span>
            </div>
          </div>
        </div>
      </div>
      ...
      <div class="step-card" :class="{ 'active': phase === 1, 'completed': phase > 1 }">
        <div class="card-header">
          <div class="step-info">
            ...
            <span class="step-title">エージェントペルソナ生成</span>
          </div>
          <div class="step-status">
            <span v-if="phase > 1" class="badge success">完了</span>
        ...
        <div class="card-content">
          ...
          <p class="description">
            コンテキストを組み合わせ、ツールを自動呼び出しして知識グラフからエンティティと関係を整理し、シミュレーション個体を初期化、現実シードを基に独自の行動と記憶を付与します
          </p>
          ...
          <div v-if="profiles.length > 0" class="stats-grid">
            <div class="stat-card">
              ...
              <span class="stat-label">現在のエージェント数</span>
            </div>
            <div class="stat-card">
              <span class="stat-value">{{ expectedTotal || '-' }}</span>
              <span class="stat-label">予定エージェント総数</span>
            </div>
            <div class="stat-card">
              <span class="stat-value">{{ totalTopicsCount }}</span>
              <span class="stat-label">現実シード関連トピック数</span>
            </div>
          </div>
          ...
          <div v-if="profiles.length > 0" class="profiles-preview">
            <div class="preview-header">
              <span class="preview-title">生成済みエージェントペルソナ</span>
            </div>
            <div class="profiles-list">
              ...
              >
                ...
                <div class="profile-meta">
                  <span class="profile-profession">{{ profile.profession || '職業不明' }}</span>
                </div>
                <p class="profile-bio">{{ profile.bio || '自己紹介なし' }}</p>
      ...
      <div class="step-card" :class="{ 'active': phase === 2, 'completed': phase > 2 }">
        <div class="card-header">
          <div class="step-info">
            ...
            <span class="step-title">デュアルプラットフォーム設定生成</span>
          </div>
          <div class="step-status">
            <span v-if="phase > 2" class="badge success">完了</span>
        ...
        <div class="card-content">
          <p class="api-note">POST /api/simulation/prepare</p>
          <p class="description">
            LLMがシミュレーション要件と現実シードに基づき、世界の時間速度・推薦アルゴリズム・各個体のアクティブ時間帯・発言頻度・イベントトリガーなどのパラメータをインテリジェントに設定します
          </p>
          ...
          <div v-if="simulationConfig" class="config-detail-panel">
            ...
            <div class="config-block">
              <div class="config-grid">
                <div class="config-item">
                  <span class="config-item-label">シミュレーション時間</span>
                  <span class="config-item-value">{{ simulationConfig.time_config?.total_simulation_hours || '-' }} 時間</span>
                </div>
                <div class="config-item">
                  <span class="config-item-label">ラウンド時間</span>
                  <span class="config-item-value">{{ simulationConfig.time_config?.minutes_per_round || '-' }} 分</span>
                </div>
                <div class="config-item">
                  <span class="config-item-label">総ラウンド数</span>
                  <span class="config-item-value">{{ Math.floor((simulationConfig.time_config?.total_simulation_hours * 60 / simulationConfig.time_config?.minutes_per_round)) || '-' }} ラウンド</span>
                </div>
                <div class="config-item">
                  <span class="config-item-label">毎時アクティブ数</span>
                  <span class="config-item-value">{{ simulationConfig.time_config?.agents_per_hour_min }}-{{ simulationConfig.time_config?.agents_per_hour_max }}</span>
                </div>
              </div>
              <div class="time-periods">
                <div class="period-item">
                  <span class="period-label">ピーク時間帯</span>
                  <span class="period-hours">{{ simulationConfig.time_config?.peak_hours?.join(':00, ') }}:00</span>
                  <span class="period-multiplier">×{{ simulationConfig.time_config?.peak_activity_multiplier }}</span>
                </div>
                <div class="period-item">
                  <span class="period-label">業務時間帯</span>
                  <span class="period-hours">{{ simulationConfig.time_config?.work_hours?.[0] }}:00-{{ simulationConfig.time_config?.work_hours?.slice(-1)[0] }}:00</span>
                  <span class="period-multiplier">×{{ simulationConfig.time_config?.work_activity_multiplier }}</span>
                </div>
                <div class="period-item">
                  <span class="period-label">朝の時間帯</span>
                  <span class="period-hours">{{ simulationConfig.time_config?.morning_hours?.[0] }}:00-{{ simulationConfig.time_config?.morning_hours?.slice(-1)[0] }}:00</span>
                  <span class="period-multiplier">×{{ simulationConfig.time_config?.morning_activity_multiplier }}</span>
                </div>
                <div class="period-item">
                  <span class="period-label">オフピーク時間帯</span>
              ...
              <div class="agents-cards">
                ...
                >
                  ...
                  <div class="agent-timeline">
                    <span class="timeline-label">アクティブ時間帯</span>
                  ...
                  <div class="agent-params">
                    <div class="param-group">
                      <div class="param-item">
                        <span class="param-label">投稿/時</span>
                        <span class="param-value">{{ agent.posts_per_hour }}</span>
                      </div>
                      <div class="param-item">
                        <span class="param-label">コメント/時</span>
                        <span class="param-value">{{ agent.comments_per_hour }}</span>
                      </div>
                      <div class="param-item">
                        <span class="param-label">応答遅延</span>
                        <span class="param-value">{{ agent.response_delay_min }}-{{ agent.response_delay_max }}min</span>
                      </div>
                    </div>
                    <div class="param-group">
                      <div class="param-item">
                        <span class="param-label">アクティブ度</span>
                      ...
                      <div class="param-item">
                        <span class="param-label">感情傾向</span>
                        <span class="param-value" :class="agent.sentiment_bias > 0 ? 'positive' : agent.sentiment_bias < 0 ? 'negative' : 'neutral'">
                          {{ agent.sentiment_bias > 0 ? '+' : '' }}{{ agent.sentiment_bias?.toFixed(1) }}
                        </span>
                      </div>
                      <div class="param-item">
                        <span class="param-label">影響力</span>
            ...
            <div class="config-block">
              <div class="config-block-header">
                <span class="config-block-title">推薦アルゴリズム設定</span>
              </div>
              <div class="platforms-grid">
                <div v-if="simulationConfig.twitter_config" class="platform-card">
                  <div class="platform-card-header">
                    <span class="platform-name">プラットフォーム 1：広場 / 情報フィード</span>
                  </div>
                  <div class="platform-params">
                    <div class="param-row">
                      <span class="param-label">時効性重み</span>
                      <span class="param-value">{{ simulationConfig.twitter_config.recency_weight }}</span>
                    </div>
                    <div class="param-row">
                      <span class="param-label">人気度重み</span>
                      <span class="param-value">{{ simulationConfig.twitter_config.popularity_weight }}</span>
                    </div>
                    <div class="param-row">
                      <span class="param-label">関連性重み</span>
                      <span class="param-value">{{ simulationConfig.twitter_config.relevance_weight }}</span>
                    </div>
                    <div class="param-row">
                      <span class="param-label">バイラル閾値</span>
            ...
            <div v-if="simulationConfig.generation_reasoning" class="config-block">
              <div class="config-block-header">
                <span class="config-block-title">LLM 設定推論</span>
              </div>
      ...
      <div class="step-card" :class="{ 'active': phase === 3, 'completed': phase > 3 }">
        ...
        <div class="card-content">
          <p class="api-note">POST /api/simulation/prepare</p>
          <p class="description">
            ナラティブ方向に基づき、初期アクティベーションイベントとホットトピックを自動生成し、シミュレーション世界の初期状態を誘導します
          </p>
          ...
          <div v-if="simulationConfig?.event_config" class="orchestration-content">
            ...
            <div class="initial-posts-section">
              <span class="box-label">初期アクティベーションシーケンス ({{ simulationConfig.event_config.initial_posts.length }})</span>
      ...
      <div class="step-card" :class="{ 'active': phase === 4 }">
        ...
        <div class="card-content">
          ...
          <p class="description">シミュレーション環境の準備が完了しました。シミュレーションを開始できます</p>
          ...
          <div v-if="simulationConfig && autoGeneratedRounds" class="rounds-config-section">
            <div class="rounds-header">
              <div class="header-left">
                ...
                <span class="section-desc">MiroFishが自動で現実 <span class="desc-highlight">{{ simulationConfig?.time_config?.total_simulation_hours || '-' }}</span> 時間を計画し、1ラウンドが現実 <span class="desc-highlight">{{ simulationConfig?.time_config?.minutes_per_round || '-' }}</span> 分の時間経過を表します</span>
              </div>
              ...
              <div v-if="useCustomRounds" class="rounds-content custom" key="custom">
                <div class="slider-display">
                  ...
                  <div class="slider-meta-info">
                    <span>エージェント規模100の場合：予想所要時間約 {{ Math.round(customMaxRounds * 0.6) }} 分</span>
                  </div>
                </div>
                  ...
                  <div class="auto-content">
                    <div class="auto-meta-row">
                      <span class="duration-badge">
                        ...
                        エージェント規模100の場合：予想所要時間 {{ Math.round(autoGeneratedRounds * 0.6) }} 分
                      </span>
                    </div>
                    <div class="auto-desc">
                      <p class="highlight-tip" @click="useCustomRounds = true">初回実行時は「カスタムモード」に切り替えてラウンド数を減らし、効果を素早く確認してエラーリスクを低減することを強くお勧めします ➝</p>
                    </div>
                  </div>
                </div>
              </div>
      ...
      <div v-if="selectedProfile" class="profile-modal-overlay" @click.self="selectedProfile = null">
        ...
        <div class="modal-body">
          ...
          <div class="modal-info-grid">
            <div class="info-item">
              <span class="info-label">表示年齢</span>
              <span class="info-value">{{ selectedProfile.age || '-' }} 歳</span>
            </div>
            <div class="info-item">
              <span class="info-label">表示性別</span>
              <span class="info-value">{{ { male: '男性', female: '女性', other: 'その他' }[selectedProfile.gender] || selectedProfile.gender }}</span>
            </div>
            <div class="info-item">
              <span class="info-label">国/地域</span>
              <span class="info-value">{{ selectedProfile.country || '-' }}</span>
            </div>
            <div class="info-item">
              <span class="info-label">表示MBTI</span>
          ...
          <div class="modal-section">
            <span class="section-label">ペルソナ概要</span>
            <p class="section-bio">{{ selectedProfile.bio || '自己紹介なし' }}</p>
          </div>

          <!-- 关注话题 -->
          <div class="modal-section" v-if="selectedProfile.interested_topics?.length">
            <span class="section-label">現実シード関連トピック</span>
          ...
          <div class="modal-section" v-if="selectedProfile.persona">
            <span class="section-label">詳細ペルソナ背景</span>
            
            <!-- 人设维度概览 -->
            <div class="persona-dimensions">
              <div class="dimension-card">
                <span class="dim-title">イベント全景経験</span>
                <span class="dim-desc">このイベントにおける完全な行動軌跡</span>
              </div>
              <div class="dimension-card">
                <span class="dim-title">行動パターン分析</span>
                <span class="dim-desc">経験の要約と行動スタイルの傾向</span>
              </div>
              <div class="dimension-card">
                <span class="dim-title">固有の記憶痕跡</span>
                <span class="dim-desc">現実シードに基づいて形成された記憶</span>
              </div>
              <div class="dimension-card">
                <span class="dim-title">社会関係ネットワーク</span>
                <span class="dim-desc">個体のつながりとインタラクショングラフ</span>
              </div>
            </div>

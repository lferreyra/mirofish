    ...
    <div class="scroll-container">
      ...
      <div class="step-card" :class="{ 'active': phase === 1, 'completed': phase > 1 }">
        ...
        <div class="card-content">
          ...
          <p class="description">
            Combines context to automatically invoke tools, organizes entities and relationships from the knowledge graph, initializes simulation agents, and endows them with unique behaviors and memories based on reality seeds
          </p>
      ...
      <div class="step-card" :class="{ 'active': phase === 2, 'completed': phase > 2 }">
        ...
        <div class="card-content">
          <p class="api-note">POST /api/simulation/prepare</p>
          <p class="description">
            LLM intelligently configures world time flow rate, recommendation algorithms, each agent's active time periods, posting frequency, event triggers, and other parameters based on simulation requirements and reality seeds
          </p>
      ...
      <div class="step-card" :class="{ 'active': phase === 3, 'completed': phase > 3 }">
        ...
        <div class="card-content">
          <p class="api-note">POST /api/simulation/prepare</p>
          <p class="description">
            Based on narrative direction, automatically generates initial activation events and trending topics to guide the simulation world's initial state
          </p>
      ...
      <div class="step-card" :class="{ 'active': phase === 4 }">
        ...
        <div class="card-content">
          ...
          <!-- Simulation rounds config - only shown after config is generated and rounds calculated -->
          <div v-if="simulationConfig && autoGeneratedRounds" class="rounds-config-section">
            <div class="rounds-header">
              <div class="header-left">
                <span class="section-title">Simulation Rounds Setting</span>
                <span class="section-desc">MiroFish automatically plans simulation of <span class="desc-highlight">{{ simulationConfig?.time_config?.total_simulation_hours || '-' }}</span> hours of real time, each round represents <span class="desc-highlight">{{ simulationConfig?.time_config?.minutes_per_round || '-' }}</span> minutes of time passing</span>
              </div>
...
const autoGeneratedRounds = computed(() => {
  ...
  // Ensure max rounds is at least 40 (recommended), prevent slider range issues
...
// Calculate total topics count across all profiles
...
const handleStartSimulation = () => {
  ...
  if (useCustomRounds.value) {
    ...
    // User chose auto-generated rounds, no max_rounds param
...
const fetchProfilesRealtime = async () => {
  ...
  try {
    ...
    if (res.success && res.data) {
      ...
      // Output profile generation progress log (only on count change)
  ...
  try {
    ...
    if (res.success && res.data) {
      ...
      if (data.config_generated && data.config) {
        ...
        stopConfigPolling()
        phase.value = 4
        addLog('Environment setup complete, ready for simulation')

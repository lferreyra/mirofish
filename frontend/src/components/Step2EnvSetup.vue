    ...
    <div class="scroll-container">
      ...
      <div class="step-card" :class="{ 'active': phase === 1, 'completed': phase > 1 }">
        ...
        <div class="card-content">
          ...
          <p class="description">
            Using context, automatically invokes tools to extract entities and relations from the knowledge graph, initializes simulated individuals, and assigns them unique behaviors and memories based on real-world seeds
          </p>
      ...
      <div class="step-card" :class="{ 'active': phase === 2, 'completed': phase > 2 }">
        ...
        <div class="card-content">
          <p class="api-note">POST /api/simulation/prepare</p>
          <p class="description">
            LLM intelligently configures world time flow, recommendation algorithms, each individual's active hours, posting frequency, event triggers, and other parameters based on simulation needs and real-world seeds
          </p>
      ...
      <div class="step-card" :class="{ 'active': phase === 4 }">
        ...
        <div class="card-content">
          ...
          <!-- Simulation rounds config - only shown after config generation is complete and rounds are calculated -->
          <div v-if="simulationConfig && autoGeneratedRounds" class="rounds-config-section">
            <div class="rounds-header">
              <div class="header-left">
                <span class="section-title">Simulation Rounds Setting</span>
                <span class="section-desc">MiroFish auto-plans simulation of <span class="desc-highlight">{{ simulationConfig?.time_config?.total_simulation_hours || '-' }}</span> real-world hours, each round represents <span class="desc-highlight">{{ simulationConfig?.time_config?.minutes_per_round || '-' }}</span> minutes of time passing</span>
              </div>
...
const autoGeneratedRounds = computed(() => {
  ...
  // Ensure max rounds is at least 40 (recommended) to avoid slider range issues
...
const handleStartSimulation = () => {
  ...
  if (useCustomRounds.value) {
    ...
    // User keeps auto-generated rounds, no max_rounds param passed
  ...
  try {
    ...
    if (res.success && res.data) {
      if (res.data.config_generated && res.data.config) {
        ...
        addLog('Config is being generated, starting polling...')
        startConfigPolling()
      }
    }
  } catch (err) {

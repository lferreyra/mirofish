      ...
      <div class="step-card" :class="{ 'active': phase === 0, 'completed': phase > 0 }">
        <div class="card-header">
          <div class="step-info">
            ...
            <span class="step-title">{{ $t('step2.initTitle') }}</span>
          </div>
          <div class="step-status">
            <span v-if="phase > 0" class="badge success">{{ $t('common.completed') }}</span>
            <span v-else class="badge processing">{{ $t('step2.initBadge') }}</span>
          </div>
        </div>
        ...
        <div class="card-content">
          ...
          <div v-if="simulationId" class="info-card">
            ...
            <div class="info-row">
              ...
              <span class="info-value mono">{{ taskId || $t('step2.taskCompleted') }}</span>
            </div>
          </div>
        </div>
      </div>
      ...
      <div class="step-card" :class="{ 'active': phase === 1, 'completed': phase > 1 }">
        <div class="card-header">
          <div class="step-info">
            ...
            <span class="step-title">{{ $t('step2.profileTitle') }}</span>
          </div>
          <div class="step-status">
            <span v-if="phase > 1" class="badge success">{{ $t('common.completed') }}</span>
            <span v-else-if="phase === 1" class="badge processing">{{ prepareProgress }}%</span>
            <span v-else class="badge pending">{{ $t('common.waiting') }}</span>
          </div>
        </div>
          ...
          <div v-if="simulationConfig" class="config-detail-panel">
            ...
            <div class="config-block">
              <div class="config-grid">
                <div class="config-item">
                  ...
                  <span class="config-item-value">{{ simulationConfig.time_config?.total_simulation_hours || '-' }} {{ $t('common.hourUnit') }}</span>
                </div>
                <div class="config-item">
                  <span class="config-item-label">{{ $t('step2.roundDuration') }}</span>
                  <span class="config-item-value">{{ simulationConfig.time_config?.minutes_per_round || '-' }} {{ $t('common.minuteUnit') }}</span>
                </div>
                <div class="config-item">
                  <span class="config-item-label">{{ $t('step2.totalRounds') }}</span>
                  <span class="config-item-value">{{ Math.floor((simulationConfig.time_config?.total_simulation_hours * 60 / simulationConfig.time_config?.minutes_per_round)) || '-' }} {{ $t('common.roundUnit') }}</span>
                </div>
                <div class="config-item">
                  <span class="config-item-label">{{ $t('step2.hourlyActive') }}</span>
            ...
            <div class="config-block">
              <div class="config-block-header">
                ...
                <span class="config-block-badge">{{ simulationConfig.agent_configs?.length || 0 }} {{ $t('common.countUnit') }}</span>
              </div>
              <div class="agents-cards">
                ...
                >
                  ...
                  <div class="agent-timeline">
                    <span class="timeline-label">{{ $t('step2.activePeriods') }}</span>
      ...
      <div class="step-card" :class="{ 'active': phase === 3, 'completed': phase > 3 }">
        <div class="card-header">
          <div class="step-info">
            ...
            <span class="step-title">{{ $t('step2.activationTitle') }}</span>
          </div>
          <div class="step-status">
            <span v-if="phase > 3" class="badge success">{{ $t('common.completed') }}</span>
            <span v-else-if="phase === 3" class="badge processing">{{ $t('step2.activationBadge') }}</span>
            <span v-else class="badge pending">{{ $t('common.waiting') }}</span>
          </div>
        </div>
        ...
        <div class="card-content">
          ...
          <div v-if="simulationConfig?.event_config" class="orchestration-content">
            ...
            <div class="topics-section">
              <span class="box-label">{{ $t('step2.initialTopics') }}</span>
            ...
            <div class="initial-posts-section">
              <span class="box-label">{{ $t('step2.activationSequence', { count: simulationConfig.event_config.initial_posts.length }) }}</span>
      ...
      <div class="step-card" :class="{ 'active': phase === 4 }">
        <div class="card-header">
          <div class="step-info">
            ...
            <span class="step-title">{{ $t('step2.readyTitle') }}</span>
          </div>
          <div class="step-status">
            <span v-if="phase >= 4" class="badge processing">{{ $t('common.inProgress') }}</span>
            <span v-else class="badge pending">{{ $t('common.waiting') }}</span>
          </div>
        </div>
        ...
        <div class="card-content">
          ...
          <p class="description">{{ $t('step2.readyDesc') }}</p>
          
          <!-- 模拟轮数配置 - 只有在配置生成完成且轮数计算出来后才显示 -->
          <div v-if="simulationConfig && autoGeneratedRounds" class="rounds-config-section">
            <div class="rounds-header">
              <div class="header-left">
                <span class="section-title">{{ $t('step2.roundSettings') }}</span>
                <span class="section-desc">{{ $t('step2.roundSettingsDesc', { hours: simulationConfig?.time_config?.total_simulation_hours || '-', minutes: simulationConfig?.time_config?.minutes_per_round || '-' }) }}</span>
              </div>
                  ...
                  <div class="auto-content">
                    <div class="auto-meta-row">
                      <span class="duration-badge">
                        ...
                        {{ $t('step2.autoEstimate', { minutes: Math.round(autoGeneratedRounds * 0.6) }) }}
                      </span>
                    </div>
                    <div class="auto-desc">
      ...
      <div v-if="selectedProfile" class="profile-modal-overlay" @click.self="selectedProfile = null">
        ...
        <div class="modal-body">
          ...
          <div class="modal-info-grid">
            <div class="info-item">
              <span class="info-label">{{ $t('step2.apparentAge') }}</span>
              <span class="info-value">{{ selectedProfile.age || '-' }} {{ $t('common.ageUnit') }}</span>
            </div>
            <div class="info-item">
              <span class="info-label">{{ $t('step2.apparentGender') }}</span>
            ...
            <div class="info-item">
              <span class="info-label">{{ $t('step2.apparentMBTI') }}</span>
          ...
          <div class="modal-section" v-if="selectedProfile.interested_topics?.length">
            <span class="section-label">{{ $t('step2.relatedTopicsModal') }}</span>
          ...
          <div class="modal-section" v-if="selectedProfile.persona">
            ...
            <div class="persona-dimensions">
              <div class="dimension-card">
                <span class="dim-title">{{ $t('step2.fullExperience') }}</span>
                <span class="dim-desc">{{ $t('step2.fullExperienceDesc') }}</span>
              </div>
              <div class="dimension-card">
...
watch(currentStage, (newStage) => {
  if (newStage === 'generating_profiles') {
    ...
    phase.value = 2
    if (!configTimer) {
      addLog(t('step2.startGeneratingConfig'))
      startConfigPolling()
    }
...
const handleStartSimulation = () => {
  ...
  if (useCustomRounds.value) {
    ...
    addLog(t('step2.startSimCustom', { n: customMaxRounds.value }))
  } else {
    // 用户选择保持自动生成的轮数，不传递 max_rounds 参数
    addLog(t('step2.startSimAuto', { n: autoGeneratedRounds.value }))
  }
  ...
  try {
    ...
    if (res.success && res.data) {
      if (res.data.already_prepared) {
        addLog(t('step2.existingPrepDetected'))
        await loadPreparedData()
        return
      }
      
      taskId.value = res.data.task_id
      addLog(t('step2.prepTaskStarted'))
      addLog(`  └─ Task ID: ${res.data.task_id}`)

      // 立即设置预期Agent总数（从prepare接口返回值获取）
      if (res.data.expected_entities_count) {
        expectedTotal.value = res.data.expected_entities_count
        addLog(t('step2.zepEntities', { n: res.data.expected_entities_count }))
      ...
      addLog(t('step2.prepFailed', { msg: res.error || t('common.unknownError') }))
      emit('update-status', 'error')
    }
  } catch (err) {
    addLog(t('step2.prepError', { msg: err.message }))
    emit('update-status', 'error')
  }
}
  ...
  try {
    ...
    if (res.success && res.data) {
      ...
      if (data.status === 'completed' || data.status === 'ready' || data.already_prepared) {
        addLog(t('step2.prepComplete'))
        stopPolling()
        stopProfilesPolling()
        await loadPreparedData()
      } else if (data.status === 'failed') {
        addLog(t('step2.prepFailedStatus', { msg: data.error || t('common.unknownError') }))
        stopPolling()
        stopProfilesPolling()
      }
    }
  ...
  try {
    ...
    if (res.success && res.data) {
      ...
      if (currentCount > 0 && currentCount !== lastLoggedProfileCount) {
        ...
        if (currentCount === 1) {
          addLog(t('step2.generateAgentPersonas'))
        }
        addLog(`→ Agent persona ${currentCount}/${total}: ${profileName} (${latestProfile?.profession || t('common.unknownProfession')})`)
  ...
  try {
    ...
    if (res.success && res.data) {
      ...
      if (data.config_generated && data.config) {
        ...
        // 显示详细配置摘要
        if (data.summary) {
          addLog(`  ├─ ${t('step2.currentAgents')}: ${data.summary.total_agents}`)
          addLog(`  ├─ ${t('step2.simDuration')}: ${data.summary.simulation_hours} ${t('common.hourUnit')}`)
        ...
        if (data.config.time_config) {
          ...
          addLog(t('views.timeConfig', { minutes: tc.minutes_per_round }) + `, ${Math.floor((tc.total_simulation_hours * 60) / tc.minutes_per_round)} ${t('common.roundUnit')}`)
        }

        // 显示事件配置
        if (data.config.event_config?.narrative_direction) {
          const narrative = data.config.event_config.narrative_direction
          addLog(`${t('step2.narrativeDirection')}: ${narrative.length > 50 ? narrative.substring(0, 50) + '...' : narrative}`)
        }

        stopConfigPolling()
        phase.value = 4
        addLog(t('step2.readyDesc'))
  ...
  try {
    ...
    if (res.success && res.data) {
      if (res.data.config_generated && res.data.config) {
        ...
        // 显示详细配置摘要
        if (res.data.summary) {
          addLog(`  ├─ ${t('step2.currentAgents')}: ${res.data.summary.total_agents}`)
          addLog(`  ├─ ${t('step2.simDuration')}: ${res.data.summary.simulation_hours} ${t('common.hourUnit')}`)
...
onMounted(() => {
  // 自动开始准备流程
  if (props.simulationId) {
    addLog(t('views.viewInit', { view: 'Step2' }))

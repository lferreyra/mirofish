        ...
        <div class="action-bar-header">
          ...
          <div class="action-bar-tabs">
            ...
            >
              ...
              <span>{{ $t('step5.chat_with_report') }}</span>
            </button>
            <div class="agent-dropdown" v-if="profiles.length > 0">
              ...
              <div v-if="showAgentDropdown" class="dropdown-menu">
                <div class="dropdown-header">{{ $t('step5.select_target') }}</div>
                <div 
        ...
        <div v-if="activeTab === 'chat'" class="chat-container">
          ...
          <div v-if="chatTarget === 'report_agent'" class="report-agent-tools-card">
            <div class="tools-card-header">
              ...
              <div class="tools-card-info">
                <div class="tools-card-name">{{ $t('step5.report_agent_chat') }}</div>
                <div class="tools-card-subtitle">{{ $t('step5.report_agent_desc') }}</div>
              </div>
              ...
              <div class="tools-grid">
                <div class="tool-item tool-purple">
                  ...
                  <div class="tool-content">
                    <div class="tool-name">{{ $t('step4.tools.insight_forge') }}</div>
                    <div class="tool-desc">{{ $t('step5.tools.insight_desc') }}</div>
                  </div>
                </div>
                <div class="tool-item tool-blue">
                  ...
                  <div class="tool-content">
                    <div class="tool-name">{{ $t('step4.tools.panorama_search') }}</div>
                    <div class="tool-desc">{{ $t('step5.tools.panorama_desc') }}</div>
                  </div>
                </div>
                <div class="tool-item tool-orange">
                  ...
                  <div class="tool-content">
                    <div class="tool-name">{{ $t('step4.tools.quick_search') }}</div>
                    <div class="tool-desc">{{ $t('step5.tools.quick_desc') }}</div>
                  </div>
                </div>
                <div class="tool-item tool-green">
                  ...
                  <div class="tool-content">
                    <div class="tool-name">{{ $t('step4.tools.interview_agents') }}</div>
                    <div class="tool-desc">{{ $t('step5.tools.interview_desc') }}</div>
                  </div>
                </div>
              </div>
            </div>
          ...
          <div v-if="chatTarget === 'agent' && selectedAgent" class="agent-profile-card">
            ...
            <div v-if="showFullProfile && selectedAgent.bio" class="profile-card-body">
              <div class="profile-card-bio">
                <div class="profile-card-label">{{ $t('step5.bio_label') }}</div>
          ...
          <div class="chat-messages" ref="chatMessages">
            <div v-if="chatHistory.length === 0" class="chat-empty">
              ...
              <p class="empty-text">
                {{ chatTarget === 'report_agent' ? $t('step5.empty_report') : $t('step5.empty_agent') }}
              </p>
            </div>
            <div 
        ...
        <div v-if="activeTab === 'survey'" class="survey-container">
          ...
          <div class="survey-setup">
            <div class="setup-section">
              <div class="section-header">
                <span class="section-title">{{ $t('step5.select_survey_target') }}</span>
                <span class="selection-count">{{ $t('step5.selected_count', { count: selectedAgents.size, total: profiles.length }) }}</span>
              </div>
              ...
              <div class="selection-actions">
                <button class="action-link" @click="selectAllAgents">{{ $t('common.select_all') }}</button>
                <span class="action-divider">|</span>
                <button class="action-link" @click="clearAgentSelection">{{ $t('step5.clear_selection') }}</button>
              </div>
            </div>
            ...
            >
              ...
              <span v-else>{{ $t('step5.submit_survey') }}</span>
            </button>
          </div>
          ...
          <div v-if="surveyResults.length > 0" class="survey-results">
            <div class="results-header">
              ...
              <span class="results-count">{{ $t('step5.survey_reply_count', { count: surveyResults.length }) }}</span>
            </div>
...
const selectAgent = (agent, idx) => {
  ...
  addLog(`${t('step5.select_target')}: ${agent.username}`)
}
...
const sendMessage = async () => {
  ...
  try {
    ...
    chatHistory.value.push({
      role: 'assistant',
      content: `${t('common.unknown_error')}: ${err.message}`,
      timestamp: new Date().toISOString()
    })
  } finally {
...
const sendToReportAgent = async (message) => {
  addLog(`${t('step5.chat_with_report')} -> ${message.substring(0, 50)}...`)
  ...
  if (res.success && res.data) {
    chatHistory.value.push({
      role: 'assistant',
      content: res.data.response || res.data.answer || t('common.no_response'),
      timestamp: new Date().toISOString()
    })
    addLog(t('step4.workflow.llm_response'))
  } else {
    throw new Error(res.error || t('common.unknown_error'))
  }
}

const sendToAgent = async (message) => {
  if (!selectedAgent.value || selectedAgentIndex.value === null) {
    throw new Error(t('step5.select_target'))
  }
  
  addLog(`${t('step5.chat_with_agent')} (${selectedAgent.value.username}) -> ${message.substring(0, 50)}...`)
  ...
  if (chatHistory.value.length > 1) {
    const historyContext = chatHistory.value
      ...
      .slice(-6)
      .map(msg => `${msg.role === 'user' ? t('step5.you') : selectedAgent.value.username}: ${msg.content}`)
      .join('\n')
    prompt = `${t('step5.tools.history_context')}\n${historyContext}\n\n${t('step5.tools.new_question')}${message}`
  }
  ...
  if (res.success && res.data) {
    ...
    let responseContent = t('common.no_response')
    ...
    if (typeof resultsDict === 'object' && !Array.isArray(resultsDict)) {
      ...
      if (agentResult) {
        responseContent = agentResult.response || agentResult.answer || t('common.no_response')
      }
    } else if (Array.isArray(resultsDict) && resultsDict.length > 0) {
      responseContent = resultsDict[0].response || resultsDict[0].answer || t('common.no_response')
    }
    ...
    addLog(`${selectedAgent.value.username} ${t('common.success').toLowerCase()}`)
  } else {
    throw new Error(res.error || t('common.unknown_error'))
  }
}
...
const submitSurvey = async () => {
  ...
  addLog(`${t('step5.send_survey')}: ${selectedAgents.value.size} ${t('common.items')}`)
    ...
    if (res.success && res.data) {
      ...
      for (const interview of interviews) {
        ...
        if (typeof resultsDict === 'object' && !Array.isArray(resultsDict)) {
          ...
          if (matchedResult) {
            responseContent = matchedResult.response || matchedResult.answer || t('common.no_response')
          }
        }
      ...
      addLog(t('step5.logs.reply_count', { count: surveyResults.value.length }))
    } else {
      throw new Error(res.error || t('common.request_failed'))
    }
  } catch (err) {
    addLog(`${t('step5.logs.survey_failed')}: ${err.message}`)
  } finally {
...
const loadReportData = async () => {
  ...
  try {
    addLog(t('step5.logs.load_report', { id: props.reportId }))
    ...
    addLog(`${t('step5.logs.load_report')} ${t('common.failed')}: ${err.message}`)
  }
}
...
const loadAgentLogs = async () => {
  ...
  try {
    ...
    if (res.success && res.data) {
      ...
      addLog(t('step5.logs.report_load_complete'))
    }
  } catch (err) {
    addLog(`${t('step4.header.console_output')} ${t('common.failed')}: ${err.message}`)
  }
}
...
const loadProfiles = async () => {
  ...
  try {
    ...
    if (res.success && res.data) {
      ...
      addLog(t('step5.logs.load_profiles', { count: profiles.value.length }))
    }
  } catch (err) {
    addLog(`${t('step5.logs.load_profiles')} ${t('common.failed')}: ${err.message}`)
  }
}

        ...
        <div class="action-bar-header">
          ...
          <div class="action-bar-tabs">
            ...
            >
              ...
              <span>{{ t('interaction.chatWithReportAgentBtn') }}</span>
            </button>
            <div class="agent-dropdown" v-if="profiles.length > 0">
              ...
              >
                ...
                <span>{{ selectedAgent ? selectedAgent.username : t('interaction.chatWithAnyAgentBtn') }}</span>
                <svg class="dropdown-arrow" :class="{ open: showAgentDropdown }" viewBox="0 0 24 24" width="12" height="12" fill="none" stroke="currentColor" stroke-width="2">
                  <polyline points="6 9 12 15 18 9"></polyline>
                </svg>
              </button>
              <div v-if="showAgentDropdown" class="dropdown-menu">
                <div class="dropdown-header">{{ t('interaction.selectChatTarget') }}</div>
                <div 
                ...
                >
                  ...
                  <div class="agent-info">
                    ...
                    <span class="agent-role">{{ agent.profession || t('interaction.unknownProfession') }}</span>
                  </div>
                </div>
              </div>
            </div>
            ...
            >
              ...
              <span>{{ t('interaction.sendSurveyToWorldBtn') }}</span>
            </button>
          </div>
        </div>
        ...
        <div v-if="activeTab === 'chat'" class="chat-container">
          ...
          <div v-if="chatTarget === 'report_agent'" class="report-agent-tools-card">
            <div class="tools-card-header">
              ...
              <div class="tools-card-info">
                ...
                <div class="tools-card-subtitle">{{ t('interaction.reportAgentSubtitle') }}</div>
              </div>
              ...
              <div class="tools-grid">
                <div class="tool-item tool-purple">
                  ...
                  <div class="tool-content">
                    <div class="tool-name">{{ t('interaction.insightForge') }}</div>
                    <div class="tool-desc">{{ t('interaction.insightForgeDesc') }}</div>
                  </div>
                </div>
                <div class="tool-item tool-blue">
                  ...
                  <div class="tool-content">
                    <div class="tool-name">{{ t('interaction.panoramaSearch') }}</div>
                    <div class="tool-desc">{{ t('interaction.panoramaSearchDesc') }}</div>
                  </div>
                </div>
                <div class="tool-item tool-orange">
                  ...
                  <div class="tool-content">
                    <div class="tool-name">{{ t('interaction.quickSearch') }}</div>
                    <div class="tool-desc">{{ t('interaction.quickSearchDesc') }}</div>
                  </div>
                </div>
                <div class="tool-item tool-green">
                  ...
                  <div class="tool-content">
                    ...
                    <div class="tool-desc">{{ t('interaction.smartSummaryDesc') }}</div>
                  </div>
                </div>
              </div>
            </div>
          ...
          <div v-if="chatTarget === 'agent' && selectedAgent" class="agent-profile-card">
            <div class="profile-card-header">
              ...
              <div class="profile-card-info">
                ...
                <div class="profile-card-meta">
                  ...
                  <span class="profile-card-profession">{{ selectedAgent.profession || t('interaction.unknownProfession') }}</span>
                </div>
              </div>
            ...
            <div v-if="showFullProfile && selectedAgent.bio" class="profile-card-body">
              <div class="profile-card-bio">
                <div class="profile-card-label">{{ t('interaction.bioLabel') }}</div>
          ...
          <div class="chat-messages" ref="chatMessages">
            <div v-if="chatHistory.length === 0" class="chat-empty">
              ...
              <p class="empty-text">
                {{ chatTarget === 'report_agent' ? t('interaction.chatWithReportHint') : t('interaction.chatWithAgentHint') }}
              </p>
            </div>
            <div 
          ...
          <div class="chat-input-area">
            <textarea 
              v-model="chatInput"
              class="chat-input"
              :placeholder="t('interaction.typeYourQuestion')"
        ...
        <div v-if="activeTab === 'survey'" class="survey-container">
          ...
          <div class="survey-setup">
            <div class="setup-section">
              <div class="section-header">
                <span class="section-title">{{ t('interaction.selectSurveyTargets') }}</span>
                <span class="selection-count">{{ t('interaction.selectedCount').replace('{selected}', selectedAgents.size).replace('{total}', profiles.length) }}</span>
              </div>
              <div class="agents-grid">
                ...
                >
                  ...
                  <div class="checkbox-info">
                    ...
                    <span class="checkbox-role">{{ agent.profession || t('interaction.unknownProfession') }}</span>
                  </div>
              ...
              <div class="selection-actions">
                <button class="action-link" @click="selectAllAgents">{{ t('interaction.selectAll') }}</button>
                <span class="action-divider">|</span>
                <button class="action-link" @click="clearAgentSelection">{{ t('interaction.clearSelection') }}</button>
              </div>
            </div>

            <div class="setup-section">
              <div class="section-header">
                <span class="section-title">{{ t('interaction.surveyQuestionLabel') }}</span>
              </div>
              <textarea 
                v-model="surveyQuestion"
                class="survey-input"
                :placeholder="t('interaction.surveyQuestionPlaceholder')"
                rows="3"
              ></textarea>
            </div>
            ...
            >
              ...
              <span v-else>{{ t('interaction.sendSurvey') }}</span>
            </button>
          </div>
          ...
          <div v-if="surveyResults.length > 0" class="survey-results">
            <div class="results-header">
              <span class="results-title">{{ t('interaction.surveyResultsTitle') }}</span>
              <span class="results-count">{{ t('interaction.repliesCount').replace('{count}', surveyResults.length) }}</span>
            </div>
              ...
              >
                <div class="result-header">
                  ...
                  <div class="result-info">
                    ...
                    <span class="result-role">{{ result.profession || t('interaction.unknownProfession') }}</span>
                  </div>
                </div>
...
const selectAgent = (agent, idx) => {
  ...
  addLog(t('interaction.selectTarget').replace('{agent}', agent.username))
}
...
const sendMessage = async () => {
  ...
  try {
    ...
    addLog(t('interaction.sendFailed').replace('{error}', err.message))
    chatHistory.value.push({
      role: 'assistant',
      content: t('interaction.sorryError').replace('{error}', err.message),
      timestamp: new Date().toISOString()
    })
  } finally {
...
const sendToReportAgent = async (message) => {
  addLog(t('interaction.sendToReportAgent').replace('{message}', message.substring(0, 50)).replace('...', '...'))
  ...
  if (res.success && res.data) {
    chatHistory.value.push({
      role: 'assistant',
      content: res.data.response || res.data.answer || t('interaction.noResponseContent'),
      timestamp: new Date().toISOString()
    })
    addLog(t('interaction.reportAgentReplied'))
  } else {
    throw new Error(res.error || t('interaction.requestFailed'))
  }
}

const sendToAgent = async (message) => {
  if (!selectedAgent.value || selectedAgentIndex.value === null) {
    throw new Error(t('interaction.pleaseSelectAgent'))
  }
  
  addLog(t('interaction.sendToAgent').replace('{agent}', selectedAgent.value.username).replace('{message}', message.substring(0, 50)))
  ...
  if (chatHistory.value.length > 1) {
    const historyContext = chatHistory.value
      ...
      .slice(-6)
      .map(msg => `${msg.role === 'user' ? t('interaction.questioner') : t('interaction.you')}：${msg.content}`)
      .join('\n')
    prompt = `${t('interaction.previousConversation')}\n${historyContext}\n\n${t('interaction.newQuestion')}${message}`
  }
  ...
  if (res.success && res.data) {
    ...
    if (responseContent) {
      ...
      addLog(t('interaction.agentRepliedLog').replace('{agent}', selectedAgent.value.username))
    } else {
      throw new Error(t('interaction.noResponseData'))
    }
  } else {
    throw new Error(res.error || t('interaction.requestFailed'))
  }
...
const submitSurvey = async () => {
  ...
  addLog(t('interaction.surveySent').replace('{count}', selectedAgents.value.size))
  ...
  try {
    ...
    if (res.success && res.data) {
      ...
      for (const interview of interviews) {
        ...
        // 优先使用 reddit 平台回复，其次 twitter
        let responseContent = t('interaction.noResponseContent')
        ...
        if (typeof resultsDict === 'object' && !Array.isArray(resultsDict)) {
          ...
          if (agentResult) {
            responseContent = agentResult.response || agentResult.answer || t('interaction.noResponseContent')
          }
        } else if (Array.isArray(resultsDict)) {
          // 兼容数组格式
          const matchedResult = resultsDict.find(r => r.agent_id === agentIdx)
          if (matchedResult) {
            responseContent = matchedResult.response || matchedResult.answer || t('interaction.noResponseContent')
          }
        }
      ...
      addLog(t('interaction.surveyReceived').replace('{count}', surveyResults.value.length))
    } else {
      throw new Error(res.error || t('interaction.requestFailed'))
    }
  } catch (err) {
    addLog(t('interaction.surveyFailed').replace('{error}', err.message))
  } finally {
...
const loadReportData = async () => {
  ...
  try {
    addLog(t('interaction.loadingReport').replace('{id}', props.reportId))
    ...
    addLog(t('interaction.reportLoadFailed').replace('{error}', err.message))
  }
}
...
const loadAgentLogs = async () => {
  ...
  try {
    ...
    if (res.success && res.data) {
      ...
      addLog(t('interaction.reportLoaded'))
    }
  } catch (err) {
    addLog(t('interaction.reportLoadFailed').replace('{error}', err.message))
  }
...
const loadProfiles = async () => {
  ...
  try {
    ...
    if (res.success && res.data) {
      ...
      addLog(t('interaction.loadedAgents').replace('{count}', profiles.value.length))
    }
  } catch (err) {
    addLog(t('interaction.loadAgentsFailed').replace('{error}', err.message))
  }
}
...
// Lifecycle
onMounted(() => {
  addLog(t('interaction.step5Init'))
  loadReportData()
  loadProfiles()

    ...
    <div class="graph-container" ref="graphContainer">
      ...
      <div v-if="graphData" class="graph-view">
        ...
        <div v-if="showSimulationFinishedHint" class="graph-building-hint finished-hint">
          ...
          <span class="hint-text">A small amount of content is still processing. It is recommended to manually refresh the graph shortly.</span>
...
watch(() => props.isSimulating, (newValue, oldValue) => {
  if (wasSimulating.value && !newValue) {
    // Transitioned from simulating to not simulating — show end hint
...
const renderGraph = () => {
  ...
  // Process edge data: count and index edges between the same node pairs
  const edgePairCount = {}
  const selfLoopEdges = {} // Self-loop edges grouped by node
  const tempEdges = edgesData
    .filter(e => nodeIds.has(e.source_node_uuid) && nodeIds.has(e.target_node_uuid))
  
  // Count edges between each node pair and collect self-loop edges
  tempEdges.forEach(e => {
  ...
  // Track the current index for each node pair
  const edgePairIndex = {}
  ...
  tempEdges.forEach(e => {
    ...
    if (isSelfLoop) {
      ...
      edges.push({
        ...
        rawData: {
          ...
          selfLoopEdges: allSelfLoops // Store full details of all self-loop edges
        }
      })
      return
    }
    ...
    // Determine if the edge direction matches the normalized direction (source UUID < target UUID)
    const isReversed = e.source_node_uuid > e.target_node_uuid

    // Calculate curvature: spread out for multiple edges, straight line for a single edge
    let curvature = 0
    if (totalCount > 1) {
      // Distribute curvature evenly to ensure clear distinction.
      // Curvature range increases with edge count.
      const curvatureRange = Math.min(1.2, 0.6 + totalCount * 0.15)
      curvature = ((currentIndex / (totalCount - 1)) - 0.5) * curvatureRange * 2

      // Flip curvature if edge direction is reversed,
      // so all edges are distributed in the same frame of reference without overlap.
      if (isReversed) {
        curvature = -curvature
      }
    }
  ...
  const simulation = d3.forceSimulation(nodes)
    .force('link', d3.forceLink(edges).id(d => d.id).distance(d => {
      // Dynamically adjust distance based on number of edges between this pair.
  ...
  const getLinkPath = (d) => {
    ...
    // Detect self-loop
    if (d.isSelfLoop) {
      // Self-loop: draw an arc that leaves and returns to the node
      const loopRadius = 30
      // Start from the right side of the node
    ...
    // Perpendicular offset relative to line direction, proportional to distance,
    // ensuring the curve is clearly visible. More edges → larger offset ratio.
  ...
  const getLinkMidpoint = (d) => {
    ...
    // Detect self-loop
    if (d.isSelfLoop) {
      // Self-loop label: to the right of the node
      return { x: sx + 70, y: sy }
    }
  ...
  // Save refs for external visibility control
  ...
  const node = nodeGroup.selectAll('circle')
    ...
    .call(d3.drag()
      .on('start', (event, d) => {
        // Record position only; do not restart simulation (distinguish click vs drag)
        d.fx = d.x
        d.fy = d.y
      ...
      .on('drag', (event, d) => {
        ...
        if (!d._isDragging && distance > 3) {
          // Only restart simulation on first genuine drag detection
          d._isDragging = true
      ...
      .on('end', (event, d) => {
        // Only cool down simulation if a real drag occurred
        if (d._isDragging) {

<template>
  <div class="graph-panel">
    <div class="panel-header">
      ...
      <!-- Bara de unelte superioară (Sus-dreapta intern) -->
      <div class="header-tools">
        <button class="tool-btn" @click="$emit('refresh')" :disabled="loading" title="Reîmprospătare graf">
          <span class="icon-refresh" :class="{ 'spinning': loading }">↻</span>
          <span class="btn-text">Refresh</span>
        </button>
        <button class="tool-btn" @click="$emit('toggle-maximize')" title="Maximizare/Restaurare">
    ...
    <div class="graph-container" ref="graphContainer">
      ...
      <div v-if="graphData" class="graph-view">
        ...
        <div v-if="currentPhase === 1 || isSimulating" class="graph-building-hint">
          ...
          {{ isSimulating ? 'GraphRAG - Actualizare în timp real memorie lungă și scurtă' : 'Actualizare în timp real...' }}
        </div>
        
        <!-- Indicație după finalizare simulare -->
      ...
      <div v-else-if="loading" class="graph-state">
        ...
        <p>Date graf se încarcă...</p>
      </div>
      
      <!-- Stare așteptare/gol -->
      <div v-else class="graph-state">
        <div class="empty-icon">❖</div>
        <p class="empty-text">Se așteaptă generarea ontologiei...</p>
      </div>
    </div>

    <!-- Legendă inferioară (Jos-stânga) -->
    ...
    <!-- Comutare afișare etichete muchii -->
...
/* Entity Types Legend - Jos-stânga */
.graph-legend {
  position: absolute;
  bottom: 24px;
  left: 24px;
...
/* Indicație după finalizare simulare样式 */

...
const showEdgeLabels = ref(true) // Default to showing edge labels
...
const renderGraph = () => {
  ...
  tempEdges.forEach(e => {
    ...
    if (isSelfLoop) {
      // Self-loop edge - only add one merged per node
    ...
    // Check if edge direction matches standardized direction (source UUID < target UUID)
    const isReversed = e.source_node_uuid > e.target_node_uuid
    
    // Calculate curvature: spread apart for multiple edges, straight line for single edge
    let curvature = 0
    if (totalCount > 1) {
      // Evenly distribute curvature for clear separation
  ...
  const simulation = d3.forceSimulation(nodes)
    ...
    // Add center gravity to cluster isolated node groups
  ...
  const getLinkPath = (d) => {
    ...
    // Calculate curve control point - dynamically adjust by edge count and distance
    const dx = tx - sx, dy = ty - sy
    const dist = Math.sqrt(dx * dx + dy * dy)
    // Perpendicular offset, proportional to distance, ensuring visible curves
  ...
  // Link labels background (white bg for better readability)
  ...
  const node = nodeGroup.selectAll('circle')
    ...
    .call(d3.drag()
      ...
      .on('drag', (event, d) => {
        // Detect if real drag started (movement exceeds threshold)

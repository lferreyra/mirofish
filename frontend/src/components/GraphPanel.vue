...
const renderGraph = () => {
  ...
  const getLinkPath = (d) => {
    ...
    // Perpendicular offset scaled by distance ratio for visible curves
  ...
  // Link labels background (white background for text clarity)
  ...
  const node = nodeGroup.selectAll('circle')
    ...
    .call(d3.drag()
      ...
      .on('end', (event, d) => {
        // Only let simulation cool down if actual drag occurred
        if (d._isDragging) {

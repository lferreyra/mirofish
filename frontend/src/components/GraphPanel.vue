    ...
    <div class="graph-container" ref="graphContainer">
      ...
      <div v-if="graphData" class="graph-view">
        ...
        <div v-if="showSimulationFinishedHint" class="graph-building-hint finished-hint">
          ...
          <span class="hint-text">Some content is still being processed. Please manually refresh the graph later</span>
...
const renderGraph = () => {
  ...
  tempEdges.forEach(e => {
    ...
    let curvature = 0
    if (totalCount > 1) {
      // Evenly distribute curvature for clear differentiation
      ...
      // This ensures all edges are distributed in the same reference frame without overlapping due to direction
      if (isReversed) {
        curvature = -curvature
      }
    }
  ...
  const getLinkPath = (d) => {
    ...
    // Offset perpendicular to line direction, proportional to distance, ensuring curve is clearly visible
  ...
  const node = nodeGroup.selectAll('circle')
    ...
    .call(d3.drag()
      ...
      .on('end', (event, d) => {
        // Only let simulation gradually stop if real drag occurred
        if (d._isDragging) {

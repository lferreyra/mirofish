  ...
  <div class="graph-panel">
    <div class="panel-header">
      ...
      <div class="header-tools">
        <button class="tool-btn" @click="$emit('refresh')" :disabled="loading" :title="$t('graph.refreshTitle')">
          <span class="icon-refresh" :class="{ 'spinning': loading }">↻</span>
          <span class="btn-text">{{ $t('graph.refresh') }}</span>
        </button>
        <button class="tool-btn" @click="$emit('toggle-maximize')" :title="$t('graph.maximizeTitle')">
    ...
    <div class="graph-container" ref="graphContainer">
      ...
      <div v-if="graphData" class="graph-view">
        ...
        <div v-if="currentPhase === 1 || isSimulating" class="graph-building-hint">
          ...
          {{ isSimulating ? $t('graph.graphRagUpdating') : $t('graph.liveUpdating') }}
        </div>
        ...
        <div v-if="showSimulationFinishedHint" class="graph-building-hint finished-hint">
          ...
          <span class="hint-text">{{ $t('graph.finishedHint') }}</span>
            ...
            <div class="detail-section" v-if="selectedItem.data.attributes && Object.keys(selectedItem.data.attributes).length > 0">
              ...
              <div class="properties-list">
                <div v-for="(value, key) in selectedItem.data.attributes" :key="key" class="property-item">
                  ...
                  <span class="property-value">{{ value || $t('graph.none') }}</span>
                </div>
              </div>
            </div>
          ...
          <div v-else class="detail-content">
            ...
            <template v-if="selectedItem.data.isSelfLoopGroup">
              <div class="edge-relation-header self-loop-header">
                ...
                <span class="self-loop-count">{{ selectedItem.data.selfLoopCount }} {{ $t('graph.items') }}</span>
              </div>
...
const renderGraph = () => {
  ...
  tempEdges.forEach(e => {
    ...
    edges.push({
      source: e.source_node_uuid,
      target: e.target_node_uuid,
      type: e.fact_type || e.name || t('graph.related'),

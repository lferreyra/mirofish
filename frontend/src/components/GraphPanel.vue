<template>
  <div class="graph-panel">
    ...
    <div class="graph-container" ref="graphContainer">
      ...
      <div v-if="graphData" class="graph-view">
        ...
        <div v-if="showSimulationFinishedHint" class="graph-building-hint finished-hint">
          ...
          <span class="hint-text">{{ $t('graph.finished_hint') }}</span>
          <button class="hint-close-btn" @click="dismissFinishedHint" :title="$t('common.close')">
        ...
        <div v-if="selectedItem" class="detail-panel">
          <div class="detail-panel-header">
            <span class="detail-title">{{ selectedItem.type === 'node' ? $t('graph.node_details') : $t('graph.relationship') }}</span>
          ...
          <div v-else class="detail-content">
            ...
            <template v-if="selectedItem.data.isSelfLoopGroup">
              <div class="edge-relation-header self-loop-header">
                {{ $t('graph.self_relations', { name: selectedItem.data.source_name }) }}
                <span class="self-loop-count">{{ $t('graph.items_count', { count: selectedItem.data.selfLoopCount }) }}</span>
              </div>
      ...
      <div v-else class="graph-state">
        ...
        <p class="empty-text">{{ $t('graph.waiting') }}</p>
      </div>
    </div>
    ...
    <div v-if="graphData" class="edge-labels-toggle">
      ...
      <span class="toggle-label">{{ $t('graph.show_edge_labels') }}</span>
    </div>
  </div>
</template>

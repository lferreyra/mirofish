...
const renderGraph = () => {
  ...
  tempEdges.forEach(e => {
    ...
    if (totalCount > 1) {
      ...
      // If edge direction is opposite to canonical, flip curvature
  ...
  const simulation = d3.forceSimulation(nodes)
    ...
    // Add gravitational pull toward center so isolated node clusters gather in center area
  ...
  const getLinkPath = (d) => {
    ...
    // Perpendicular offset from line direction, calculated by distance ratio, ensures visible curves
  ...
  // Link labels background (white bg makes text more readable)
  ...
  const node = nodeGroup.selectAll('circle')
    ...
    .call(d3.drag()
      ...
      .on('drag', (event, d) => {
        // Detect if actual dragging started (moved beyond threshold)
  ...
  simulation.on('tick', () => {
    ...
    // Update edge label positions (no rotation, horizontal is more readable)

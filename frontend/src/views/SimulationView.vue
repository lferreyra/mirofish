...
// Retorna true se viva, false se 404/morta
async function verificarTask(taskId) {
  try {
    const res  = await service.get(`/api/graph/task/${taskId}`)
    const task = res.data || res
    // Task existe mas pode estar failed
    if (task.status === 'failed') return false
    return true
  } catch (e) {
    // 404 = task não existe mais
    const status = e?.response?.status
    if (status === 404) return false
    // Outro erro de rede — assumir que pode estar viva
    return true
  }
}
  ...
  return new Promise((resolve, reject) => {
    ...
    pollTimer = setInterval(async () => {
      ...
      try {
        ...
        if (httpStatus === 404) {
          // Task não existe → servidor reiniciou, task morreu
          notFound++
          if (notFound >= 2) {
            clearInterval(pollTimer)
            // Tentar fallback: checar se grafo foi concluído antes de morrer
            try {
              const proj = await getProject(pid)
              if (proj.status === 'graph_completed' && proj.graph_id) {
                resolve() // Tinha concluído antes do restart!
              } else {
        ...
        // Outros erros de rede: ignorar e tentar novamente
      }
    }, interval)
  })
}
  ...
  <AppShell title="Nova Simulação">
    ...
    <div v-else class="pipeline">
      ...
      <div class="status-card"
        ...
        <!-- Botão cancelar — sempre visível durante execução -->
        <button

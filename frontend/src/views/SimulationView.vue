...
const handleNextStep = (params = {}) => {
  ...
  // Log simulation round configuration
  if (params.maxRounds) {
  ...
  // Pass custom rounds via query param if present
  if (params.maxRounds) {
 ...
 * When the user returns from Step 3 to Step 2, the simulation is stopped by default.
 */
const checkAndStopRunningSimulation = async () => {
  ...
  try {
    ...
    if (envStatusRes.success && envStatusRes.data?.env_alive) {
      ...
      // Attempt graceful shutdown of the simulation environment
      try {
        ...
        // If graceful shutdown throws, attempt force stop
        await forceStopSimulation()
      }
    } else {
      // Environment is not running, but the process may still exist — check simulation status
    ...
    // Failure to check environment status should not block subsequent flow
...
onMounted(async () => {
  ...
  // Check and stop any running simulation (when the user returns from Step 3)

...
const handleNextStep = (params = {}) => {
  ...
  // Record simulation round configuration
  if (params.maxRounds) {
  ...
  // If custom rounds, pass via query param
  if (params.maxRounds) {
  ...
  try {
    ...
    if (envStatusRes.success && envStatusRes.data?.env_alive) {
      ...
      try {
        ...
        // If graceful shutdown errors, try force stop
        await forceStopSimulation()
      }
    } else {
...
const forceStopSimulation = async () => {
  try {
    ...
    if (stopRes.success) {
      addLog('✓ Simulation forcefully stopped')
    } else {
      addLog(`Failed to forcefully stop simulation: ${stopRes.error || 'unknown error'}`)
    }
  } catch (err) {

    ...
    <header class="app-header">
      ...
      <div class="header-right">
        <div class="workflow-step">
          <span class="step-num">{{ $t('simulationView.stepNum') }}</span>
          <span class="step-name">{{ $t('simulationView.stepName') }}</span>
        </div>
...
const statusText = computed(() => {
  if (currentStatus.value === 'error') return t('simulationView.status.error')
  if (currentStatus.value === 'completed') return t('simulationView.status.ready')
  return t('simulationView.status.preparing')
})
...
const handleNextStep = (params = {}) => {
  ...
  // Pass custom round count via query param if provided
  if (params.maxRounds) {
 ...
 * Called when the user navigates back from Step 3 to Step 2.
 */
const checkAndStopRunningSimulation = async () => {
  ...
  try {
    ...
    if (envStatusRes.success && envStatusRes.data?.env_alive) {
      ...
      // Environment not running, but the process may still be active
    ...
    // Failure to check env status should not block the rest of the flow
...
onMounted(async () => {
  ...
  // Check and stop any running simulation (user navigating back from Step 3)

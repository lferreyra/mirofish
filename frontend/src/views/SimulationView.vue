...
const handleNextStep = (params = {}) => {
  ...
  // Pass custom rounds via query params if provided
  if (params.maxRounds) {
  ...
  try {
    ...
    if (envStatusRes.success && envStatusRes.data?.env_alive) {
      ...
      try {
        ...
        // If graceful shutdown threw, try force stop
        await forceStopSimulation()
      }
    } else {
...
const forceStopSimulation = async () => {
  try {
    ...
    if (stopRes.success) {
      ...
      addLog(`Failed to force stop: ${stopRes.error || 'Unknown error'}`)
    }
  } catch (err) {
    addLog(`Error force stopping: ${err.message}`)
  }
}

    ...
    <header class="app-header">
      ...
      <div class="header-right">
        <div class="workflow-step">
          <span class="step-num">{{ $t('simulationRunView.stepNum') }}</span>
          <span class="step-name">{{ $t('simulationRunView.stepName') }}</span>
        </div>
...
// Read maxRounds from query param at init so the child component gets it immediately
...
const statusText = computed(() => {
  if (currentStatus.value === 'error') return t('simulationRunView.status.error')
  if (currentStatus.value === 'completed') return t('simulationRunView.status.completed')
  return t('simulationRunView.status.running')
})
...
const handleGoBack = async () => {
  // Shut down the simulation before returning to Step 2
  ...
  try {
    ...
    if (envStatusRes.success && envStatusRes.data?.env_alive) {
      ...
      // Environment not running — check if the process needs stopping
    ...
    if (simRes.success && simRes.data) {
      ...
      try {
        ...
        if (configRes.success && configRes.data?.time_config?.minutes_per_round) {
          ...
          addLog(`Time config: ${minutesPerRound.value} minutes/round`)
        }
      } catch (configErr) {
...
const loadGraph = async (graphId) => {
  // During simulation, suppress fullscreen loading to avoid flicker.
  // Show loading on manual refresh or initial load only.
  if (!isSimulating.value) {
...
const startGraphRefresh = () => {
  ...
  addLog('Graph auto-refresh started (30s interval)')
...
onMounted(() => {
  ...
  // Log maxRounds config (already read from query params at init)
  if (maxRounds.value) {

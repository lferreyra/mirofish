      ...
      <div class="header-center">
        <div class="view-switcher">
          <button 
            ...
            @click="viewMode = mode"
          >
            {{ { graph: 'Graph', split: 'Dual Panel', workbench: 'Workspace' }[mode] }}
          </button>
        </div>
      </div>
...
// Read maxRounds from query params at initialization so child components receive it immediately
...
const handleGoBack = async () => {
  ...
  try {
    ...
    if (envStatusRes.success && envStatusRes.data?.env_alive) {
      ...
      // Environment is not running — check if the process needs to be stopped
  ...
  try {
    ...
    if (simRes.success && simRes.data) {
      ...
      try {
        ...
        addLog(`Failed to fetch time config, using default: ${minutesPerRound.value} min/round`)
      }
...
const loadGraph = async (graphId) => {
  // During simulation, skip the full-screen loading indicator on auto-refresh to avoid flicker
  // Show loading only on manual refresh or initial load
  if (!isSimulating.value) {
  ...
  try {
    ...
    if (res.success) {
      ...
      if (!isSimulating.value) {
        addLog('Knowledge graph data loaded successfully')
      }
    }
  } catch (err) {
    addLog(`Knowledge graph load failed: ${err.message}`)
  } finally {
...
const startGraphRefresh = () => {
  ...
  addLog('Enabling real-time graph refresh (30s)')
...
const stopGraphRefresh = () => {
  if (graphRefreshTimer) {
    ...
    addLog('Real-time graph refresh stopped')
  }
}
...
onMounted(() => {
  ...
  // Log maxRounds configuration (value already obtained from query params at initialization)
  if (maxRounds.value) {

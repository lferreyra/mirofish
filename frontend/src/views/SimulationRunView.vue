...
// Get maxRounds from query params at init so child component gets it immediately
  ...
  try {
    ...
    if (envStatusRes.success && envStatusRes.data?.env_alive) {
      ...
      try {
        ...
        addLog('Done: Simulation environment closed')
      } catch (closeErr) {
        addLog(`Failed to close environment, trying force stop...`)
        try {
          await stopSimulation({ simulation_id: currentSimulationId.value })
          addLog('Done: Simulation force stopped')
        } catch (stopErr) {
...
const loadGraph = async (graphId) => {
  // During simulation, auto-refresh without full-screen loading to avoid flicker
...
const startGraphRefresh = () => {
  ...
  addLog('Started graph live refresh (30s)')
...
const stopGraphRefresh = () => {
  if (graphRefreshTimer) {
    ...
    addLog('Stopped graph live refresh')
  }
}

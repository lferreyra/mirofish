...
// ─── Etapa 4: Parâmetros ──────────────────────────────────────
const agentes = ref(20)
const periodo = ref(24)  // período de análise em meses
const rodadas = ref(24)  // número de rodadas

// Escala calculada automaticamente
const diasPorRodada = computed(() => Math.round((periodo.value * 30) / rodadas.value))
const escalaLabel = computed(() => {
  const d = diasPorRodada.value
  if (d >= 28 && d <= 32) return '1 mês'
  if (d >= 13 && d <= 16) return '2 semanas'
  if (d >= 6 && d <= 8)   return '1 semana'
  if (d >= 1 && d <= 3)   return `${d} dias`
  if (d > 32) return `${Math.round(d/30)} meses`
  return `${d} dias`
})
const escalaParaRoute = computed(() => {
  const d = diasPorRodada.value
  if (d >= 25) return 'meses'
  if (d >= 6)  return 'semanas'
  return 'dias'
})
...
const descPeriodo = computed(() => {
  if (periodo.value <= 3)  return 'Curto prazo — reação imediata ao evento'
  if (periodo.value <= 12) return 'Médio prazo — captura tendências e sazonalidade'
  if (periodo.value <= 24) return 'Longo prazo — ciclo completo de maturação'
  return 'Muito longo prazo — evolução profunda'
})
const descRodadas = computed(() => {
  return `${rodadas.value} rodadas de ${escalaLabel.value} cada — cobrindo ${periodo.value} meses`
})
...
async function criar() {
  ...
  try {
    ...
    router.push(`/simulacao/${pid}?agentes=${agentes.value}&rodadas=${rodadas.value}&periodo=${periodo.value}&escala=${escalaParaRoute.value}`)
  } catch (e) {
    ...
    <div class="page">
      ...
      <Transition name="slide" mode="out-in">
        ...
        <div v-else-if="etapa === 4" key="e4" class="card">
          ...
          <div class="param">
            <div class="param-header">
              <span class="param-label">Período de Análise</span>
              <span class="param-val">{{ periodo }} meses</span>
            </div>
            <input type="range" min="1" max="60" step="1" v-model.number="periodo" class="slider"/>
            <div class="param-bounds"><span>1 mês</span><span>60 meses (5 anos)</span></div>
            <div class="param-desc">{{ descPeriodo }}</div>
          </div>
          ...
          <div class="param">
            ...
            <div class="param-bounds"><span>1 — rápido</span><span>100 — granular</span></div>
            <div class="param-desc">{{ descRodadas }}</div>
          </div>

          <!-- Escala calculada -->
          <div class="escala-auto">
            <span class="escala-icon">📐</span>
            <span class="escala-text">
              <b>{{ periodo }} meses</b> em <b>{{ rodadas }} rodadas</b> = cada rodada simula <b>{{ escalaLabel }}</b>
            </span>
          </div>
          ...
          <div class="estimativas">
            ...
            <div class="est-sep"></div>
            <div class="est"><div class="est-l">📅 Período</div><div class="est-v accent2">{{ periodo }}m / {{ rodadas }}r</div></div>
          </div>
          ...
          <div class="resumo">
            ...
            <div class="resumo-linha"><span class="rk">Simulação</span><span class="rv">{{ agentes }} agentes · {{ periodo }} meses · {{ rodadas }} rodadas ({{ escalaLabel }}/rodada)</span></div>
          </div>
...
/* Escala calculada */
.escala-auto { display:flex; align-items:center; gap:10px; padding:12px 16px; background:linear-gradient(90deg, rgba(124,111,247,0.06), rgba(0,229,195,0.04)); border:1px solid rgba(124,111,247,0.15); border-radius:12px; }
.escala-icon { font-size:18px; }
.escala-text { font-size:13px; color:var(--text-secondary); line-height:1.5; }
.escala-text b { color:var(--text-primary); }

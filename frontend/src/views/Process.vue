  ...
  <div class="process-page">
    ...
    <nav class="navbar">
      ...
      <div class="nav-center">
        <div class="step-badge">{{ $t('process.navbar.step') }} 01</div>
        <div class="step-name">{{ $t('main.steps.step1') }}</div>
      </div>
    ...
    <div class="main-content">
      ...
      <div class="left-panel" :class="{ 'full-screen': isFullScreen }">
        <div class="panel-header">
          <div class="header-left">
            ...
            <span class="header-title">{{ $t('process.left_panel.title') }}</span>
          </div>
          <div class="header-right">
            <template v-if="graphData">
              <span class="stat-item">{{ graphData.node_count || graphData.nodes?.length || 0 }} {{ $t('process.left_panel.nodes') }}</span>
              <span class="stat-divider">|</span>
              <span class="stat-item">{{ graphData.edge_count || graphData.edges?.length || 0 }} {{ $t('process.left_panel.edges') }}</span>
              <span class="stat-divider">|</span>
            </template>
            <div class="action-buttons">
                <button class="action-btn" @click="refreshGraph" :disabled="graphLoading" :title="$t('process.left_panel.refresh')">
                  <span class="icon-refresh" :class="{ 'spinning': graphLoading }">↻</span>
                </button>
                <button class="action-btn" @click="toggleFullScreen" :title="isFullScreen ? $t('process.left_panel.fullscreen_exit') : $t('process.left_panel.fullscreen_enter')">
        ...
        <div class="graph-container" ref="graphContainer">
          ...
          <div v-if="graphData" class="graph-view">
            ...
            <div v-if="currentPhase === 1" class="graph-building-hint">
              ...
              {{ $t('process.left_panel.updating') }}
            </div>
            
            <!-- 节点/边详情面板 -->
            <div v-if="selectedItem" class="detail-panel">
              <div class="detail-panel-header">
                <span class="detail-title">{{ selectedItem.type === 'node' ? $t('process.left_panel.node_details') : $t('process.left_panel.relationship_details') }}</span>
              ...
              <div v-if="selectedItem.type === 'node'" class="detail-content">
                ...
                <div class="detail-row" v-if="selectedItem.data.created_at">
                  <span class="detail-label">{{ $t('history.created_at') }}:</span>
                  <span class="detail-value">{{ formatDate(selectedItem.data.created_at) }}</span>
                </div>
                
                <!-- Properties / Attributes -->
                <div class="detail-section" v-if="selectedItem.data.attributes && Object.keys(selectedItem.data.attributes).length > 0">
                  <span class="detail-label">{{ $t('step1.ontology.attributes') }}:</span>
                ...
                <div class="detail-section" v-if="selectedItem.data.summary">
                  <span class="detail-label">{{ $t('step4.interview.interview_summary') }}:</span>
              ...
              <div v-else class="detail-content">
                ...
                <div class="detail-subtitle">{{ $t('step1.ontology.relation') }}</div>
                ...
                <div class="detail-row">
                  <span class="detail-label">{{ $t('common.label') }}:</span>
          ...
          <div v-else-if="graphLoading" class="graph-loading">
            ...
            <p class="loading-text">{{ $t('process.left_panel.loading') }}</p>
          </div>
          ...
          <div v-else-if="currentPhase < 1" class="graph-waiting">
            ...
            <p class="waiting-text">{{ $t('process.left_panel.waiting_ontology') }}</p>
            <p class="waiting-hint">{{ $t('process.left_panel.waiting_ontology_hint') }}</p>
          </div>
          ...
          <div v-else-if="currentPhase === 1 && !graphData" class="graph-waiting">
            ...
            <p class="waiting-text">{{ $t('process.left_panel.building') }}</p>
            <p class="waiting-hint">{{ $t('process.left_panel.building_hint') }}</p>
          </div>
      ...
      <div class="right-panel" :class="{ 'hidden': isFullScreen }">
        <div class="panel-header dark-header">
          ...
          <span class="header-title">{{ $t('process.right_panel.title') }}</span>
        </div>
        ...
        <div class="process-content">
          ...
          <div class="process-phase" :class="{ 'active': currentPhase === 0, 'completed': currentPhase > 0 }">
            <div class="phase-header">
              ...
              <div class="phase-info">
                <div class="phase-title">{{ $t('process.right_panel.phase1.title') }}</div>
            ...
            <div class="phase-detail">
              <div class="detail-section">
                <div class="detail-label">{{ $t('process.right_panel.api_desc') }}</div>
                <div class="detail-content">
                  {{ $t('process.right_panel.phase1.desc') }}
                </div>
              </div>
              
              <!-- 本体生成进度 -->
              <div class="detail-section" v-if="ontologyProgress && currentPhase === 0">
                <div class="detail-label">{{ $t('process.right_panel.progress') }}</div>
              ...
              <div class="detail-section" v-if="projectData?.ontology">
                <div class="detail-label">{{ $t('process.right_panel.entity_types') }} ({{ projectData.ontology.entity_types?.length || 0 }})</div>
              ...
              <div class="detail-section" v-if="projectData?.ontology">
                <div class="detail-label">{{ $t('process.right_panel.relation_types') }} ({{ projectData.ontology.relation_types?.length || 0 }})</div>
              ...
              <div class="detail-section waiting-state" v-if="!projectData?.ontology && currentPhase === 0 && !ontologyProgress">
                <div class="waiting-hint">{{ $t('process.right_panel.waiting_phase1') }}</div>
              </div>
            </div>
          </div>
          ...
          <div class="process-phase" :class="{ 'active': currentPhase === 1, 'completed': currentPhase > 1 }">
            <div class="phase-header">
              ...
              <div class="phase-info">
                <div class="phase-title">{{ $t('process.right_panel.phase2.title') }}</div>
            ...
            <div class="phase-detail">
              <div class="detail-section">
                <div class="detail-label">{{ $t('process.right_panel.api_desc') }}</div>
                <div class="detail-content">
                  {{ $t('process.right_panel.phase2.desc') }}
                </div>
              </div>
              
              <!-- 等待本体完成 -->
              <div class="detail-section waiting-state" v-if="currentPhase < 1">
                <div class="waiting-hint">{{ $t('process.right_panel.waiting_phase2') }}</div>
              </div>
              ...
              <div class="detail-section" v-if="graphData">
                <div class="detail-label">{{ $t('process.right_panel.result') }}</div>
                <div class="build-result">
                  <div class="result-item">
                    <span class="result-value">{{ graphData.node_count }}</span>
                    <span class="result-label">{{ $t('process.right_panel.nodes_count') }}</span>
                  </div>
                  <div class="result-item">
                    <span class="result-value">{{ graphData.edge_count }}</span>
                    <span class="result-label">{{ $t('process.right_panel.edges_count') }}</span>
                  </div>
                  <div class="result-item">
                    <span class="result-value">{{ entityTypes.length }}</span>
                    <span class="result-label">{{ $t('process.right_panel.types_count') }}</span>
                  </div>
                </div>
              </div>
            </div>
          ...
          <div class="process-phase" :class="{ 'active': currentPhase === 2, 'completed': currentPhase > 2 }">
            <div class="phase-header">
              ...
              <div class="phase-info">
                <div class="phase-title">{{ $t('process.right_panel.phase3.title') }}</div>
                <div class="phase-api">{{ $t('process.right_panel.phase3.meta') }}</div>
              </div>
          ...
          <div class="next-step-section" v-if="currentPhase >= 2">
            <button class="next-step-btn" @click="goToNextStep" :disabled="currentPhase < 2">
              {{ $t('process.right_panel.next_step') }}
        ...
        <div class="project-panel">
          <div class="project-header">
            ...
            <span class="project-title">{{ $t('process.project_info.title') }}</span>
          </div>
          <div class="project-details" v-if="projectData">
            <div class="project-item">
              <span class="item-label">{{ $t('process.project_info.name') }}</span>
              <span class="item-value">{{ projectData.name }}</span>
            </div>
            <div class="project-item">
              <span class="item-label">{{ $t('process.project_info.id') }}</span>
              <span class="item-value code">{{ projectData.project_id }}</span>
            </div>
            <div class="project-item" v-if="projectData.graph_id">
              <span class="item-label">{{ $t('process.project_info.graph_id') }}</span>
              <span class="item-value code">{{ projectData.graph_id }}</span>
            </div>
            <div class="project-item">
              <span class="item-label">{{ $t('process.project_info.requirement') }}</span>
...
const statusText = computed(() => {
  if (error.value) return t('process.status.failed')
  if (currentPhase.value >= 2) return t('process.status.completed')
  if (currentPhase.value === 1) return t('process.status.building')
  if (currentPhase.value === 0) return t('process.status.ontology')
...
const getPhaseStatusText = (phase) => {
  if (currentPhase.value > phase) return t('process.status.done')
  if (currentPhase.value === phase) {
    if (phase === 1 && buildProgress.value) {
      return `${buildProgress.value.progress}%`
    }
    return t('process.status.processing')
  }
  return t('process.status.pending')
}

  ...
  <div class="process-page">
    ...
    <nav class="navbar">
      ...
      <div class="nav-center">
        ...
        <div class="step-name">{{ $t('process.graphBuildTitle') }}</div>
      </div>

      <div class="nav-status">
        <span class="status-dot" :class="statusClass"></span>
        <span class="status-text">{{ statusText }}</span>
        <button class="lang-toggle" @click="toggleLocale">{{ $t('nav.langSwitch') }}</button>
      </div>
    </nav>
    ...
    <div class="main-content">
      ...
      <div class="left-panel" :class="{ 'full-screen': isFullScreen }">
        ...
        <div class="graph-container" ref="graphContainer">
          <!-- Graph visualization (show when data available) -->
          <div v-if="graphData" class="graph-view">
            <svg ref="graphSvg" class="graph-svg"></svg>
            <!-- Building hint -->
            <div v-if="currentPhase === 1" class="graph-building-hint">
              <span class="building-dot"></span>
              {{ $t('process.realtimeUpdate') }}
            </div>
          ...
          <div v-else-if="currentPhase < 1" class="graph-waiting">
            ...
            <p class="waiting-hint">{{ $t('process.ontologyHint') }}</p>
          </div>
          ...
          <div class="process-phase" :class="{ 'active': currentPhase === 0, 'completed': currentPhase > 0 }">
            <div class="phase-header">
              ...
              <div class="phase-info">
                <div class="phase-title">{{ $t('process.ontologyGen') }}</div>
            ...
            <div class="phase-detail">
              <div class="detail-section">
                <div class="detail-label">{{ $t('process.apiDescription') }}</div>
              ...
              <div class="detail-section" v-if="projectData?.ontology">
                <div class="detail-label">{{ $t('process.entityTypes', { count: projectData.ontology.entity_types?.length || 0 }) }}</div>
              ...
              <div class="detail-section" v-if="projectData?.ontology">
                <div class="detail-label">{{ $t('process.relationTypes', { count: projectData.ontology.relation_types?.length || 0 }) }}</div>
                <div class="relation-list">
                  ...
                  <div v-if="(projectData.ontology.relation_types?.length || 0) > 5" class="relation-more">
                    {{ $t('process.moreRelations', { n: projectData.ontology.relation_types.length - 5 }) }}
                  </div>
                </div>
              </div>
              ...
              <div class="detail-section waiting-state" v-if="!projectData?.ontology && currentPhase === 0 && !ontologyProgress">
                <div class="waiting-hint">{{ $t('process.waitingForOntology') }}</div>
              </div>
            </div>
          </div>
          ...
          <div class="process-phase" :class="{ 'active': currentPhase === 1, 'completed': currentPhase > 1 }">
            ...
            <div class="phase-detail">
              ...
              <div class="detail-section" v-if="graphData">
                <div class="detail-label">{{ $t('process.buildResults') }}</div>
                <div class="build-result">
                  <div class="result-item">
                    <span class="result-value">{{ graphData.node_count }}</span>
                    <span class="result-label">{{ $t('process.entityNodesLabel') }}</span>
                  </div>
                  <div class="result-item">
                    <span class="result-value">{{ graphData.edge_count }}</span>
                    <span class="result-label">{{ $t('process.edgesLabel') }}</span>
                  </div>
                  <div class="result-item">
          ...
          <div class="process-phase" :class="{ 'active': currentPhase === 2, 'completed': currentPhase > 2 }">
            <div class="phase-header">
              ...
              <div class="phase-info">
                ...
                <div class="phase-api">{{ $t('process.readyForNext') }}</div>
              </div>
...
const statusText = computed(() => {
  ...
  if (currentPhase.value === 0) return t('process.statusOntologyGen')
...
const goToNextStep = () => {
  alert(t('process.envSetupDeveloping'))
}
...
// Init - handle new or existing project
...
// Handle new project - call ontology/generate
const handleNewProject = async () => {
  const pending = getPendingUpload()
  
  if (!pending.isPending || pending.files.length === 0) {
    error.value = 'No pending files found, please return to the home page.'
    loading.value = false
    return
  }
  ...
  try {
    ...
    error.value = 'Project init failed: ' + (err.message || t('common.unknownError'))
  } finally {
    loading.value = false
  }
}
...
const loadProject = async () => {
  try {
    ...
    error.value = 'Failed to load project: ' + (err.message || t('common.unknownError'))
  } finally {
    loading.value = false
  }
}
...
const startBuildGraph = async () => {
  try {
    ...
    if (response.success) {
      ...
      // Start graph data polling (separate from task polling)
      startGraphPolling()
    ...
    error.value = 'Failed to start graph build: ' + (err.message || t('common.unknownError'))
    buildProgress.value = null
  }
}
  ...
  try {
    ...
    if (response.success) {
      ...
      if (task.status === 'completed') {
        ...
        stopPolling()
        stopGraphPolling()
        error.value = 'Graph build failed: ' + (task.error || t('common.unknownError'))
        buildProgress.value = null
      }
    }
  } catch (err) {
...
const renderGraph = () => {
  ...
  if (nodesData.length === 0) {
    ...
    svg.append('text')
      ...
      .attr('fill', '#999')
      .text(t('graphPanel.waitingOntology'))
    return
  }
  ...
  // Transparent wide line for click area
  ...
  simulation.on('tick', () => {
    // Update all edge positions (visible + transparent)

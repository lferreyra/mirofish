  ...
  <div class="process-page">
    ...
    <nav class="navbar">
      ...
      <div class="nav-center">
        ...
        <div class="step-name">{{ t('process.step1') }}</div>
      </div>
      ...
      <div class="left-panel" :class="{ 'full-screen': isFullScreen }">
        <div class="panel-header">
          <div class="header-left">
            ...
            <span class="header-title">{{ t('process.graph') }}</span>
          </div>
          <div class="header-right">
            ...
            <div class="action-buttons">
                <button class="action-btn" @click="refreshGraph" :disabled="graphLoading" :title="t('process.refresh')">
        ...
        <div class="graph-container" ref="graphContainer">
          ...
          <div v-else-if="graphLoading" class="graph-loading">
            ...
            <p class="loading-text">{{ t('process.loading') }}</p>
          </div>
          ...
          <div v-else-if="currentPhase < 1" class="graph-waiting">
            ...
            <p class="waiting-text">{{ t('process.waiting') }}</p>
            <p class="waiting-hint">{{ t('process.waitingHint') }}</p>
          </div>
        ...
        <div class="process-content">
          ...
          <div class="process-phase" :class="{ 'active': currentPhase === 0, 'completed': currentPhase > 0 }">
            <div class="phase-header">
              ...
              <div class="phase-info">
                <div class="phase-title">{{ t('process.phase1') }}</div>
            ...
            <div class="phase-detail">
              <div class="detail-section">
                <div class="detail-label">{{ t('process.interfaceDesc') }}</div>
                <div class="detail-content">
                  {{ t('process.phase1Desc') }}
                </div>
              </div>
          ...
          <div class="process-phase" :class="{ 'active': currentPhase === 1, 'completed': currentPhase > 1 }">
            <div class="phase-header">
              ...
              <div class="phase-info">
                <div class="phase-title">{{ t('process.phase2') }}</div>
            ...
            <div class="phase-detail">
              <div class="detail-section">
                <div class="detail-label">{{ t('process.interfaceDesc') }}</div>
                <div class="detail-content">
                  {{ t('process.phase2Desc') }}
                </div>
              </div>
              
              <!-- 等待本体完成 -->
              <div class="detail-section waiting-state" v-if="currentPhase < 1">
                <div class="waiting-hint">{{ t('process.waitingForOntologyComplete') }}</div>
              </div>
              ...
              <div class="detail-section" v-if="graphData">
                ...
                <div class="build-result">
                  ...
                  <div class="result-item">
                    ...
                    <span class="result-label">{{ t('process.relationshipEdges') }}</span>
                  </div>
                  <div class="result-item">
          ...
          <div class="process-phase" :class="{ 'active': currentPhase === 2, 'completed': currentPhase > 2 }">
            <div class="phase-header">
              ...
              <div class="phase-info">
                <div class="phase-title">{{ t('process.phase3') }}</div>
                <div class="phase-api">{{ t('process.phase3Api') }}</div>
              </div>
...
const statusText = computed(() => {
  ...
  if (currentPhase.value === 0) return t('process.status.generating')
...
const getPhaseStatusText = (phase) => {
  if (currentPhase.value > phase) return t('process.status.completedStatus')
  if (currentPhase.value === phase) {
    if (phase === 1 && buildProgress.value) {
      return `${buildProgress.value.progress}%`
    }
    return t('process.status.inProgress')
  }
  return t('process.status.waiting')
}
...
const handleNewProject = async () => {
  ...
  try {
    ...
    error.value = t('process.initFailed', { error: err.message || 'Unknown error' })
  } finally {
    loading.value = false
  }
}
...
const loadProject = async () => {
  try {
    ...
    if (response.success) {
      ...
      error.value = response.error || t('process.loadProjectFailed', { error: '' })
    }
  } catch (err) {
    console.error('Load project error:', err)
    error.value = t('process.loadProjectFailed', { error: err.message || 'Unknown error' })
  } finally {
    loading.value = false
  }
}
...
const startBuildGraph = async () => {
  try {
    ...
    if (response.success) {
      buildProgress.value.message = t('process.graphBuildStarted')
      ...
      error.value = response.error || t('process.startGraphBuildFailed', { error: '' })
      buildProgress.value = null
    }
  } catch (err) {
    console.error('Build graph error:', err)
    error.value = t('process.startGraphBuildFailed', { error: err.message || 'Unknown error' })
    buildProgress.value = null
  }
}
  ...
  try {
    ...
    if (response.success) {
      ...
      if (task.status === 'completed') {
        ...
        stopPolling()
        stopGraphPolling()
        error.value = t('process.graphBuildFailed', { error: task.error || 'Unknown error' })
        buildProgress.value = null
      }
    }
  } catch (err) {
